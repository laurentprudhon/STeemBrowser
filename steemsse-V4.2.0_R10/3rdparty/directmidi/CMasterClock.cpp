...
/* https://learn.microsoft.com/en-us/previous-versions/ms808947(v=msdn.10)
Remarks

If another running application is also using DirectMusic, it is not possible to change the master
clock until that application is shut down.

Most applications do not need to call SetMasterClock. It should not be called unless there is a need
to synchronize tightly with a hardware timer other than the system clock.
*/
	hr = m_pMusic8->SetMasterClock(ClockInfo->guidClock);
#else
