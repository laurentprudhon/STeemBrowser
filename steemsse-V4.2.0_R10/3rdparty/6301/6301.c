...
extern BYTE ST_Key_Down[MAX_ST_KEYS];
extern int mousek;
extern DWORD nSysCyclesPerSecond;
extern BYTE NumJoysticks; // USB controllers

// functions from Steem (declared as extern "C" there)
extern void joy_read_buttons(int njoys);
extern BYTE joy_get_pos(int joy);
...
#endif
#else 
#define ASSERT(x) {if(!(x)) TRACE("%s: %s\n","ASSERT",#x);} //just a trace
#endif
#else
...
hd6301_run_cycles(COUNTER_VAR to_m68_cycle) {
  ...
  cycles_run*=nSysCyclesPerSecond;
  cycles_run/=clk6301;
  if(SSEConfig.CpuBoosted)
...
hd6301_receive_byte(BYTE byte_in) {
