...
recently executed instructions. There is also a small list in the menu.
---------------------------------------------------------------------------*/
...
void THistoryList::Show() {
  ...
  // can be on taskbar:
  Handle=CreateWindowEx(WS_EX_CONTROLPARENT|mem_browser::ex_style,"Steem History List","History List",
  ...
  HWND hLB=CreateWindowEx(WS_EX_CLIENTEDGE,"ListBox","History",WS_CHILD|
    WS_VISIBLE|WS_VSCROLL|LBS_USETABSTOPS|WS_TABSTOP|LBS_NOINTEGRALHEIGHT|
    LBS_HASSTRINGS|LBS_NOTIFY,10,10,280,250,Handle,(HMENU)100,hInstance,NULL);
  ...
  CreateWindow("Button","Dump to Trace",WS_CHILDWINDOW|WS_VISIBLE|WS_TABSTOP
    |BS_PUSHBUTTON,10,267,135,23,Handle,(HMENU)101,hInstance,NULL);
#else
...
#endif
  CreateWindow("Button","Refresh",WS_CHILDWINDOW|WS_VISIBLE|WS_TABSTOP
    |BS_PUSHBUTTON,155,267,135,23,Handle,(HMENU)102,hInstance,NULL);
...
void THistoryList::RefreshHistoryBox(bool dump) {
  ...
  int nentries=(DEBUGGER_CONTROL_MASK1&DEBUGGER_CONTROL_LARGE_HISTORY) ? HISTORY_SIZE : 1000;
  ...
  do
  {
    ...
    if(pc_history[n]==MAGIC_HIST_BLIT)
#if defined(SSE_420R5) // in cycles
      Disassembly=Str("BLiT (") + Str(pc_history_c[n]/TICKS8) + Str(" cycles)");
#elif defined(SSE_420R2)
      Disassembly=Str("BLiT ") + Str(pc_history_c[n]) + Str(" words");
#else
      Disassembly="BLiT";
#endif
      ...
      Disassembly=Str("irq ")+Str((pc_history[n]>>16)&0xff)+"-"+Str((pc_history[n]>>8)&0xFF);
    else
    ...
    sprintf(timing,"%d %d",pc_history_y[n],pc_history_c[n]/TICKS8);
#else
    ...
    if(dump)
    {
      ...
      if(DEBUGGER_CONTROL_MASK1&DEBUGGER_CONTROL_HISTORY_TMG)
        sprintf(str,"%s%s %s - %s",header.Text,HEXSl(pc_history[n],6).Text,timing,Disassembly.Text);
      else
        sprintf(str,"%s%s - %s",header.Text,HEXSl(pc_history[n],6).Text,Disassembly.Text);
      TRACE("%s\n",str);
    }
    else // refresh
    {
      if(DEBUGGER_CONTROL_MASK1&DEBUGGER_CONTROL_HISTORY_TMG)
        SendMessage(Win,LB_INSERTSTRING,(WPARAM)-1,(LPARAM)((header
        +HEXSl(pc_history[n],6)+" - "+timing+" - "+Disassembly).Text)); //-1 end of list
      else
        SendMessage(Win,LB_INSERTSTRING,(WPARAM)-1,(LPARAM)((header
        +HEXSl(pc_history[n],6)+" - "+Disassembly).Text));
...
LRESULT CALLBACK THistoryList::WndProc(HWND Win,UINT Mess,WPARAM wPar,LPARAM lPar) {
  LRESULT Ret=DefStemDialogProc(Win,Mess,wPar,lPar);
  if(StemDialog_RetDefVal) 
    return Ret;
  THistoryList *This=(THistoryList*)GetWindowLongPtr(Win,GWLP_USERDATA);
  ...
  case WM_COMMAND:
    if(wpar_lo==100)
    {
      ...
      {
        HWND hwnd=GetDlgItem(Win,100);
        LRESULT Selected=SendMessage(hwnd,LB_GETCURSEL,0,0);
        PostMessage(DWin,WM_COMMAND,17000+SendMessage(hwnd,LB_GETITEMDATA,Selected,0),0);
      }
    }
    else if(wpar_lo==101)
    {
      ...
      {
#if defined(SSE_DEBUGGER) // originally it was toggle breakpoint
    ...
    else if(wpar_lo==102 && wpar_hi==BN_CLICKED)
      This->RefreshHistoryBox();
    break;
  case WM_SIZE:
    SetWindowPos(GetDlgItem(Win,100),0,0,0,lpar_lo-20,lpar_hi-50,SWP_NOMOVE|SWP_NOZORDER);
    SetWindowPos(GetDlgItem(Win,101),0,10,lpar_hi-30,(lpar_lo/2)-15,23,SWP_NOZORDER);
    SetWindowPos(GetDlgItem(Win,102),0,lpar_lo/2+5,lpar_hi-30,lpar_lo/2-15,23,SWP_NOZORDER);
    if(!IsIconic(Win))
    {
      if(!IsZoomed(Win))
      {
