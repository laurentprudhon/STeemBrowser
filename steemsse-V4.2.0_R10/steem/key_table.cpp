...
Because they tend to get trashed when moving between Windows and Linux,
international characters aren't included anymore in this file, even in comments.
...
bool bEnableShiftSwitching=false;
...
void SetSTKeys(char *Letters,int Val1,...) {
  ...
  do
  {
    ...
    if(HIBYTE(Code)==0) // VK code exists + No PC modifier required
      key_table[(BYTE)Code]=LOBYTE(arg);
    arg=va_arg(vl,int);
  } while(Letters[++l]);
  va_end(vl);
}
...
#define PC_SHIFT BIT_0
#define NO_PC_SHIFT 0
#define PC_ALT BIT_1
#define NO_PC_ALT 0

#define ST_SHIFT BIT_0
#define NO_ST_SHIFT 0
#define ST_ALT BIT_1
#define NO_ST_ALT 0


void AddToShiftSwitchTable(int PCModifiers,int PCAscii,BYTE STModifiers,BYTE STCode) {
  BYTE Code;
#ifdef WIN32
  Code=(BYTE)(VkKeyScanEx((CHAR)PCAscii,keyboard_layout));
#endif
#ifdef UNIX
...
#define SHIFT BIT_0 // ST or PC
#define ALT BIT_1 // ST or PC

bool ReadMappingFile(char *path) {
  bool ok=false;
  TRACE_INIT("Parsing %s, EnableShiftSwitching %d ",CHECKPATH(path),bEnableShiftSwitching);
  TRACE_INIT("T%X-%d\n",tos_version,SSEConfig.TosLanguage);
  FILE *fp=fopen(path,"rb"); // it's a text file but we treat it as binary
  if(fp!=NULL)
  {
    size_t fl=(size_t)GetFileLength(fp);
...
void InitKeyTable() {
  ...
  */
  ...
  SetSTKeys("zxcvbnm",0x2C,0x2D,0x2E,0x2F,0x30,0x31,0x32);
  ...
  // reinit [this part was missing in the 402 beta... notice player could create a map!]
  SetSTKeys("1234567890-=",0x2,0x3,0x4,0x5,0x6,0x7,0x8,0x9,0xA,0xB,0xC,0xD);
  SetSTKeys("qwertyuiop[]",0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1A,0x1B);
  SetSTKeys("asdfghjkl;'",0x1E,0x1F,0x20,0x21,0x22,0x23,0x24,0x25,0x26,0x27,0x28);
  ...
  case LANG_ENGLISH:
    if(SubLang==SUBLANG_ENGLISH_AUS)
    {
      ...
      AddToShiftSwitchTable(PC_SHIFT,'2',ST_SHIFT,0x28);
      AddToShiftSwitchTable(PC_SHIFT,'\'',ST_SHIFT,0x3);
      AddToShiftSwitchTable(PC_SHIFT,'3',NO_ST_SHIFT,0x2b);
      AddToShiftSwitchTable(PC_SHIFT,'`',ST_SHIFT,0x2b);
    }
  ...
  case LANG_FRENCH:
    if(SubLang==SUBLANG_FRENCH_BELGIAN) // PC keyboard is AZERTY Belgian
    {
      ...
      AddToShiftSwitchTable(PC_ALT,0xe9,ST_ALT,0x2b);
      AddToShiftSwitchTable(PC_ALT,'\"',0,0x2b);
      AddToShiftSwitchTable(PC_ALT,0xe7,ST_ALT+ST_SHIFT,0x1a);
      AddToShiftSwitchTable(PC_ALT,0xe0,ST_ALT+ST_SHIFT,0x1b);
      AddToShiftSwitchTable(PC_ALT,'^',ST_ALT,0x1a);
      AddToShiftSwitchTable(PC_ALT,'$',ST_ALT,0x1b);
      AddToShiftSwitchTable(PC_ALT,0xb5,0,0x29);
    ...
    { 
      ...
      // French ST keyboard has Section sign on 6 key
...
//      AddToShiftSwitchTable(0,'-',0,0xd);
      AddToShiftSwitchTable(PC_SHIFT,'-',ST_SHIFT,0x7);
      AddToShiftSwitchTable(0,'_',ST_SHIFT,0xd);
      AddToShiftSwitchTable(PC_SHIFT,'_',ST_SHIFT,0x9);
      AddToShiftSwitchTable(0,'!',0,0x9);
      AddToShiftSwitchTable(PC_SHIFT,'!',0,0x7);
      AddToShiftSwitchTable(PC_SHIFT,'$',ST_SHIFT,0x29);
      AddToShiftSwitchTable(PC_ALT,0xe9,ST_ALT+ST_SHIFT,0x2b);
      AddToShiftSwitchTable(PC_ALT,'\"',0,0x2b);
      AddToShiftSwitchTable(PC_ALT,'\'',ST_ALT+ST_SHIFT,0x1a);
      AddToShiftSwitchTable(PC_ALT,'(',ST_ALT,0x1a);
      AddToShiftSwitchTable(PC_ALT,'-',ST_SHIFT,0x2b);
      AddToShiftSwitchTable(PC_ALT,0xe8,0,0x29);
      AddToShiftSwitchTable(PC_ALT,'_',ST_ALT,0x28);
//      AddToShiftSwitchTable(PC_ALT,0xe7,0,0x1a);
      AddToShiftSwitchTable(PC_ALT,0xe0,ST_ALT,0x2b);
      AddToShiftSwitchTable(PC_ALT,')',ST_ALT,0x1b);
...
//    AddToShiftSwitchTable(PC_SHIFT,'^',NO_ST_SHIFT,0x2b);
    ...
    AddToShiftSwitchTable(PC_ALT,0xdf,ST_ALT+ST_SHIFT,0x1a);
  ...
  case LANG_BASQUE:
    ...
    AddToShiftSwitchTable(PC_SHIFT,'3',NO_ST_SHIFT,0x71);
    AddToShiftSwitchTable(PC_SHIFT,'6',ST_SHIFT,0x8);
    AddToShiftSwitchTable(PC_SHIFT,'7',ST_SHIFT,0x7);
    AddToShiftSwitchTable(PC_SHIFT,'8',ST_SHIFT,0xa);
    AddToShiftSwitchTable(PC_SHIFT,'9',ST_SHIFT,0xb);
    AddToShiftSwitchTable(PC_SHIFT,'\''/*39 */,ST_SHIFT,0x33);
    AddToShiftSwitchTable(NO_PC_SHIFT,0xba,NO_ST_SHIFT,0x35);
//    AddToShiftSwitchTable(PC_SHIFT,0xba,NO_ST_SHIFT,0x2b);
    AddToShiftSwitchTable(NO_PC_SHIFT,0xa1,ST_SHIFT,0x2);
    AddToShiftSwitchTable(PC_SHIFT,0xa1,ST_SHIFT,0x3);
    AddToShiftSwitchTable(NO_PC_SHIFT,'+',ST_SHIFT,0xd);
    AddToShiftSwitchTable(PC_SHIFT,'+',NO_ST_SHIFT,0x66);
    AddToShiftSwitchTable(PC_SHIFT,',',NO_ST_SHIFT,0x28);
    AddToShiftSwitchTable(PC_SHIFT,'.',ST_SHIFT,0x28);
    AddToShiftSwitchTable(PC_SHIFT,'0',NO_ST_SHIFT,0x0d);
    AddToShiftSwitchTable(PC_ALT,0xba,0,0x2b);
    AddToShiftSwitchTable(PC_ALT,'1',ST_SHIFT,0x2b);
    AddToShiftSwitchTable(PC_ALT,'2',ST_ALT+ST_SHIFT,0x2b);
    AddToShiftSwitchTable(PC_ALT,'3',ST_ALT,0x2b);
    AddToShiftSwitchTable(PC_ALT,0x60,ST_ALT,0x1a);
    AddToShiftSwitchTable(PC_ALT,'+',ST_ALT,0x1b);
    AddToShiftSwitchTable(PC_ALT,0xb4,ST_ALT+ST_SHIFT,0x1a);
    AddToShiftSwitchTable(PC_ALT,0xe7,ST_ALT+ST_SHIFT,0x1b);
    AddToShiftSwitchTable(PC_ALT,'4',ST_SHIFT,0x29);
    break;
  case LANG_ITALIAN:
    // missing: good ST keyboard pic
...
void UNIX_get_fake_VKs() {
  ...
  VK_F1=XKeysymToKeycode(XD,XK_F1);
