...
HRESULT PA_StartBuffer(int flatlevel1,int flatlevel2) {
  ...
  {
    TRACE_LOG("portaudio %s\n",sound_device_name.Text);
  }
...
int PA_Callback(const void*,void *pOutBuf,unsigned long bufferSize, const 
  ...
  if(pOutBuf==NULL || !bufferSize || !psg_buf_length) 
    return 0;
...
HRESULT PA_Stop(bool Immediate) {
  ...
  {
    FloppyDrive[DRIVE_A].Sound_ReleaseBuffers();
    FloppyDrive[DRIVE_B].Sound_ReleaseBuffers();
  }
#endif  
  return DS_OK;
}
