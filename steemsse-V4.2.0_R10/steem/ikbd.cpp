...
DOMAIN: Emu
FILE: ikbd.cpp
DESCRIPTION: The code to emulate the ST's Intelligent Keyboard Controller
...
for the mouse pointer although it has been improved in the low-level emu too.
...
#define IKBD_JOY_MODE_MONITORING 100
#define IKBD_JOY_MODE_FIRE_BUTTON_MONITORING 101


BYTE keyboard_buffer[MAX_KEYBOARD_BUFFER_SIZE];
BYTE ST_Key_Down[MAX_ST_KEYS];
int ikbd_joy_poll_line=0;
int mouse_vbl_delta_x,mouse_vbl_delta_y;
int mousek;
const BYTE ikbd_clock_days_in_mon[13]={0,31,28,31,30,31,30,31,31,30,31,30,31};
const BYTE ikbd_clock_max_val[6]={99,12,0,23,59,59};

WORD keyboard_buffer_length;
BYTE old_stick[2]={0,0};
BYTE mouse_speed=10;
BYTE disable_input_vbl_count=0;
#ifndef CYGWIN
BYTE no_set_cursor_pos=0;
#else
BYTE no_set_cursor_pos=true;
#endif
bool mouse_vbl_delta;

#ifdef WIN32
// When task switching is turned off we have to manually update these keys at the VBL
// TODO does it work with C1?
BYTE const TaskSwitchVKList[4]={VK_ESCAPE,VK_TAB,VK_DELETE,0};
...
bool ikbd_keys_disabled() { // called by HandleKeyPress() if !C1
  return (Ikbd.joy_mode>=IKBD_JOY_MODE_MONITORING);
}


void IKBD_VBL() {
  if(!OPTION_RTC_HACK && (++Ikbd.clock_vbl_count)>=VideoFreqAtStartOfVbl)
  {
  ...
  int max_line=TopScanlines[VideoFreqIdx]+( (MONO_MONITOR) ? 400 : 200);
  ...
  *(WORD*)&old_stick[N_JOY_PORT_0]=*(WORD*)&stick[N_JOY_PORT_0];
  ...
  else
  {
#ifndef SSE_LIBRETRO
    joy_read_buttons();
    for(int Port=N_JOY_PORT_0;Port<=SSEConfig.MaxJoy;Port++)
    ...
    if(!OPTION_C1)
    {
      for(int i=N_JOY_PORT_0;i<=N_JOY_PORT_1;i++)
      {
        if((stick[i]&0x7F) && (old_stick[i]&0x7F)!=(stick[i]&0x7F))
          Stats.nJoy[i]++;
        if((stick[i]&0x80) && (old_stick[i]&0x80)!=(stick[i]&0x80))
          Stats.nClick[i]++;
      }
    }
#endif
    ...
    case TDongle::CRICKET: // port 0 bit 0 toggles (?) // this doesn't work
...
#endif
    // if joystick 0 active, no mouse, simple to check at VBL
#if !defined(SSE_LIBRETRONUKE)
    SSEConfig.Port0Joy=IsJoyActive(N_JOY_PORT_0);
  ...
  case IKBD_JOY_MODE_MONITORING: // rarely used
    //keyboard_buffer_write((BYTE)(int ((stick[0]&MSB_B)?BIT_1:0)
    //   refactored vv               |int((stick[1]&MSB_B)?BIT_0:0)));
    //keyboard_buffer_write( ((stick[0]&MSB_B)>>6) | ((stick[1]&MSB_B)>>7) );
    keyboard_buffer_write(((stick[N_JOY_PORT_0]>>6) | (stick[N_JOY_PORT_1]>>7)) & (BIT_1|BIT_0));
    keyboard_buffer_write(((stick[N_JOY_PORT_0]&0xf)<<4) | (stick[N_JOY_PORT_1]&0xf));
    break;
  case IKBD_JOY_MODE_AUTO_NOTIFY:
    ...
    break;
  case IKBD_JOY_MODE_FIRE_BUTTON_MONITORING:
    if(stick[N_JOY_PORT_1]&MSB_B)
  ...
  case IKBD_JOY_MODE_CURSOR_KEYS: {
    if(stick[N_JOY_PORT_0]&(~old_stick[N_JOY_PORT_0])&0xc) 
    ...
    else if(stick[N_JOY_PORT_0]) 
    {
      for(int xy=0;xy<2;xy++) 
      {
        BYTE s=stick[N_JOY_PORT_0]&(BYTE)(0xc>>(xy*2));
        if(s) 
  ...
  mousek=0;
  //TRACE3("ikbd vbl sticks %X %X\n",stick[N_JOY_PORT_0],stick[N_JOY_PORT_1]);
  if(stick[N_JOY_PORT_0]&BIT_7)
    mousek|=BIT_LMB;
  if(stick[N_JOY_PORT_1]&BIT_7)
    mousek|=BIT_RMB;
  if(stem_mousemode==STEM_MOUSEMODE_WINDOW)
  {
#if defined(SSE_GUI_STATUS_BAR)
    if(SSEConfig.StatusBarMask&0x80) // has moved last frame, should update
    {
      UPDATE_STATUS_BAR_PART(SB_PART_FREQ);
      SSEConfig.StatusBarMask&=0x7F;
    }
#endif
    POINT pt;
    GetCursorPos(&pt);
    if(pt.x!=window_mouse_centre_x||pt.y!=window_mouse_centre_y)
    {
      ...
      if(pt.y!=window_mouse_centre_y)
        Stats.nMousey++;
#endif
      mouse_vbl_delta_x+=(pt.x-window_mouse_centre_x);
      mouse_vbl_delta_y+=(pt.y-window_mouse_centre_y);
      if(mouse_speed!=10) 
      {
        int x_if_0=0;
        if(mouse_vbl_delta_x<0) 
          x_if_0=-1;
        if(mouse_vbl_delta_x>0) 
          x_if_0=1;
        int y_if_0=0;
        if(mouse_vbl_delta_y<0) 
          y_if_0=-1;
        if(mouse_vbl_delta_y>0) 
          y_if_0=1;
        mouse_vbl_delta_x*=mouse_speed;
        mouse_vbl_delta_y*=mouse_speed;
        mouse_vbl_delta_x/=10;
        mouse_vbl_delta_y/=10;
        if(mouse_vbl_delta_x==0) 
          mouse_vbl_delta_x=x_if_0;
        if(mouse_vbl_delta_y==0) 
          mouse_vbl_delta_y=y_if_0;
      }
      if(Ikbd.mouse_upside_down)
        mouse_vbl_delta_y=-mouse_vbl_delta_y;
      if(SSEConfig.Port0Joy)
      {
#if defined(SSE_OSD_DEBUGINFO)
        // confusing to some players so...
        TRACE_OSD_DBG(T("Port 0 (disables mouse!)"));
#endif
        mouse_vbl_delta_x=mouse_vbl_delta_y=0;
      }
      else
...
#if defined(SSE_GUI_STATUS_BAR)
      if(SSEConfig.StatusBarMask&(1<<SB_PART_FREQ)) // mouse coord enabled
        SSEConfig.StatusBarMask|=0x80; // hopefully we don't get 8 parts
#endif
    }
  }
  if(macro_record)
    macro_record_mouse(mouse_vbl_delta_x,mouse_vbl_delta_y);
  if(macro_play_has_mouse) 
  {
    mouse_vbl_delta=false;
    macro_play_mouse(mouse_vbl_delta_x,mouse_vbl_delta_y);
    if(mouse_vbl_delta_x||mouse_vbl_delta_y)
      mouse_vbl_delta=true;
  }
  ...
  if(mouse_vbl_delta) 
  {
    ...
    ikbd_mouse_move(mouse_vbl_delta_x,mouse_vbl_delta_y,mousek,max_mouse_move);
    mouse_vbl_delta=false;
    mouse_vbl_delta_x=mouse_vbl_delta_y=0;
  }
#if defined(SSE_HD6301_LL) && !defined(SSE_LIBRETRO)
  else
  ...
  if(!macro_play_has_keys)
  {
    ...
    bool StemWinActive=(GetForegroundWindow()==StemWin);//GUICanGetKeys();
...
void ikbd_inc_hack(int &hack_val,int inc_val) { // C1 unchecked
  if(!Ikbd.resetting)
    return;
  hack_val=(hack_val==inc_val) ? (inc_val+1) : -1;
}
...
*/
  ...
  if(OPTION_C1)
  {
    ...
    hd6301_run_cycles(A_S_T); // run up to IO time
    hd6301_receive_byte((BYTE)src);
    acia[ACIA_IKBD].LineTxBusy=0;
...
#endif//6301
  Ikbd.send_nothing=FALSE;  // This should only happen if valid command is received!
  if(Ikbd.command_read_count)
  {
    ...
    if(Ikbd.command_read_count<=0)
    {
      ...
      case 0x17://joystick duration
        Ikbd.joy_mode=IKBD_JOY_MODE_MONITORING;
...
*/
    if(Ikbd.joy_mode==IKBD_JOY_MODE_FIRE_BUTTON_MONITORING) 
    ...
    case 0x7:case 0x17:case 0x80:
      Ikbd.command_read_count=1;
      break;
    case 0x8: //return relative mouse position from now on
      ...
      break;
    case 0x9:
      Ikbd.command_read_count=4;
      break;
    case 0xa:case 0xb:case 0xc:case 0x21:case 0x22:
      Ikbd.command_read_count=2;
      break;
    ...
    case 0x10: //mouse goes right way up
      Ikbd.port_0_joy=Ikbd.mouse_upside_down=FALSE;
      break;
    ...
    case 0x18: //fire button duration, constant high speed joystick button test
      Ikbd.joy_mode=IKBD_JOY_MODE_FIRE_BUTTON_MONITORING;
      Ikbd.mouse_mode=IKBD_MOUSE_MODE_OFF;  //disable mouse
      agenda_delete(ikbd_report_abs_mouse);
      Ikbd.port_0_joy=TRUE;
      agenda_delete(ikbd_send_joystick_message); // just in case sending other type of packet
    ...
    case 0x1a: //turn off joysticks
      Ikbd.port_0_joy=FALSE;
      Ikbd.joy_mode=IKBD_JOY_MODE_OFF;
      stick[N_JOY_PORT_0]=stick[N_JOY_PORT_1]=0;
      ikbd_inc_hack(Ikbd.reset_121A_hack,1);
      agenda_delete(ikbd_send_joystick_message); // just in case sending other type of packet
      break;
    case 0x1b:case 0x19:
      Ikbd.command_read_count=6;
      break;
    ...
    case 0x8f:case 0x90: //return 0xf if mouse is upside down, 0x10 otherwise
      keyboard_buffer_write(0xf6);
      keyboard_buffer_write((Ikbd.mouse_upside_down) ? 0xf : 0x10);
      keyboard_buffer_write_string(0,0,0,0,0,0,(-1));
      break;
    case 0x92:  //is mouse off?
      keyboard_buffer_write(0xf6);
      keyboard_buffer_write((Ikbd.mouse_mode==IKBD_MOUSE_MODE_OFF) ? 0x12 : 0x00);
      keyboard_buffer_write_string(0,0,0,0,0,0,(-1));
      break;
    case 0x94:case 0x95:case 0x99: {
      ...
      if(Ikbd.joy_mode==IKBD_JOY_MODE_CURSOR_KEYS)
        for(int n=0;n<6;n++) 
    ...
    case 0x9a:  //is joystick off?
      keyboard_buffer_write(0xf6);
      keyboard_buffer_write((Ikbd.joy_mode==IKBD_JOY_MODE_OFF) ? 0x1a : 0x00);
      keyboard_buffer_write_string(0,0,0,0,0,0,(-1));
      break;
...
void agenda_keyboard_replace(int) {
  ...
  if(OPTION_C1) 
  {
    if(keyboard_buffer_length) 
    {
      ...
      ASSERT(stem_runmode==STEM_MODE_CPU);
    ...
    if(keyboard_buffer_length)
    {
      ...
      acia[ACIA_IKBD].LineRxBusy=1;
      acia[ACIA_IKBD].TimeRx=A_S_T + acia[ACIA_IKBD].TransmissionTime();
      if(acia[ACIA_IKBD].TimeRx-time_of_event_acia<=0)
  ...
  if(keyboard_buffer_length)
  {
    ...
    if(keyboard_buffer_length) 
      agenda_add(agenda_keyboard_replace,ACIAClockToHBLS(acia[ACIA_IKBD].clock_divide),false);
  }
  if(macro_start_after_ikbd_read_count) 
    macro_start_after_ikbd_read_count--;
}
...
*/
    if(keyboard_buffer_length<MAX_KEYBOARD_BUFFER_SIZE)
    {
      ...
      else
      {
        ...
        COUNTER_VAR clk6301=HD6301_CLOCK;
        if(SSEConfig.CpuBoosted)
          clk6301*=SSEConfig.CpuBoost;
        acia[ACIA_IKBD].TimeRx=TimeOfHSyncOff + ((cycles_run*nSysCyclesPerSecond)/clk6301)
...
void keyboard_buffer_write_string(int s1,...) {
  ...
  while(arg!=-1)
  {
    keyboard_buffer_write((BYTE)arg);
    arg=va_arg(vl,int);
  }
  va_end(vl);
}
  ...
  {  //not in duration mode or joystick mode
    ...
    else if(Ikbd.mouse_mode==IKBD_MOUSE_MODE_RELATIVE) 
    {
      ...
      while(abs_quick(x-x1)>max_mouse_move||abs_quick(y-y1)>max_mouse_move) 
      {
        ...
        keyboard_buffer_write((BYTE)x2);
        keyboard_buffer_write((BYTE)y2);
        x1+=x2;
        y1+=y2;
      }
      keyboard_buffer_write((BYTE)(0xf8+(mouseb & 3)));
      keyboard_buffer_write((BYTE)(x-x1));
      keyboard_buffer_write((BYTE)(y-y1));
    }
    else if(Ikbd.mouse_mode==IKBD_MOUSE_MODE_CURSOR_KEYS)
    {
...
void ikbd_reset(bool hardware_reset) {
  ASSERT(hardware_reset||!OPTION_C1);
...
void agenda_keyboard_reset(int SoftwareReset) { // scheduled by ikbd_reset()
  ...
  if(!SoftwareReset)
  {
    ...
    Ikbd.mouse_mode=IKBD_MOUSE_MODE_RELATIVE;
    Ikbd.joy_mode=IKBD_JOY_MODE_AUTO_NOTIFY;
    Ikbd.cursor_key_mouse_pulse_count_x=Ikbd.cursor_key_mouse_pulse_count_y=3;
    Ikbd.relative_mouse_threshold_x=Ikbd.relative_mouse_threshold_y=1;
    Ikbd.abs_mouse_scale_x=Ikbd.abs_mouse_scale_y=1;
    Ikbd.abs_mouse_x=shifter_x/2;
    Ikbd.abs_mouse_y=shifter_y/2;
    Ikbd.abs_mouse_max_x=shifter_x;
    Ikbd.abs_mouse_max_y=shifter_y;
    Ikbd.mouse_upside_down=FALSE;
    Ikbd.send_nothing=Ikbd.port_0_joy=FALSE;
    Ikbd.abs_mousek_flags=Ikbd.psyg_hack_stage=0;
  ...
  else
  {
    ...
    Ikbd.send_nothing=FALSE; // Fix Just Bugging (probably correct though)
    if(!OPTION_C1)
    {
      // Send break codes for "stuck" keys
      // The break code for each key is obtained by ORing 0x80 with the make code.
      for(BYTE n=1;n<118;n++)
        if(ST_Key_Down[n]) 
          keyboard_buffer_write((0x80|n));
    }
  }
  Ikbd.resetting=0;
}
...
void ikbd_send_joystick_message(int jn) {
  BYTE s[2]={stick[N_JOY_PORT_0],stick[N_JOY_PORT_1]};
...
*/
  ...
  // new command?
  else if(CurrentCommand==0xFF && ByteIn) 
  {
    switch(ByteIn) {
    case 0x80: // RESET
...
#endif
#if defined(SSE_OSD_DEBUGINFO)
      OsdControl.Trace(TOsdControl::OUTPUT_SB,"KBD");
#endif
      // no break
  ...
  else if(nParameters>6) //snapshot trouble
    CurrentCommand=0xFF;
  // taking parameters of a command?
  else if(CurrentCommand!=0xFF)
  {
  ...
  {} // could be junk?
  // report?
  if(CurrentCommand!=0xFF && nParameters==CurrentParameter)
  {
#if defined(SSE_IKBDI)
#if defined(SSE_ENABLE_TRACE_LOG)
    ReportCommand();
...
void THD6301::ResetProgram() {
  ...
  CurrentParameter=nParameters=0;
}
...
void THD6301::Init() { // called in 'main'
  ...
  if(fp)
  {
    size_t l=FREAD(pram+rombase,1,romlen,fp);
    add_to_crc32(checksum,pram+rombase,romlen);
    checksum=~checksum;
    TRACE2("%s %s %d bytes %s %08X\n","Load",CHECKPATH(romfile.Text),l,"CRC32",checksum);
...
#ifdef WIN32
    HRSRC rc=FindResource(hInstance,MAKEINTRESOURCE(IDR_HD6301),RT_RCDATA);
    //ASSERT(rc);
    if(rc)
#endif
    {
      ...
      {
#ifdef WIN32        
        size_t n=SizeofResource(hInstance,rc);
        BYTE *pdata=(BYTE*)LockResource(hglob);
#endif
#ifdef UNIX
        BYTE *pdata=(BYTE*)Get_HD6301V1ST_img();
        BYTE *pdata_end=(BYTE*)Get_HD6301V1ST_img_end();
        size_t n=pdata_end-pdata;
#endif
...
void THD6301::Vbl() { // this is called in run.cpp right after IKBD_VBL()
  ...
  BYTE max_pix_h=30,max_pix_v=30;
  switch(screen_res) { // could be optimised
  case LORES:
    break;
  case MEDRES:
    max_pix_h=35;
  ...
  if(!VideoFreqAtStartOfVbl)
    return; // anti-crash
#endif
  int cycles_per_frame=HD6301_CLOCK/VideoFreqAtStartOfVbl; //TODO
  MouseCyclesPerTickX=(MouseVblDeltaX) ? (cycles_per_frame/abs_quicki(MouseVblDeltaX)) : 0;
  MouseCyclesPerTickY=(MouseVblDeltaY) ? (cycles_per_frame/abs_quicki(MouseVblDeltaY)) : 0;
...
#undef IKBD_MOUSE_MODE_ABSOLUTE
#undef IKBD_MOUSE_MODE_RELATIVE
#undef IKBD_MOUSE_MODE_CURSOR_KEYS

#undef IKBD_JOY_MODE_OFF
#undef IKBD_JOY_MODE_ASK
#undef IKBD_JOY_MODE_AUTO_NOTIFY
#undef IKBD_JOY_MODE_CURSOR_KEYS
#undef IKBD_JOY_MODE_MONITORING
#undef IKBD_JOY_MODE_FIRE_BUTTON_MONITORING
