...
mr_static::mr_static(char*new_label,char*new_name,int x,int y,HWND new_owner,
  ...
  if(n<MAX_MR_STATICS)
  {
    ...
    if(*new_label)
    {
      ...
      hLABEL=CreateWindowEx(0,"Static",new_label,WS_VISIBLE|WS_CHILD|SS_RIGHT|SS_CENTERIMAGE,x,y,
                            label_width,CharHeight,owner,id,hInstance,NULL);
    ...
    else
    {
      ...
      if(type!=MST_DECIMAL)
        text_width+=(FONT_SIZE!=GUI_SMALLFONT_SIZE) ? (nchars+4) : (MAX(nchars,4)+6); // D is bigger than 8
      else
        text_width+=4; // 2 to the left and right
    }

    handle=CreateWindowEx(WS_EX_CLIENTEDGE,"Steem Mr Static Control","",WS_VISIBLE|WS_CHILD|SS_NOTIFY
                          |SS_CENTER,x,y,text_width+4,CharHeight,owner,id,hInstance,NULL);
...
LRESULT CALLBACK mr_static_WndProc(HWND Win,UINT Mess,WPARAM wPar,LPARAM lPar) {
  ...
  case WM_PAINT:
  {
    ...
    else
    {
      ...
      if(ms->type==MST_DECIMAL)
        box.left+=2,box.right-=4;
      else
        box.left+=4;
      DrawText(ps.hdc,Text,-1,&box,((ms->type==MST_DECIMAL)?DT_RIGHT:DT_LEFT)
  ...
  case WM_CONTEXTMENU:
  {
    ...
    TrackPopupMenu(insp_menu,TPM_LEFTALIGN|TPM_LEFTBUTTON,
      GET_X_LPARAM(lPar),GET_Y_LPARAM(lPar),0,DWin,NULL);
    return 0;
  }
  case WM_NOTIFY:
  ...
  case WM_LBUTTONDOWN:
    ...
    if(ms->type==MST_IOLIST||ms->type==MST_HISTORIC_IOLIST)
    {
      ...
      if(ms->type==MST_IOLIST)
        iolist_box_click(x,ms->ile,NULL);
      else
        iolist_box_click(x,ms->ile,(BYTE*)(ms->ptr));
//        InvalidateRect(Win,NULL,false);
...
void mr_static::edit(char*ttt) {
  ...
  MEM_ADDRESS val=HexToVal(ttt);
   // request
   // check if user wants to use a register to set the address of the browser
  if(type==MST_MEM_BROWSER_ADDRESS)
  {
    char* FindLeftPar=strchr(ttt,'(');
    if(FindLeftPar)
    {
      char* FindRightPar=strchr(FindLeftPar,')');
      if(FindRightPar>FindLeftPar+2)
      {
        char FirstLetter=*(FindLeftPar+1); // right after (
        //char* end;
        bool isAn=false;
        switch(FirstLetter) {
        case 'A': case 'a': // address register
          isAn=true;
        case 'D': case 'd': // data register
          //val=strtol((FindLeftPar+2),&end,10); // get n in An or Dn
          val=*(FindLeftPar+2)-'0'; // eg '6' -> 6
          val&=7; // D0-D8, A0-A8
          val=(isAn)?(AREG(val)):(REGL(val));
          break;
        }
      }
    }
  ...
  {
    SET_PC(pc);
    mem_browser_update->ad=pc; // v410 scroll to new pc
  }
