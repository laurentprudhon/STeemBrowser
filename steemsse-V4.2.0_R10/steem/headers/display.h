...
enum EDisplay {
  ...
  DISPMETHOD_X,DISPMETHOD_XSHM,DISPMETHOD_BE,MAX_DISPMETHODS=5,
  NFSRES=40,
// 501x224 = 112224
...
struct TSteemDisplay {
  ...
  bool Blit(BOOL erase=FALSE);
#else
  bool Blit();
#endif
  void WaitForAsyncBlitToFinish();
  void Unlock();
  void FullscreenRunStart(bool Temp=false);
  void FullscreenRunEnd();
  void ScreenChange();
  ...
  HRESULT LoadScreenShot(char *path);
#endif
  bool BorderPossible();
  int STXPixels();
  int STYPixels();
...
#if defined(SSE_VID_BFI)
  inline bool D3DBlit(BOOL erase=FALSE);
#else
  inline bool D3DBlit();
#endif
...
#endif
  //DATA
  draw_type* VideoMemoryEnd; // =draw_mem+size
  ...
  UINT D3DFsW,D3DFsH;
  D3DPRESENT_PARAMETERS d3dpp,d3dpp_save; // double use for w,h, but we need other params
  ...
  DWORD DDLockFlags;
  int DDClosestHz[MAX_DISPMETHODS][NUM_HZ];
  COUNTER_VAR VSyncTiming;
  RECT LetterBoxRectangle;
  bool DDDisplayModePossible[MAX_DISPMETHODS];
  ...
  int Method,UseMethod[MAX_DISPMETHODS],nUseMethod;
  ...
  // there is a setting only for ScreenShotMinSize, the others could be added, meanwhile we
  // have documented them in the manual and changed the default to "full name" (SSE_420R4)
  bool ScreenShotUseFullName,ScreenShotAlwaysAddNum,ScreenShotMinSize;
};
