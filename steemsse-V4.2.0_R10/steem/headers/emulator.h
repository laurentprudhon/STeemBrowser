...
DOMAIN: Emu
FILE: emulator.h
DESCRIPTION: Declarations for miscellaneous emulator functions and
objects.
...
#define STRS(a) itoa((a),d2_t_buf,10) // convert int to decimal string
#define HEXS(a) itoa((a),d2_t_buf,16) // convert int to hexadecimal string
#endif
...
#define MSB_B (BYTE)0x80 // Most Significant Bit
...
extern DWORD agenda_next_time;
extern DWORD hbl_count;
...
void init_timings();
extern COUNTER_VAR sys_timer; // absolute system cycle counter target
extern int sys_cycles; // system cycles to run before we reach sys_timer
extern "C" DWORD nSysCyclesPerSecond; // used by 6301.c
extern DWORD new_n_cpu_cycles_per_second;
extern DWORD n_millions_cycles_per_sec;
extern "C" double cpu_cycles_multiplier; // used by 6301.c, 1.0 is normal
extern const int EIGHT_MILLION; //8000000*TICKS8 // must be a round number!
...
extern SHORT TimerBLinecycle; // scanline cycle when timer B ticks (was sys_cycles_from_hbl_to_timer_b)
extern BYTE VideoFreqIdx; // 0: 50Hz 1: 60Hz 2: 71/72Hz
extern int video_first_draw_line; // 0 normally
extern int video_last_draw_line; // 199 or 399 (hires) normally
extern BYTE screen_res; // 0: low 1: medium 2: high
extern SHORT scan_y; // 0 is the first scanline where video memory is normally displayed
extern BYTE video_mixed_output; // frames with low and medium resolution scanlines, eg: The Pawn
#define VIDEO_MIXED_ESTABLISHED 2
...
extern COUNTER_VAR GlueFreqChangeTime[NMODECHANGES];
extern BYTE glue_freq_change[NMODECHANGES];
extern BYTE GlueFreqChangeIdx;
extern COUNTER_VAR ShifterModeChangeTime[NMODECHANGES];
extern BYTE shifter_mode_change[NMODECHANGES];
...
extern DWORD MasterSync; // ST (no VSync) or PC (VSync) frequency
#else
#define MasterSync Glue.VideoFreq // ST
#endif
...
// possible values for Shifter's mode, used internally in Steem too
#define LORES   0
#define MEDRES  1
#define HIRES   2

extern const BYTE TopScanlines[4]; // how many border scanlines before video memory gets shifted
extern const WORD CyclesPerScanline8MHz[4]; // eg 512*TICKS8 at 50Hz
extern int CyclesPerScanline[4]; // cycles can be multiplied
...
extern UINT extmon_res[EXTMON_RESOLUTIONS][3]; // [3]: w,h,#bitplanes
void call_a000();
#endif
...
extern bool emudetect_called,emudetect_write_logs_to_printer,emudetect_overscans_fixed;
...
extern TGhostDisk GhostDisk[2]; // only 2
#endif

#if defined(SSE_DISK_STW)
#if defined(SSE_DISK_STW2)
extern TImageSTW2 ImageSTW[3];
#define IMAGE_STW2 (FloppyDrive[DRIVE].ImageType.Extension==EXT_STW && ImageSTW[DRIVE].Version>=0x200)
#else
...
#endif
#if defined(SSE_DISK_STW2)
//#define IMAGE_LL_MFM (IMAGE_SCP||IMAGE_STW&&ImageSTW[DRIVE].LowLevel)
#define IMAGE_LL_MFM (FloppyDrive[DRIVE].MfmManager&&FloppyDrive[DRIVE].MfmManager->LowLevel)
#else
...
#define ADAT                       (FloppyDrive[DRIVE].bAdat)
...
struct TDiskEmu {
  ...
  BYTE InterTrack; // for sound: distance from target track in a SEEK
  bool direction; // for step/seek sound
  bool StatusBar;
  WORD bytes;
  WORD BitRate; // for OSD intensity, 500 = base (Kbit/s)
  DWORD crc32; // crc32 of crc16
  TWD1772Crc crc16;
};
...
extern int ioaccess;
extern int interrupt_depth; // overflow possible
extern int on_rte;
extern int on_rte_interrupt_depth;
extern MEM_ADDRESS old_pc;
extern MEM_ADDRESS pc_high_byte; // unused
...
struct TStatsStatic { // not reset
  TStatsStatic() { 
    memset(this,0,sizeof(TStatsStatic));
  }
  int TotalRuntime;
  int nReset;
  int nBlitError; // ;)
...
struct TStats { // used to record and display information about what the program is doing
  enum EStats {
    IKBD_22=BIT_0,EMU_DETECT=BIT_1
  }; 
  // FUNCTIONS
  TStats() {
    Restore();
  ...
  void Restore() {
    memset(this,0,sizeof(TStats)); // simple
  }
  void Report();
  // DATA
#if defined(SSE_STATS_CPU)
  ...
  COUNTER_VAR nBlit,nBlitT,nBlith,nExtendedPal,nExtendedPalT,nHscroll,nVbaseLo;
  COUNTER_VAR nVbaseMid,nVbaseHi;
  COUNTER_VAR nReadvc;
  COUNTER_VAR nPsgSound,nMicrowire,nMicrowireT;
  COUNTER_VAR nKeyIn,nJoy[2],nClick[2],nMousex,nMousey;
  ...
  DWORD nPal,nTimerbtick,nBlit1,nHbi1,nReadvc1,nVbaseMid1; // per frame
  DWORD nLinePlus16; 
  DWORD mskSpecial,mskDigitalSound,mskOverscan,mskOverscan1;
  DWORD fTimer[4];
  LONGLONG tEmuT,tLockT,tUnlockT,tBlitT;
#if defined(SSE_STATS_EXT)
  COUNTER_VAR Ast0,RamAccessWs,ChipsAccessWs,BusRead,BusWrite;
#endif
  DWORD tEmuN;
#ifdef WIN32
  LARGE_INTEGER tEmu0,tBlit1;
  LONGLONG tEmu,tBlit,tLock,tUnlock,tFrameT;
#else
  DWORD tLock,tEmu,tEmu0,tUnlock,tBlit;
#endif
  int run_time;
  WORD nFps,tCpuUsage;
...
Wrong doc! It's bit 0 for cache, bit 1 for freq

Correct in The Atari compendium

If SSE_MEGA16 is defined, we add an extra bit 2 for the inexistent "turbo"
button on both Mega ST and Mega STE (=convenience hack)
*/
  BYTE ScuReg;
};
