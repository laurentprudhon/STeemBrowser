...
#define PORTTYPE_LAN 6    // UNIX
#define PORTTYPE_TCPIP 7
#define PORTTYPE_UNIX_SEQUENCER 100
#define PORTTYPE_UNIX_OTHER 101

#if defined(SSE_DONGLE_PORT) 
#define DONGLE_ID STPort[TSTPort::DONGLE].Type // in case we choose another way later
#define NSTPORTS 4
#else
#define DONGLE_ID 0
#define NSTPORTS 3
...
struct TDongle { 
  // they're not all protection dongles
  enum {NONE,BAT2,MUSIC_MASTER,URC,LEADERBOARD,TENTHFRAME,MULTIFACE,
...
struct TSTPort {
  enum ESTPorts { MIDI,PARALLEL,SERIAL,DONGLE
#if defined(SSE_NETWORK)
  ...
  bool IsOpen() { 
    return (MIDI_Out || MIDI_In || PCPort || fp || LoopBuf || PCPortIn);
  }
  bool IsPCPort() { 
    return PCPort!=NULL; 
  }
...
#if defined(SSE_NETWORK) // one of the I/O options for the ports is TCP/IP
  SOCKET ListenSocket,Socket[MAXCLIENT_SOCKETS];
  EasyStr sIPAddr;
  WORD IPPort;
  BYTE Connected;
  BYTE nSockets,MaxClients;
  bool ByteComing;
#endif
