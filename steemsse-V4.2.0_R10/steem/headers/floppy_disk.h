...
enum EDiskImage { 
  // error codes
  FIMAGE_OK=0,
  FIMAGE_WRONGFORMAT=-15, // errors are traditionally negative
  FIMAGE_CANTOPEN,
  ...
  DISK_NODLL, // when Steem didn't find the necessary plugin
  DISK_COMPRESSED_NODLL,
#if defined(SSE_DISK_M3U)
  DISK_IS_PLAYLIST,
#endif
  // extensions ("Steem runs everything!")
  EXT_NONE=0,EXT_ST,EXT_MSA,EXT_DIM,EXT_STT,EXT_STX,EXT_IPF,EXT_CTR,EXT_STG,
  EXT_STW,EXT_PRG,EXT_TOS,EXT_SCP,EXT_HFE,NUM_EXT,
  
  /* image managers
    MNGR_STEEM is Steem's original native emulation to handle DIM/ST/MSA and STX images
    MNGR_WD1772 is Steem's different emulation to handle STW/SCP/HFE disk images 
    MNGR_PASTI is when pasti.dll handles the image
    MNGR_CAPS is when the CAPSImage plugin handles the image
    MNGR_PRG is when the image is a .PRG or .TOS file (ST executable)
    MNGR_ACSI is for ACSI hard drive emulation and ACSI Laser Printer emulation  */
  MNGR_NONE=0,MNGR_STEEM,MNGR_PASTI,MNGR_CAPS,MNGR_WD1772,MNGR_PRG,MNGR_ACSI,NUM_MNGR
};


extern char *extension_list[NUM_EXT];
extern char *disk_manager[NUM_MNGR];
...
struct TFloppyDisk { // FloppyDisk[] objects inserted in FloppyDrive[] objects
  ...
  DWORD DiskFileLen;
  DWORD STT_TrackStart[2][FLOPPY_MAX_TRACK_NUM_CST+1];
  DWORD crc32;
  int FormatMostSectors,FormatLargestSector;
  int PastiBufLen;
  WORD STT_TrackLen[2][FLOPPY_MAX_TRACK_NUM_CST+1]; 
  ...
  bool TrackIsFormatted[2][FLOPPY_MAX_TRACK_NUM_CST+1];
  ...
  bool WriteProtect; // we use this instead of ReadOnly for WP flag
};
...
struct TImageMfm { // base class for TImageHFE, TImageSCP, TImageSTW
  ...
  virtual WORD GetImageWord(WORD position)=0;
  virtual int GetNextTransition(WORD& us_to_next_flux);
  virtual void SetImageWord(WORD position,WORD mfm_data)=0;
  // variables
  FILE *fCurrentImage;
  WORD Position;
  BYTE Id;
  ...
  DWORD nTrackBits;
  bool LowLevel;
  virtual void IncBitPosition()=0;
};

#if defined(SSE_DISK_M3U)

// devised for the Libretro core, used in main build since v421
// "dumb" as opposed to already existing "smart" swapper
struct TDumbDiskSwapper {
  TDumbDiskSwapper();
  EasyStr m3upath;
  EasyStr image_path;
  WORD num_images,image_index;
  bool ejected; // Libretro
  WORD Open(EasyStr path); // updates m3upath image_index and num_images
  EasyStr GetPath(WORD index); // updates image_path and image_index
};

extern TDumbDiskSwapper DumbDiskSwapper;
