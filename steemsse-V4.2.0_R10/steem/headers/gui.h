...
#if !defined(ONEGAME)
#ifdef WIN32
#define MF_CHECK(x) ((x)?MF_CHECKED:MF_UNCHECKED) // 420R4
#define ENABLE_CHECK(x) ((x==0)?WS_DISABLED:0) // 420R5
...
enum EGui {
  ...
  IDC_HDGEMDOS=10,IDC_HDACSI,IDC_DRIVEA=98,IDC_DRIVEB, // DiskMan + ...

  IDC_DISK_MANAGER=100,
  IDC_PLAY=101,
  IDC_RESET=102,
  IDC_JOYSTICKS=103,
  IDC_INFO=105,
  ...
  IDC_CONFIGS=121,
  IDC_DEBUGGER=199,
  IDC_LOADSNAPSHOT=200,IDC_SAVESNAPSHOT,
  IDC_DELAY_LOADSNAPSHOT=203,
  IDC_SNAPSHOTSAVEOVER=205,IDC_UNDOSNAPSHOTSAVEOVER,
  IDC_UNDORESET=207,
  IDC_UNDOSNAPSHOTLOAD,
  IDC_DEFAULTSNAPHOT,
  IDS_STATIC=209, // once and for all
  IDC_SNAPSHOT_HISTORY=210, // 210-219

  IDC_PASTE_SPEED=300,
  IDC_SCREENSHOT_FORMATS=400,
  IDC_FI_FORMATS=420,
  IDC_SCREENSHOT_DIR_CHANGE=440,IDC_SCREENSHOT_DIR_OPEN,IDC_SCREENSHOT_MIN,
  IDC_LOADCONFIG,IDC_SAVECONFIG,
#if 0
  IDC_MENULOADCONFIG/*=443!*/,IDC_MENUSAVECONFIG,
  IDC_MENULOADSNAPSHOT,IDC_MENUSAVESNAPSHOT,
#endif
  IDC_MENURUNSTOP=601,IDC_MENUDISKMAN,IDC_MENUPATCHESBOX,IDC_MENUKILLTHREAD,
  IDC_MENUTOGGLEFULLSCREEN,IDC_MENUEXIT=609,IDC_MENUREBOOT=611,IDC_MENURESET,
  IDC_MENUINSERTA,IDC_MENUINSERTB,IDC_MENUGEMDOSHD,IDC_MENUACSIHD,
  IDC_MENUSWAPPERPREV,IDC_MENUSWAPPERNEXT,

  IDC_MENUOPTIONS=651,IDC_MENUSHORTCUTS,IDC_MENUJOYSTICKS,IDC_MENUINFO,IDC_MENUDEBUGGER,
  IDC_MENUTOOLBAR,

  MSG_UPDATETOOLBOXICONS=1234,

  ICON_SIZE_SMALL=20,ICON_SIZE_BIG=36, // not so big!
...
extern char gAppBuildInfo[64];


#pragma pack(push, 8)

struct TStatusInfo { // no condition, it's updated even if there's no status bar
  enum EStatusInfo {MESSAGE_NONE,MC68000_CRASH,X86_CRASH,BLIT_ERROR,
    BOILER_MESSAGE,HD6301_CRASH,MESSAGE_MISC};
  char text[256];
...
DWORD GetScreenWidth(),GetScreenHeight();
...
// TempPath: extracting archives, format disks, MSA conversion etc.
// typically in User/AppData
// UsersPath: steem.ini, folders for configs, snapshots, screenshots, trace, tos links etc.
// typically in Documents
// We now use those instead of WriteDir
// RunDir: patches, plugins/data
extern EasyStr TempPath,UsersPath;
#else
extern EasyStr &TempPath,&UsersPath; // alias to WriteDir
#endif
extern EasyStr AcsiDir;
extern EasyStr LastSnapShot,BootStateFile,StateHist[STATE_HISTORY_LEN],AutoSnapShotName;
extern EasyStr DefaultSnapshotFile;
extern Str BootDisk[2];
extern bool bPauseWhenInactive;
extern bool BootTOSImage;
extern BYTE MuteWhenInactive;
extern bool bAlwaysOnTop,bAppMaximized;
...
extern bool bAllowTaskSwitch;

#if defined(SSE_GUI_MENUBAR)
extern HMENU StemWinMenu,StemWinMenuFile,StemWinMenuEmu,StemWinMenuTools;
extern bool AltMenuOn; // keyboard control
...
extern bool SuspendRendering; // video rendering of ST frames

#endif//#if defined(SSE_EMU_THREAD)


#if defined(SSE_GUI_BIGICONS)
#define GUIMUL(n) ((n)*FONT_SIZE/GUI_FONT_SIZE)
#else
#define GUIMUL(n) n
#endif
...
#define BM_CLICKBUTTON_LEFT 1
#define BM_CLICKBUTTON_RIGHT 2
#define BM_RELOADICON 0x00F9
#define PDS_VCENTRESTATIC 1

#define CLICK_PLAY_BUTTON() \
  if (!bRunMessagePosted){ \
    ...
    bRunMessagePosted=true; \
  }


struct TSystemMetrics {
  DWORD m_cx_screen,m_cy_screen;
  WORD m_cx_frame,m_cy_frame,m_cy_caption,m_cx_vscroll;
  ...
  TSystemMetrics() {
    memset(this,0,sizeof(TSystemMetrics));
  }
  int cx_screen() {
    if(!m_cx_screen)
      Update();
...
extern HINSTANCE hInstance; // Win32 instance of our Steem executable
extern TSystemMetrics GuiSM;
extern bool bRunMessagePosted;
...
int timerproc(void*,Window,INT_PTR);
...
#define ICO64_HARDDRIVES_ACSI 3
#endif
...
inline bool SetProp(Window Win,XContext Prop,DWORD_PTR Val)
{
  return SetProp(XD,Win,Prop,Val);
}


inline DWORD_PTR GetProp(Window Win,XContext Prop)
{
  return GetProp(XD,Win,Prop);
}


inline DWORD_PTR RemoveProp(Window Win,XContext Prop)
{
