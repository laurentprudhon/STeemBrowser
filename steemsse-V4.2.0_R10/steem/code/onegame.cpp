...
char OG_TextMem[OG_TEXT_LEN];
int OGExtraSamplesPerVBL=300;
...
bool OGInit()
{
  ...
  mf.size=SizeofResource(hInstance,res);
...
#endif
  SET_PC(pc); // Might start at altered address, reset prefetch
...
void OGSetRestart()
{
  if (runstate==RUNSTATE_RUNNING) CLICK_PLAY_BUTTON();
...
void OGLoadData(TConfigStoreFile *pCSF)
{
...
{
  FILE *fp=fopen(WriteDir+SLASH+"hiscores","r+b");
  if (fp==NULL) return;
  ...
  for(;;){
    ...
    SavePos=ftell(fp);
    fread(&SavedSide,1,sizeof(int),fp);
    fread(&SavedTrack,1,sizeof(int),fp);
    fread(&SavedSector,1,sizeof(int),fp);
    fread(&SavedBytes,1,sizeof(int),fp);
    ...
    if (SavedSide==Side && SavedTrack==Track && SavedSector==Sector){
      if (SavedBytes==Bytes){
        ...
        fseek(fp,SavePos+sizeof(int)*2,SEEK_SET);
        SavedSector=256; // Clear this one
        fwrite(&SavedSector,1,sizeof(int),fp);
        fseek(fp,SavePos+sizeof(int)*4+SavedBytes,SEEK_SET);
      }
    }else{
      fseek(fp,SavedBytes,SEEK_CUR);
    }
  }
  fseek(fp,SavePos,SEEK_SET);
  fwrite(&Side,1,sizeof(int),fp);
  fwrite(&Track,1,sizeof(int),fp);
  fwrite(&Sector,1,sizeof(int),fp);
  fwrite(&Bytes,1,sizeof(int),fp);
...
{
  FILE *fp=fopen(WriteDir+SLASH+"hiscores","rb");
  if (fp==NULL) return;
  ...
  for(;;){
    ...
    if (SavedSector<256){
      if (floppy->SeekSector(SavedSide,SavedTrack,SavedSector,0)==0){
        fread(Sector,1,SavedBytes,fp);
        fwrite(Sector,1,MIN(int(floppy->BytesPerSector),SavedBytes),floppy->fp);
      }
    }else{
      fseek(fp,SavedBytes,SEEK_CUR);
    }
