...
for output (Windows). The Linux build uses PulseAudio, see interface_pulse.cpp.
SoundVBL is the main function writing one frame of sound to the output buffer.
So there is some unavoidable delay in sound rendering, of minimum one video
frame time (20ms at 50Hz).
Outputs from the PSG (YM2149) and the GST Shifter (STE sound) are mixed, but
it's possible to mute one or the other (convenience hack).
...
DWORD sound_freq=44100,sound_comline_freq=0,sound_chosen_freq=44100;
#else
DWORD sound_freq=48000,sound_comline_freq=0,sound_chosen_freq=48000;
#endif
int SoundVolume=-1400; // 0db = max!
DWORD sound_record_start_time; //by timer variable = timeGetTime()
#if defined(SHOW_WAVEFORM)
DWORD min_write_time;
DWORD play_cursor,write_cursor;
#endif
DWORD psg_buf_length=DEFAULT_SOUND_BUFFER_LENGTH;
DWORD SoundBufStartTime;

#if (SCREENS_PER_SOUND_VBL != 1)
COUNTER_VAR sys_time_of_last_sound_vbl=0;
#endif
...
DWORD psg_n_samples_this_vbl;

BYTE psg_write_n_screens_ahead=PSG_BUFFER_FRAMES;
BYTE UseSound=0; // not bool for Linux build
BYTE sound_num_channels=2;
BYTE sound_num_bits=16;
BYTE sound_bytes_per_sample=4; //(sound_num_bits/8)*sound_num_channels
bool sound_low_quality=false;
bool sound_click_at_start=false;
bool bSoundRecord=false;
bool ste_sound_on_this_screen=false;
...
void SoundRecordOpenFile() {
  ...
  if(Wav_file==NULL)
  {
    Alert(T("Could not open sound file for writing"),T("ERROR"),MB_ICONEXCLAMATION);
    bSoundRecord=false;
    return;
  }
  if(OPTION_SOUND_RECORD_FORMAT==TOption::SoundFormatWav) // not YM
  {
...
void SaveInt(int i,FILE *fp) { // was in mymisc, used only here
  int a=i;
  SWAP_LITTLE_ENDIAN_DWORD(a); // we think of everything!
  FWRITE(&a,sizeof(i),1,fp);
}
...
void SoundRecordCloseFile() {
  if(Wav_file==NULL)
    return;
  fflush(Wav_file);
  size_t length=(size_t)FTELL(Wav_file);
...
*/
  ...
  Wav_file=NULL;
  bSoundRecord=false;
  OptionBox.UpdateSoundRecordBut();
}
...
HRESULT InitSound() {
  SoundRelease();
  HRESULT Ret=S_OK;
  if(!SSEOptions.AudioInterface)
    return Ret;

#ifdef WIN32
#if !defined(SSE_LIBRETROSOUND1) || defined(SSE_LIBRETRO_DRIVESOUND)
  ...
  // SetCooperativeLevel needs a correct app handle
  ...
  DSObj->SetCooperativeLevel(StemWin,DSSCL_NORMAL); 
  ...
  UseSound=1;
#endif//#if !defined(SSE_LIBRETROSOUND1) || defined(SSE_LIBRETRO_DRIVESOUND)
#endif//WIN32 
...
DWORD SoundGetTime() { // called by SoundVBL()

#if defined(SSE_LIBRETROSOUND1)

  return 0;//n_samples_this_vbl;
...
#endif
  DWORD s_time=0;
  ASSERT(SoundBuf && sound_bytes_per_sample);
#ifndef SSE_LEAN_AND_MEAN
  if(SoundBuf && sound_bytes_per_sample)
#endif
  {
    play_cursor=write_cursor=0;
/*
...
-> 'write cursor' is the right method, enforced now, other code deleted

*/
    SoundBuf->GetCurrentPosition(&play_cursor,&write_cursor);
    DWORD cursor=write_cursor;
    cursor/=sound_bytes_per_sample;
    if(cursor<psg_last_play_cursor)
      psg_time_of_start_of_buffer+=psg_buf_length;
...
#if defined(SSE_UNIX_PULSEAUDIO)
  if (UseSound==XS_PULSE) return Pulse_GetTime();
#endif  
  return 0;
#endif

#endif//#if defined(SSE_LIBRETROSOUND1)
}
...
HRESULT SoundStartBuffer(int flatlevel1,int flatlevel2) {
  ...
  snd_max_frames=sound_freq; // 1s is far more than enough! //todo need no glob
  ASSERT(SoundBuf==NULL);
  //SoundBuf=(int16_t*)malloc(snd_max_frames);
  /*The data passed to the batch callback should, if possible, be aligned to 16 bytes (depends on
  platform), to allow accelerated SIMD operations on audio.*/
  SoundBuf=(int16_t*)_aligned_malloc(snd_max_frames,16);
  if(SoundBuf)
  {
  ...
  SoundBuf->SetVolume(SoundVolume);
...
#if defined(SSE_UNIX_PULSEAUDIO)
  case XS_PULSE:
    Ret=Pulse_StartBuffer(flatlevel1,flatlevel2);
    // drive sound in context callback
    break;
#endif  
  }//sw
  if(Ret==DS_OK)
...
#if defined(SSE_UNIX_PULSEAUDIO)
  if (UseSound==XS_PULSE) return Pulse_Stop(false);
#endif
#endif//#if !defined(SSE_LIBRETRONUKE)
#endif
...
#if defined(SSE_UNIX_PULSEAUDIO)
  if (UseSound==XS_PULSE) Pulse_ChangeVolume(); 
#endif  
#endif
}
...
HRESULT SoundError(char *ErrorText,HRESULT DErr) {
  ...
  Err+=Str("\n\n")+T("Steem will not be able to output any sound until you restart the program.\
 Would you like to permanently stop Steem trying to use DirectSound at startup?");
#if !defined(SSE_SOUND_NO_NOSOUND_OPTION)
#ifndef ONEGAME
  int Ret=MessageBox(NULL,Err,T("DirectSound Error"),
    MB_YESNO|MB_ICONEXCLAMATION|MB_SETFOREGROUND|MB_TASKMODAL|MB_TOPMOST);
  if(Ret==IDYES)
    WriteCSFStr("Options","NoDirectSound","1",globalINIFile);
  OptionBox.UpdateForNoSound();
#endif
#endif
#endif//WIN32
...
*/
  ...
  if(psg_channels_buf_len+16!=samples_per_vbl)
  {
    ...
    ZeroMemory(psg_channels_buf,(samples_per_vbl+16+PSG_WRITE_EXTRA)*sizeof(int));
    psg_channels_buf_len=samples_per_vbl;
    TRACE_INIT("buffer for psg %dHz = %p, %d x32bit =%d bytes\n",psg_channels_buf,sound_freq,psg_channels_buf_len,psg_channels_buf_len*sizeof(int));
  }
  samples_per_vbl=((sound_freq/50)+1)*SCREENS_PER_SOUND_VBL*2;
  if(SteSoundChannelBufLen+16!=samples_per_vbl)
  {
    if(SteSoundChannelBuf!=NULL)
      delete[] SteSoundChannelBuf;
    SteSoundChannelBuf=new WORD[samples_per_vbl+16];
    ZeroMemory(SteSoundChannelBuf,(samples_per_vbl+16)*sizeof(WORD));
    SteSoundChannelBufLen=samples_per_vbl;
    TRACE_INIT("buffer for dma %dHz = %p, %d x16bit =%d bytes\n",SteSoundChannelBuf,sound_freq,SteSoundChannelBufLen,SteSoundChannelBufLen*sizeof(WORD));
  }
#if defined(SSE_YM2149_LL)
  if(sound_freq)
  {
    Psg.ym2149_cycles_per_sample=(IS_STE)
      ? (((double)STE_CLOCK8/4)/(double)sound_freq)
      : ((double)CpuNormalHz/(4*TICKS8))/(double)sound_freq;
  }
  if(!Psg.AntiAlias && OPTION_LOWPASS<YM_LOW_PASS_MAX)
  {
    if((Psg.AntiAlias=new Filter(LPF,51,250.0,(double)OPTION_LOWPASS/1000))!=NULL)
    {
      ...
      TRACE_LOG("anti-alias %d %p\n",OPTION_LOWPASS,Psg.AntiAlias);
    }
  }
#endif
...
HRESULT DSGetPrimaryBuffer() { // called by SoundStartBuffer()
  ...
  DS_GetFormat_Wrong=false;
  DWORD desired_freq=sound_chosen_freq;
  for(;;)
  {
    ...
    if(DS_GetFormat_Wrong) 
      break;
    if(sound_freq>=(DS_SetFormat_freq-2500)&&sound_freq<=(DS_SetFormat_freq+2500))
      break;
...
HRESULT DSCreateSoundBuf() { // called by SoundStartBuffer()
  ...
  psg_buf_length=DEFAULT_SOUND_BUFFER_LENGTH;
  HRESULT Ret;
  DSBUFFERDESC dsbd;
  WAVEFORMATEX wfx;
  ...
  if(Ret!=DS_OK)
  {
    if(DS_GetFormat_Wrong)
    {
      ...
      ZeroMemory(&dsbd,sizeof(DSBUFFERDESC));
      dsbd.dwSize=sizeof(DSBUFFERDESC);
      dsbd.dwFlags=DSBCAPS_CTRLVOLUME|DSBCAPS_GLOBALFOCUS|DSBCAPS_GETCURRENTPOSITION2
        |DSBCAPS_STICKYFOCUS;
      dsbd.dwBufferBytes=psg_buf_length * sound_bytes_per_sample;
      dsbd.lpwfxFormat=&wfx;
      Ret=DSObj->CreateSoundBuffer(&dsbd,&SoundBuf,NULL);
  ...
  if(SoundBuf->GetCaps(&caps)==DS_OK) 
    psg_buf_length=caps.dwBufferBytes/sound_bytes_per_sample;
  return DS_OK;
}
...
void XSoundInitBuffer(int flatlevel1,int flatlevel2) {
  BYTE *p=x_sound_buf;
  for(int i=0;i<psg_buf_length;i++)
  {
    if(sound_num_bits==8)
    {
      *(p++)=(BYTE)flatlevel1;
...
void AlterV(int Alter_V,int &v,int &dv,int *source_p) {
...
void WriteSoundLoop(int Alter_V, int* Out_P,int Size,DWORD& c,int &val,
...
/*  Try to avoid clicks when a program aggressively changes microwire volume
    ...
    This is enabled with option LmcSlowFade, else we use dsp.
*/
#if defined(SSE_SOUND_MICROWIRE_HACKS)
  if(!IS_STE||!OPTION_MICROWIRE||!SSEOptions.LmcSlowFade)
  {}
...
#if defined(SSE_SOUND_MICROWIRE_HACKS)
        if(SSEOptions.LmcSlowFade && sound_num_channels==2 // not for monosound: Rebirth
          && (Microwire.top_val_l!=128||Microwire.old_top_val_l!=Microwire.top_val_l))
        {
...
#if defined(SSE_SOUND_MICROWIRE_HACKS)
          if(SSEOptions.LmcSlowFade && (Microwire.top_val_r!=128
  ...
  { // choose our loop according to config
    ...
    if(IS_STE&&OPTION_MICROWIRE) // most complex
    {
      ...
      { 
        ...
        Microwire.Process(0,val);
#if defined(SSE_SOUND_MICROWIRE_HACKS)
        if(SSEOptions.LmcSlowFade&&((Microwire.top_val_l!=128||Microwire.old_top_val_l
        ...
        *(WORD*)*(WORD**)Out_P=((WORD)val);
        ...
        { //RIGHT-16bit
          ...
          Microwire.Process(1,val);
#if defined(SSE_SOUND_MICROWIRE_HACKS)
          if(SSEOptions.LmcSlowFade&&(Microwire.top_val_r<128||Microwire.old_top_val_r
            !=Microwire.top_val_r))
          {
            val*=Microwire.old_top_val_r;
            val/=128;
    ...
    else // STF PSG-only
    {
      ...
      { 
        AlterV(Alter_V,v,dv,*source_p);
        //LEFT-16bit
#if defined(SSE_420R5) // oops forgot it was commented out - Goldrunner STF
        if(v>0x7FFF)
          v=0x7FFF;
#endif
        //if(v>32767)
...
void SoundRecord(int Alter_V, int Write,int& c,int &val,int &v,int &dv,int **source_p,
                 WORD**lp_ste_sound_channel,WORD**lp_max_ste_sound_channel,FILE* wav_file) {
#if defined(SSE_SOUND_MICROWIRE_HACKS)
  if(!IS_STE||!OPTION_MICROWIRE||!SSEOptions.LmcSlowFade)
  {}
  else if(Microwire.old_top_val_l<Microwire.top_val_l)
  ...
  {       
    ...
    if(Write==WRITE_TO_WAV_FILE_B) 
      FPUTC(LOBYTE(WORD_B_1(&(val))),wav_file);
    else 
    {
...
void sound_record_to_wav(int c,bool chipmode,int *source_p) {
  if(timer<sound_record_start_time) 
    return;
  int v=psg_voltage,dv=psg_dv; //restore from last time
  WORD *lp_ste_sound_channel=SteSoundChannelBuf;
  WORD *lp_max_ste_sound_channel=SteSoundChannelBuf+SteSoundChannelBufIdx;
  int val;
  if(source_p==NULL)
  {}
...
HRESULT SoundVBL() {
#if SCREENS_PER_SOUND_VBL != 1 // note it is 1
  ...
  sys_time_of_last_sound_vbl=ABSOLUTE_SYS_TIME;
#endif
#if !defined(SSE_NO_INTERNAL_SPEAKER)
...
#endif
  // This just clears up some clicks when SoundVBL is called very soon after SoundStart
  if(OPTION_SOUNDMUTE || (MuteWhenInactive&&!bAppActive))
    return DS_OK;
  if(UseSound==0)  
  ...
  DWORD n_samples_per_vbl=(DWORD)(((sound_freq*SCREENS_PER_SOUND_VBL*
    (DWORDLONG)Glue.nFrameCycles) / nSysCyclesPerSecond));
#else
  DWORD n_samples_per_vbl=(sound_freq*SCREENS_PER_SOUND_VBL)/Glue.VideoFreq;
#endif
  s_time=SoundGetTime();
  ...
  if((write_time_2-write_time_1)>psg_channels_buf_len)
    write_time_2=write_time_1+psg_channels_buf_len;
  //psg_last_write_time=write_time_2;
  DWORD time_of_next_vbl_to_write=MAX(s_time+n_samples_per_vbl
    *psg_write_n_screens_ahead,psg_time_of_next_vbl_for_writing);
  // new bit added by Ant 9/1/2001 to stop the sound lagging behind
#if 1
  DWORD fwd=s_time+n_samples_per_vbl*(psg_write_n_screens_ahead+2);
  if(time_of_next_vbl_to_write>fwd)
    time_of_next_vbl_to_write=fwd;
#else
...
#endif
#ifdef SHOW_WAVEFORM
  temp_waveform_display_counter=write_time_1 %psg_buf_length;
  ...
  {
#if defined(SSE_LIBRETROSOUND1)
    Psg.psg_write_buffer(time_of_next_vbl_to_write,false);
#else
    ...
    for(DWORD i=MAX(1,(int)psg_buf_pointer[0]);i<psg_buf_pointer[0]+PSG_WRITE_EXTRA;i++)
...
#endif
  {
    ...
    for(char abc=2;abc>=0;abc--)
      psg_write_buffer(abc,time_of_next_vbl_to_write+PSG_WRITE_EXTRA);
  }
  if(ste_sound_on_this_screen && SteSoundChannelBufIdx>1)
  {
#if(PSG_WRITE_EXTRA>0) //300
    WORD w[2]={SteSoundChannelBuf[SteSoundChannelBufIdx-2],
               SteSoundChannelBuf[SteSoundChannelBufIdx-1]};
    for(int i=0;i<PSG_WRITE_EXTRA;i++)
    {
      if(SteSoundChannelBufIdx>=SteSoundChannelBufLen)
        break;
      SteSoundChannelBuf[SteSoundChannelBufIdx++]=w[0];
      SteSoundChannelBuf[SteSoundChannelBufIdx++]=w[1];
    }
#endif
#if defined(SSE_STATS) // record for every frequency
    Stats.mskDigitalSound|=( (1 | ((shifter_sound_mode&0x80)>>3))<<shifter_sound_mode);
#endif
  }
#if defined(SSE_SOUND_CARTRIDGE)
  else if(SSEConfig.mv16)
  {
    SteSoundFreq=(WORD)sound_freq; // seems to work with 96K though
    SteSoundChannelBuf[0]=SteSoundChannelBuf[1]=SteSoundLastWord.d16;
  ...
  else
  {
    ...
    SteSoundChannelBuf[0]=w1;
    SteSoundChannelBuf[1]=w2;
    SteSoundChannelBufIdx=0;
  }
  // write_time_1 and 2 are sample variables, convert to bytes
  DWORD StartByte=(write_time_1 % psg_buf_length)*sound_bytes_per_sample;
  DWORD NumBytes=((write_time_2-write_time_1)+1)*sound_bytes_per_sample;
#if defined(SSE_LIBRETROSOUND1)
  n_samples_this_vbl=n_samples_per_vbl;
  DatAdr[0]=SoundBuf;
  LockLength[0]=NumBytes*2;//enough space //n_samples_per_vbl*sound_bytes_per_sample;
  DatAdr[1]=NULL;
  LockLength[1]=0;
  Ret=DS_OK;
#else
  ...
  if(Ret!=DSERR_BUFFERLOST)
  {
    ...
    int i=MIN((DWORD)MAX((write_time_1-psg_time_of_last_vbl_for_writing),0ul),
      psg_channels_buf_len-10);
    int v=psg_voltage,dv=psg_dv; //restore from last time
    for(int j=0;j<i;j++)
      psg_channels_buf[j]=VOLTAGE_FP(VOLTAGE_ZERO_LEVEL); //zero the start of the buffer
    source_p=psg_channels_buf+i;
    int samples_left_in_buffer=MAX(psg_channels_buf_len-i,0ul);
    DWORD countdown_to_storing_values=MAX((int)(time_of_next_vbl_to_write-write_time_1),0);
...
*/
        ...
        BYTE env=written_to_env_this_vbl ? psg_reg[PSGR_ENVELOPE_SHAPE] : 0xFF;
        FWRITE(&env,sizeof(BYTE),1,Wav_file);
        written_to_env_this_vbl=false;
      }
      else
      {
        //ASSERT(countdown_to_storing_values<=psg_channels_buf_len);
        sound_record_to_wav(MIN((DWORD)countdown_to_storing_values,
          psg_channels_buf_len),chipmode,source_p);
      }
    }
#ifdef WRITE_ONLY_SINE_WAVE
    ...
    for(int n=0;n<2;n++)
    {
      if(DatAdr[n])
      {
        pb=(BYTE*)(DatAdr[n]);
        pw=(WORD*)(DatAdr[n]);
        //ASSERT(sound_bytes_per_sample);
        DWORD c=MIN(int(LockLength[n]/sound_bytes_per_sample),samples_left_in_buffer),oc=c;
...
#endif
    while(source_p<(psg_channels_buf+psg_channels_buf_len))
  ...
  psg_time_of_next_vbl_for_writing=MIN(psg_time_of_next_vbl_for_writing,s_time+(psg_buf_length/2));
  psg_n_samples_this_vbl=psg_time_of_next_vbl_for_writing-psg_time_of_last_vbl_for_writing;
  return DS_OK;
}


HRESULT SoundStart() { //Called from run()
  ...
  int envshape=psg_reg[PSGR_ENVELOPE_SHAPE]&0x0F;
  int flatlevel=0;
  for(int abc=0;abc<TYM2149::NUM_CHANNELS;abc++)
  {
  ...
  psg_dv=0;
  WORD dma_l,dma_r;
  Shifter.SoundGetLastSample(&dma_l,&dma_r);
...
// of samples, divide to get the number of samples per VBL and add extra.
    OGExtraSamplesPerVBL=((((run_speed_ticks_per_second-1000)*sound_freq)/1000)/Glue.VideoFreq)+300;
  }
#endif
  ...
  for(DWORD i=0;i<psg_channels_buf_len;i++) 
...
void TLMC1992::Reset(bool Cold) { // there's no reset line
  if(Cold)
  {
    ...
    Mask=0x07ff;
    volume=40;
    volume_l=volume_r=20;
    top_val_l=top_val_r=128;
  }
  StartTime=0;
...
#if !defined(SSE_NO_MICROWIRE)
void TLMC1992::Process(int const channel,int &val) {
  double d_dsp_v=val;
  if(bass!=6) // don't process if the gain is 0 (value 6)
    d_dsp_v=MicrowireBass[channel].FilterAudio(d_dsp_v,MW_LOW_SHELF_FREQ,bass-6);
  if(treble!=6)
    d_dsp_v=MicrowireTreble[channel].FilterAudio(d_dsp_v,MW_HIGH_SHELF_FREQ,treble-6);
#if defined(SSE_SOUND_MICROWIRE_HACKS)
  if(!SSEOptions.LmcSlowFade)
#endif
  {
    // faders
    if(volume_l<0x14 &&!channel)
      d_dsp_v=MicrowireVolume[channel].FilterAudio(d_dsp_v,volume_l-0x14);
    if(volume_r<0x14 &&channel)
      d_dsp_v=MicrowireVolume[channel].FilterAudio(d_dsp_v,volume_r-0x14);
    // main volume
    if(volume<0x28)
