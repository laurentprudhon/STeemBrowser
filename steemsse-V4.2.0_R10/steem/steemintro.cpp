...
int SteemIntro(TConfigStoreFile &CSF) {
  EasyStr caption=T("Welcome to Steem SSE"),text;
#if defined(SSE_BUILD)
  text=T("Thank you for running Steem SSE. We hope you will get many hours of\
...
#ifndef SSE_BETA
  text+=" ";
  text+=T("Do you want to put a shortcut to Steem SSE in your Start Menu?");
#endif
...
#endif
  // v410: after link step, skip the rest if ini is already OK
  EasyStr tos_path=CSF.GetStr("Machine","ROM_File","");
  if(tos_path.NotEmpty())
  {
    if(load_TOS(tos_path))
      return ERR_OK;
  }
  caption=T("TOS Image");
 ...
 easily downloadable from the internet. Alternatively you can get ST programs\
 that will save out the TOS from an ST. After clicking OK find a TOS image file\
 and click open. This setting, and all the other settings you are about to set,\
 can be easily changed at any time when running Steem.");
#ifdef WIN32
  mbp.lpszText=text;
  mbp.lpszCaption=caption;
  mbp.dwStyle=MB_USERICON|MB_OK;
  ...
  for(;;) {
    ...
    ROMFile=RunDir+SLASH "tos.img"; // hopefully player copied the file there
    ROMFile=FileSelect(NotifyWin,T("Select TOS Image"),RunDir,fstypes,1,true,"img",ROMFile);
#elif defined(SSE_VID_2SCREENS)
    // Windows deduces screen from handle
    ROMFile=FileSelect(NotifyWin,T("Select TOS Image"),RunDir,fstypes,1,true,"img");
#else
    ROMFile=FileSelect(NULL,T("Select TOS Image"),RunDir,fstypes,1,true,"img");
#endif
    free(fstypes);
#endif//WIN32
#ifdef UNIX
    fileselect.set_corner_icon(&Ico16,ICO16_CHIP);
    ROMFile=fileselect.choose(XD,UsersPath,"",T("Select TOS Image"),FSM_LOAD|FSM_LOADMUSTEXIST,
 ...
 all other ST emulators, use files with the extension ST, STX, DIM or MSA on a\
 ...
 it easy to switch disks in and out of drives.")+"\n\n"+
  ...
  if(proceed)
  {
    ...
    Path=ChooseFolder(NotifyWin,T("Pick a Folder"),RunDir);
#else
    ...
    if(Path.NotEmpty())
    {
      ...
      if(!Found)
      {
        if(DiskMan.CreateDiskImage(DiskMan.HomeFol+SLASH+T("Blank Disk")+".st",80*9*2,9,2))
        {
          FloppyDrive[DRIVE_A].SetDisk(DiskMan.HomeFol+SLASH+T("Blank Disk")+".st");
          FloppyDisk[DRIVE_A].DiskName=T("Blank Disk");
          DiskMan.InsertHistoryAdd(DRIVE_A,FloppyDisk[DRIVE_A].DiskName,
            FloppyDrive[DRIVE_A].GetDisk(),"");
        }
      }
    }
  }
  caption=T("GEMDOS Hard Drives");
  text=T("Steem can also emulate hard drives on the ST. You can have up\
 to 10 hard drives, each one is mapped to a folder on your PC hard drive.")+
 "\n\n"+T("Would you like to select a folder to be ST hard drive C now?");
  ...
  if(proceed)
  {
    ...
    {
#if defined(SSE_VID_2SCREENS)
      Path=ChooseFolder(NotifyWin,T("Pick a Folder"),Path);
#else
  ...
  text=T("Congratulations, Steem is now ready to go. Select a disk in the Disk\
 Manager. Click on the yellow play\
