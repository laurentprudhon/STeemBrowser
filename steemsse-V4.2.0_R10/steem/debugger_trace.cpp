...
short &CharHeight=GuiSM.mCharHeight;
...
void trace() {
  ...
  DbgStatusBarMsg("Trace");
#endif 
  TRACE("%s %06X\n","Trace",pc); // TRACE Trace, the concept!
#if SSE_VERSION>=420
  MEM_ADDRESS old_pc2=pc;
#else
  old_pc=pc; // disruptive!
#endif
  trace_init();
  d2_trace=true;
  disa_d2(pc&0x00FFFFFF);
  int time_text_entry=trace_entries,sys_cycles_this_instruction;
...
//  }catch (TMC68kException &e){
  CATCH_M68K_EXCEPTION
    ...
    {
#if defined(SSE_DEBUGGER_STATUS_BAR) // v402
      char Mess[40];
      sprintf(Mess,"Exception %d",e.bombs);
      DbgStatusBarMsg(Mess);
 ...
 (=RETRY)\nor skip?(=IGNORE)",(EasyStr("Exception ")+itoa(e.bombs,d2_t_buf,10)).
        c_str(),MB_ABORTRETRYIGNORE|MB_ICONEXCLAMATION))
      {
      case IDABORT:
        e.crash();
        trace_exception_display(&e);
        break;
      case IDRETRY: // not sure it's useful?
#if SSE_VERSION>=420
        SET_PC(old_pc2);
#else
        SET_PC(old_pc);
  ...
  strcpy(t_d_e[time_text_entry].name,EasyStr("Instruction time: ")
         +(sys_cycles_this_instruction/TICKS8)+" cycles ("+ sys_cycles_this_instruction + ")");
#else
  strcpy(t_d_e[time_text_entry].name,EasyStr("Instruction time: ")
         +(sys_cycles_this_instruction)+" cycles");
#endif
...
void trace_display_clear() {
  ...
  InvalidateRect(trace_scroller,NULL,FALSE);
}
...
void trace_exception_display(TMC68kException *exc) {
  d2_trace=true;
...
void trace_display() {
  ...
  for(int n=0;n<trace_entries;n++)
  {
    ...
    if(t_d_e[n].when!=TDE_TEXT_ONLY)
    {
      ...
      else
      {
        ...
        x+=get_text_width(ad_desc)+GUIMUL(60);
        if(t_d_e[n].when & TDE_BEFORE)
        {
          ox=x;
  ...
  {
    SetWindowPos(trace_window_handle,HWND_TOP,0,0,max_x+30,
      y+GUIMUL(153+EXTRA_HEIGHT+GuiSM.cy_caption()+GetSystemMetrics(SM_CYHSCROLL)),
...
LRESULT CALLBACK trace_window_WndProc(HWND Win,UINT Mess,
  ...
  switch(Mess) {
  case WM_SIZE:
    MoveWindow(m_b_trace.handle,10,5,GET_X_LPARAM(lPar)-GUIMUL(20),GUIMUL(50+EXTRA_HEIGHT),true);
    MoveWindow(trace_repeat_trace_button,10,GET_Y_LPARAM(lPar)-GUIMUL(35),
      GET_X_LPARAM(lPar)-GUIMUL(20),GUIMUL(30),true);
    SetWindowPos(trace_scroller,0,0,0,GET_X_LPARAM(lPar)-GUIMUL(20),GET_Y_LPARAM(lPar)
      -GUIMUL(145)-GUIMUL(EXTRA_HEIGHT),SWP_NOZORDER|SWP_NOMOVE);
    break;
  case WM_DRAWITEM:
  {
...
void trace_window_init() {
 // TRACE("**** %d %d\n",FONT_SIZE,GUI_FONT_SIZE);
  trace_window_handle=CreateWindowEx(/*WS_EX_TOOLWINDOW*/0,"Steem Trace Window","Trace",
                                     ...
                                     110,310,370,400+EXTRA_HEIGHT,NULL,NULL,hInstance,0);
  m_b_trace.owner=trace_window_handle;
  m_b_trace.handle=CreateWindowEx(WS_EX_CLIENTEDGE,WC_LISTVIEW,"",LVS_REPORT|LVS_SHAREIMAGELISTS
                                  |LVS_NOSORTHEADER|LVS_OWNERDRAWFIXED|WS_VISIBLE|WS_CHILD|WS_CLIPSIBLINGS,
                                  10,1,400,55,m_b_trace.owner,(HMENU)1,hInstance,NULL);
...
//  m_b_trace.active=false;
  ...
  m_b_trace.init();
  CreateWindowEx(0,"Static","sr before",WS_VISIBLE|WS_CHILDWINDOW|SS_LEFTNOWORDWRAP,GUIMUL(10),
                 GUIMUL(63+EXTRA_HEIGHT),GUIMUL(50),CharHeight,trace_window_handle,(HMENU)2001,
                 hInstance,NULL);
  trace_sr_before_display=CreateWindowEx(WS_EX_CLIENTEDGE,"Static","trace sr display",
                                         WS_BORDER|WS_VISIBLE|WS_CHILDWINDOW|SS_NOTIFY,
                                         GUIMUL(60),GUIMUL(60+EXTRA_HEIGHT),GUIMUL(200),
                                         GUIMUL(20),trace_window_handle,(HMENU)0,hInstance,NULL);
  SetWindowLongPtr(trace_sr_before_display,GWLP_USERDATA,(LONG_PTR)&trace_sr_before);
  SetWindowLongPtr(trace_sr_before_display,GWLP_WNDPROC,(LONG_PTR)sr_display_WndProc);
  CreateWindowEx(0,"Static","sr after",WS_VISIBLE|WS_CHILDWINDOW|SS_LEFTNOWORDWRAP,GUIMUL(10),
                 GUIMUL(83+EXTRA_HEIGHT),GUIMUL(50),CharHeight,trace_window_handle,(HMENU)2002,
                 hInstance,NULL);
  trace_sr_after_display=CreateWindowEx(WS_EX_CLIENTEDGE,"Static","trace sr display",WS_BORDER|WS_VISIBLE
                                        |WS_CHILDWINDOW|SS_NOTIFY,GUIMUL(60),GUIMUL(80+EXTRA_HEIGHT),
                                        GUIMUL(200),GUIMUL(20),trace_window_handle,(HMENU)0,hInstance,NULL);
  SetWindowLongPtr(trace_sr_after_display,GWLP_USERDATA,(LONG_PTR)&SR);
  SetWindowLongPtr(trace_sr_after_display,GWLP_WNDPROC,(LONG_PTR)sr_display_WndProc);
  trace_scroller.CreateEx(WS_EX_CLIENTEDGE,WS_CHILD|WS_VISIBLE|WS_VSCROLL|WS_HSCROLL,GUIMUL(10),
                          GUIMUL(105+EXTRA_HEIGHT),GUIMUL(260),GUIMUL(130),trace_window_handle,
                          (HMENU)100,hInstance);
  trace_repeat_trace_button=CreateWindowEx(WS_EX_CLIENTEDGE,"Button","Repeat Trace",
                                           WS_BORDER|WS_VISIBLE|WS_CHILDWINDOW|BS_PUSHBUTTON,GUIMUL(150),
                                           GUIMUL(76+EXTRA_HEIGHT),GUIMUL(130),GUIMUL(35),
                                           trace_window_handle,(HMENU)1003,hInstance,NULL);
