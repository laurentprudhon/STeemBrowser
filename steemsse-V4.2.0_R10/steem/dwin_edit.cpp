...
//            if((int)ad-(int)DWin_edit_subject_ad>=2*wpl)break;
...
#if defined(SSE_VID_STVL_UPD) // a bit silly
  ...
  Stvl.mde0=(Glue.ShiftMode&1);
  Stvl.mde1=Glue.ShiftMode>>1;
  Stvl.exts=(Glue.SyncMode&1);
  Stvl.pal=Glue.SyncMode>>1;
#endif
  ...
  case WM_CONTEXTMENU:
  {
    if(DWin_edit_subject_type==0)
    {
      ...
      TrackPopupMenu(insp_menu,TPM_LEFTALIGN|TPM_LEFTBUTTON,
        GET_X_LPARAM(lPar),GET_X_LPARAM(lPar),0,DWin,NULL);
      return 0;
    }
    break;
  }
