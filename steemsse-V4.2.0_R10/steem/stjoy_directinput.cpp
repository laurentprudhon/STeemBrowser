...
CONDITION: WIN32
DESCRIPTION: Code to read PC joysticks using DirectInput8.
It's another deprecated library...
---------------------------------------------------------------------------*/
...
BOOL CALLBACK DIEnumJoysticksCallback(LPCDIDEVICEINSTANCE pInst,LPVOID) {
  ...
  TRACE2("%s %s %s $%X\n","Joystick","detect",pInst->tszProductName,
    pInst->dwDevType);
...
#if !defined(MINGW_BUILD) && !defined(VC_BUILD)
...
void DIJoyGetPos(int j) {
  if(DIJoy[j])
  {
    ...
    for(int i=0;i<2;i++)
    {
      ...
      {
        if(Ret!=DI_OK)
          DIDisconnected[j]=(runstate==RUNSTATE_RUNNING)?200:100;
        break;
      }
    }
