...
char reg_browser_entry_name[MAX_MEMORY_BROWSERS][8];
DWORD *reg_browser_entry_pointer[MAX_MEMORY_BROWSERS];
...
void mem_browser::new_window(MEM_ADDRESS address,ETypeDispType new_disp_type) {
  ...
  else
  {
    ...
    else if(new_disp_type==DT_BITMAP)
    {
      Title="Bitmap";
      owner_w-=20;
      owner_h+=100; // window is resizable anyway
      res=screen_res; // defaults can be changed
      pixelsh=(res)?640:320;
      pixelsv=(res&2)?400:200;
    }
#endif
    ...
    owner=CreateWindowEx(ex_style,"Steem Mem Browser Window",Title,
      ...
      10,20,GUIMUL(owner_w),GUIMUL(owner_h),NULL,NULL,hInstance,0);
    if(!IsWindow(owner)||owner==NULL)
    {
      MessageBox(NULL,"Failed to open new window. Bad problem with Windows!!! \
Time to restart!!!!","ERROR",MB_ICONEXCLAMATION|MB_SETFOREGROUND|MB_TASKMODAL|MB_TOPMOST);
      delete this; // no leak
    }
    else
    {
      ...
      y=(new_disp_type==DT_REGISTERS||IS_IOLIST_PSEUDO_ADDRESS(ad))?2:GUIMUL(30);
      //create the list box
      handle=CreateWindowEx(WS_EX_CLIENTEDGE,WC_LISTVIEW,"",
        ...
        10,y,rc.right-20,rc.bottom-5-y,owner,(HMENU)1,hInstance,NULL);
      ...
      else
      {
        ...
        {
          int offset=10;
          editbox=new mr_static(/*label*/"",/*name*/"browser address",/*x*/offset,
          ...
          int wid=GetTextSize(hSteemGuiFont,"XXXXXX").Width;
          offset+=wid+GuiSM.mHorizontalSeparation*2;
          wid=GuiSM.mCbUnits*6;
          HWND Win=CreateWindowEx(WS_EX_CLIENTEDGE,"Combobox","",WS_VISIBLE|
            WS_CHILD|CBS_DROPDOWNLIST|WS_VSCROLL,offset,2,wid,160,owner,
            (HMENU)2,hInstance,NULL);
          offset+=wid+GuiSM.mHorizontalSeparation;
#if defined(SSE_DEBUGGER_SHOWBITMAP)
          if(new_disp_type==DT_BITMAP)
          {
            SendMessage(Win,CB_ADDSTRING,0,(LPARAM)"4 planes");
            SendMessage(Win,CB_ADDSTRING,0,(LPARAM)"2 planes");
            SendMessage(Win,CB_ADDSTRING,0,(LPARAM)"1 plane");
            SendMessage(Win,CB_SETCURSEL,(WPARAM)res,0); // 0-2
          }
          else
#endif
          {
          ...
          CreateWindow("Static","",WS_VISIBLE|WS_CHILD|SS_ETCHEDVERT,
            offset,0,2,CharHeight,owner,(HMENU)7,hInstance,NULL);
          offset+=GuiSM.mHorizontalSeparation;
          Win=CreateWindowEx(WS_EX_CLIENTEDGE,"Edit","",WS_VISIBLE|WS_CHILD|
            ES_AUTOHSCROLL,offset,2,wid,CharHeight,owner,(HMENU)8,hInstance,NULL);
          offset+=wid+GuiSM.mHorizontalSeparation;
#if defined(SSE_DEBUGGER_SHOWBITMAP)
          if(new_disp_type==DT_BITMAP)
          {
            char tmp[64];
            sprintf(tmp,"%d",pixelsh);
            // TODO check all WM_SETTEXT, wparam is unused
            SendMessage(Win,WM_SETTEXT,0,(LPARAM)tmp);
            Win=CreateWindowEx(WS_EX_CLIENTEDGE,"Edit","",WS_VISIBLE|WS_CHILD| // same, not ideal
              ES_AUTOHSCROLL,offset,2,wid,CharHeight,owner,(HMENU)9,hInstance,NULL);
            sprintf(tmp,"%d",pixelsv);
            SendMessage(Win,WM_SETTEXT,0,(LPARAM)tmp);
            offset+=wid+GuiSM.mHorizontalSeparation;
            DbgCreatePushButton("Show",offset,2,wid,owner,9); // because it's slow, not in update()
          }
          else
#endif
          {
            SendMessage(Win,EM_LIMITTEXT,200,0);
            OldEditWndProc=(WNDPROC)SetWindowLongPtr(Win,GWLP_WNDPROC,(LPARAM)FindEditWndProc);
            TOOLINFO ti;
 ...
 prefix with \" (case sensitive)."
            ...
            DbgCreatePushButton("Find Up",offset,2,wid,owner,9);
            offset+=wid+GuiSM.mHorizontalSeparation;
            DbgCreatePushButton("Find Down",offset,2,wid,owner,10);
            offset+=wid+GuiSM.mHorizontalSeparation;
            CreateWindow("Static","",WS_VISIBLE|WS_CHILD|SS_ETCHEDVERT,
              offset,0,2,CharHeight/*27*/,owner,NULL,hInstance,NULL);
            offset+=GuiSM.mHorizontalSeparation;
            Win=DbgCreatePushButton("Dump->",offset,2,wid,owner,4);
            ToolAddWindow(ToolTip,Win,"Right click = to clipboard");
            offset+=wid+GuiSM.mHorizontalSeparation;
            wid=GuiSM.mCbUnits*3;
            Win=CreateWindowEx(WS_EX_CLIENTEDGE,"Combobox","",WS_VISIBLE|WS_CHILD|
              WS_VSCROLL|CBS_DROPDOWN,offset,2,wid,300,owner,(HMENU)5,hInstance,NULL);
            ...
            offset+=wid+GuiSM.mHorizontalSeparation;
            DbgCreatePushButton("Load",offset,2,wid,owner,6);
          }
        }
      }
...
void mem_browser::init() {
  ...
  break_column=-1;
  hex_column=-1;
#ifdef SSE_GUI_BIGICONS
#define MULTI *FONT_SIZE/GUI_FONT_SIZE
#else
#define MULTI
#endif
  ...
  else if(disp_type==DT_MEMORY)
  {
    ...
    lc.cx=150 MULTI;
    //lc.pszText="Disassembly";
    lc.pszText="Disassembly/Description";
...
void mem_browser::get_breakpoint_labels(MEM_ADDRESS addr,int bpl,char *t[3]) {
  ...
  while(n<8)
  {
    ...
    n++;
    regname[1]=debug_uppercase_disa?'A':'a';
    regname[2]=char('0'+n);rv=Cpu.r[n+8];
  }
...
#if defined(SSE_DEBUG_SYMBOLS)
  if(addr==Tos.Basepage)
    strcat(t[0],"bp");
  for(int i=0;i<MAX_SYMBOLS&&TosSymbol[i].ad!=0xFFFFFFFF;i++) // check all symbols
  {
    if((addr&0xFFFFFFFE)==(TosSymbol[i].ad&0xFFFFFFFE))
    {
      if(t[0][0]!='\0')
        strcat(t[0]," ");
      if(TosSymbol[i].ad&1)
        strcat(t[0],"--");
      strcat(t[0],TosSymbol[i].name);
      // no break because there could be more than one
    }
  }
#endif
        ...
        else if(Col==hex_column)
        {
          if(hex_map.NumStrings)
          {
            ...
            while(map[0])
            {
              ...
              DrawText(di->hDC,byte_text,2,&rc,DT_LEFT|DT_SINGLELINE|DT_VCENTER
                |DT_END_ELLIPSIS);
              if(bk_col)
                SetTextColor(di->hDC,text_col);
              rc.left+=wh.Width;
              last_l=rc.left;
              map++;
            }
...
//                *c++='x';*c++='x';
  ...
  //      ldpc2=addr+i*2*my_wpl;
    ...
    dpc=save_dpc;
    if(nOldItems)
      SendMessage(handle,LVM_DELETEITEM,SendMessage(handle,LVM_GETITEMCOUNT,0,0)-1,0);
...
void STfile_write_from_ST_memory(FILE *fp,MEM_ADDRESS ad,int n_bytes) {
  ...
  {
    FPUTC(d2_peek(ad),fp); // doesn't care about SUPER bit, returns 0 if invalid
    ad++;
  }
}
...
LRESULT CALLBACK mem_browser_window_WndProc(HWND Win,UINT Mess,WPARAM wPar,LPARAM lPar) {
  ...
  case WM_CONTEXTMENU:
    if(GetDlgItem(Win,4)==(HWND)wPar)
    {
      ...
      FILE *fp=NULL; // signal for these functions
      disa_to_file(fp,mb->ad,bytes/10,mb->disp_type!=DT_MEMORY);
    }//if
    break;
  case WM_COMMAND:
    mb=(mem_browser*)GetWindowLongPtr(Win,GWLP_USERDATA);
    ...
    case BN_CLICKED:
      ...
      { // SS Dump
        ...
        if(mb->disp_type==DT_MEMORY)
          fn=FileSelect(Win,"Save Memory Block As...",UsersPath,
            "Memory Dump Files\0*.bin\0All Files\0*.*\0\0",1,false,"bin");
        else if(mb->disp_type==DT_INSTRUCTION)
          fn=FileSelect(Win,"Save Disassembly As...",UsersPath,
        ...
        if(fn.NotEmpty())
        {
          ...
          {
            if(mb->disp_type==DT_MEMORY)
              STfile_write_from_ST_memory(fp,mb->ad,bytes);
            else
            {
              bool as_s=IsSameStr_I(fn.Rights(2),".S");
              disa_to_file(fp,mb->ad,bytes,as_s);
            }
            fclose(fp);
          }
        }
...
#if defined(SSE_DEBUGGER_SHOWBITMAP)
      else if(mb->disp_type==DT_BITMAP && wpar_lo==9) // Show
        mb->showbitmap();
#endif      
      else if(wpar_lo==9||wpar_lo==10) // search
      {
        ...
        bool WordOnly=false;
        parse_search_string(Text,BytesToFind,WordOnly);
...
#if defined(SSE_DEBUG_SYMBOLS)
        if(Text[0]=='@' && BytesToFind.NumItems==3)
        {
          MEM_ADDRESS ad=0;
          for(INT_PTR j=0;j<3;j++)
          {
            ad<<=8;
            ad|=BytesToFind[j];
            //TRACE("%x -> %x ",BytesToFind[j],ad);
          }
          //TRACE("-> %X",ad);
          SWAP_BIG_ENDIAN_DWORD(ad);
          ad>>=8;
          //TRACE("-> %X\n",ad);
          mb->ad=ad;
          mb->update();
        ...
        if(ad>=rom_addr_end && dir<0) 
          ad=rom_addr_end-BytesToFind.NumItems;
        if(mb->disp_type==DT_INSTRUCTION)
          ad=oi(ad & ~1,int(dir<0?-1:1));  //offset instruction
        else
        {
          BYTE ToFind=BytesToFind[0];
          if(SafePeek(ad)==ToFind||SafePeek(ad+1)==ToFind) 
            ad+=dir*2;
        }
        ad+=dir*2;
...
//              SetFocus(mb->handle);
    ...
    case EN_CHANGE:
      if(mb->disp_type==DT_BITMAP)
      {
        int control=LOWORD(wPar);
        BOOL bSuccess;
        SHORT number = (SHORT) GetDlgItemInt(Win, control, &bSuccess, FALSE);
        switch(control) {
        case 8:
          mb->pixelsh=number;
          break;
...
void mem_browser::setup_contextmenu(int row,int col) {
  ...
  if(col<=mon_column)
  {
    ...
    for(int w=0;w<my_wpl;w++)
    {
      ...
      bool InRom=!(row_ad<0xe00000||row_ad>=MEM_IO_BASE);
      { // Monitors only on RAM and IO
        HMENU MonPop[2]={CreatePopupMenu(),CreatePopupMenu()};
        if(!InRom)
          AppendMenu(insp_menu,MF_BYCOMMAND|MF_POPUP|MF_CHECK(mask[0]),
                     (UINT_PTR)MonPop[0],"Monitor writes to");
        AppendMenu(insp_menu,MF_BYCOMMAND|MF_POPUP|MF_CHECK(mask[1]),
  ...
  else
  {
    ...
    else
    {
      int n=row+(ad);
      if(n>17)
        return; // fix old, old bug, crash on right click in register browser
...
LRESULT CALLBACK mem_browser_WndProc(HWND Win,UINT Mess,WPARAM wPar,LPARAM lPar) {
  ...
  case WM_RBUTTONUP: case WM_LBUTTONDOWN: case WM_LBUTTONDBLCLK: 
    ...
    if(mb->handle!=NULL)
    {
      int col,row;
      POINT m;RECT rc;
      m.x=GET_X_LPARAM(lPar)+GetScrollPos(Win,SB_HORZ),m.y=GET_Y_LPARAM(lPar);
      ...
      if(row>=0&&row<(mb->lb_height))
      {
        if((col==1||col==2)&&(mb->disp_type)!=DT_REGISTERS)
        {
          if(Mess==WM_LBUTTONDOWN||Mess==WM_LBUTTONDBLCLK)
          {
            ...
            bool InRom=(ad>=0xe00000&&ad<MEM_IO_BASE);
...
//                    MessageBox(0,EasyStr("Clicked ")+(il->name),"Clicky",0);
        ...
        {
          POINT pt;
          pt.x=GET_X_LPARAM(lPar);
          pt.y=GET_Y_LPARAM(lPar);
...
LRESULT CALLBACK mem_browser::FindEditWndProc(HWND Win,UINT Mess,WPARAM wPar,LPARAM lPar) {
  if(Mess==WM_KEYDOWN)
  {
    HWND hParent=GetParent(Win);
    if(wPar==VK_RETURN||wPar==VK_DOWN||wPar==VK_NEXT)
    {
      PostMessage(hParent,WM_COMMAND,MAKEWPARAM(10,BN_CLICKED),(LPARAM)GetDlgItem(hParent,10));
      return 0;
    }
    else if(wPar==VK_UP||wPar==VK_PRIOR)
    {
      PostMessage(hParent,WM_COMMAND,MAKEWPARAM(9,BN_CLICKED),(LPARAM)GetDlgItem(hParent,9));
      return 0;
    }
  }
...
void mem_browser::showbitmap() {
  // request and long overdue: a memory browser that displays ST bitmaps anywhere in memory
  HDC dc=GetDC(NULL);
  HDC dch=GetDC(handle); // our list control becomes a graphics window of sorts
  RECT rc;
  GetClientRect(handle,&rc);
  FillRect(dch,&rc,(HBRUSH)GetProp(handle,"BackgroundBrush")); // erase previous picture
  HDC memdc=CreateCompatibleDC(dch);
  SHORT maph,mapv,st_mem_row_len;
  // adjust size based on resolution
  if(res)
  {
    if(res&2) // hires
    {
      st_mem_row_len=pixelsh/8;
      maph=pixelsh;
      mapv=pixelsv;
    }
    else // med
    {
      st_mem_row_len=pixelsh/4;
      maph=pixelsh;
      mapv=pixelsv*2;
    }
  }
  ...
  {
    st_mem_row_len=pixelsh/2;
    maph=pixelsh*2;
    mapv=pixelsv*2;
  }
  HBITMAP hBitmap=CreateCompatibleBitmap(dc,maph,mapv);
  BITMAP bm;
  GetObject(hBitmap,sizeof(BITMAP),&bm);
  /*      TRACE("bm.bmWidth %d bm.bmHeight %d bm.bmBitsPixel %d bm.bmType %d bm.bmPlanes %d bm.bmWidthBytes %d bm.bmBits %p\n",
          bm.bmWidth,bm.bmHeight,bm.bmBitsPixel,bm.bmType,bm.bmPlanes,bm.bmWidthBytes,bm.bmBits);
        BYTE *pb=(BYTE*)bm.bmBits; // NULL
  */
  HGDIOBJ oldbmp=SelectObject(memdc,hBitmap);
  // must do the bitplanes to chunky conversion
  for(int line=0;line<pixelsv;line++)
  {
    for(int row=0;row<pixelsh;row+=16)
    {
      DU64 raster; // we use 64bit rasters anyway
      switch(res) {
      case 0: // 4 planes
        raster.d16[B0]=SafeDPeek(ad+line*st_mem_row_len+row/2);
        raster.d16[B1]=SafeDPeek(ad+line*st_mem_row_len+row/2+2);
        raster.d16[B2]=SafeDPeek(ad+line*st_mem_row_len+row/2+4);
        raster.d16[B3]=SafeDPeek(ad+line*st_mem_row_len+row/2+6);
        for(int i=15;i>=0;i--)
        {
          WORD r0=(raster.d16[B0]>>i)&1;
          WORD r1=(raster.d16[B1]>>i)&1;
          WORD r2=(raster.d16[B2]>>i)&1;
          WORD r3=(raster.d16[B3]>>i)&1;
          int stpixel=r0 | (r1<<1) | (r2<<2) | (r3<<3);
          int pcpix=*(PCpal+stpixel);
          //pcpix=rand();
          SetPixel(memdc,row+(16-i),line,pcpix);
        }
        break;
      case 1: // 2 planes
#if 1 // optimised
        raster.d16[B1]=SafeDPeek(ad+line*st_mem_row_len+row/2);
        raster.d16[B0]=SafeDPeek(ad+line*st_mem_row_len+row/2+4);
        raster.d16[B3]=SafeDPeek(ad+line*st_mem_row_len+row/2+2);
        raster.d16[B2]=SafeDPeek(ad+line*st_mem_row_len+row/2+6);
        for(int i=31;i>=0;i--)
        {
          DWORD r0=(raster.d32[LO]>>i)&1;
          DWORD r1=(raster.d32[HI]>>i)&1;
          int stpixel=r0 | (r1<<1);
          int pcpix=*(PCpal+stpixel);
          //pcpix=rand();
          SetPixel(memdc,row*2+(31-i),line,pcpix);
        }
#else
        raster.d16[B0]=SafeDPeek(ad+line*st_mem_row_len+row/2);
        raster.d16[B1]=SafeDPeek(ad+line*st_mem_row_len+row/2+2);
        ...
        for(int i=31;i>=0;i--)
        {
          DWORD r0=(((raster.d16[B0]<<16)|raster.d16[B2])>>i)&1;
          DWORD r1=(((raster.d16[B1]<<16)|raster.d16[B3])>>i)&1;
          int stpixel=r0 | (r1<<1);
          int pcpix=*(PCpal+stpixel);
          //pcpix=rand();
          SetPixel(memdc,row*2+(31-i),line,pcpix);
        ...
        break;
      default: // 1 plane
        raster.d16[B3]=SafeDPeek(ad+line*st_mem_row_len+row/2); // reverse order
        raster.d16[B2]=SafeDPeek(ad+line*st_mem_row_len+row/2+2);
        raster.d16[B1]=SafeDPeek(ad+line*st_mem_row_len+row/2+4);
        raster.d16[B0]=SafeDPeek(ad+line*st_mem_row_len+row/2+6);
        for(int i=63;i>=0;i--)
        {
          int stpixel=(raster.d64>>i)&1;
          int pcpix=(stpixel) ? 0 : 0x00ffffff;
          //pcpix=rand();
          SetPixel(memdc,row*4+(63-i),line,pcpix);
        }
        break;
      }//sw
    }
  ...
  if(res&2)
    BitBlt(dch,0,0,pixelsh,pixelsv,memdc,0,0,SRCCOPY);
  else
    StretchBlt(dch,0,0,bm.bmWidth,bm.bmHeight,memdc,0,0,pixelsh,pixelsv,SRCCOPY);
...
mem_browser::~mem_browser() {
  for(int n=0;n<MAX_MEMORY_BROWSERS;n++)
  {
    //TRACE("mem_browser::~mem_browser()\n");
    //TRACE("mb %d = %p vs %p\n",n,m_b[n],this);
    if(m_b[n]==this)
    {
      for(int m=n;m<MAX_MEMORY_BROWSERS-1;m++)
        m_b[m]=m_b[m+1];
      m_b[MAX_MEMORY_BROWSERS-1]=NULL; //TRACE("mb %d NULL\n",n);
      n--;
    }
  }
}
