...
LRESULT CALLBACK PicButton_WndProc(HWND Win,UINT Mess,WPARAM wPar,LPARAM lPar);
LRESULT CALLBACK PathDisplay_WndProc(HWND Win,UINT Mess,WPARAM wPar,LPARAM lPar);
LRESULT CALLBACK STCharChoose_WndProc(HWND Win,UINT Mess,WPARAM wPar,LPARAM lPar);
LRESULT CALLBACK HyperLinkWndProc(HWND Win,UINT Mess,WPARAM wPar,LPARAM lPar);
LRESULT CALLBACK TextDisplayGroupBox_WndProc(HWND Win,UINT Mess,WPARAM wPar,LPARAM lPar);
...
void UnregisterSteemControls() {
  UnregisterClass("Steem Flat PicButton",hInstance);
  KillTimer(NULL,PicButton_TimerID);
  UnregisterClass("Steem Path Display",hInstance);
  UnregisterClass("Steem ST Character Chooser",hInstance);
  UnregisterClass("Steem HyperLink",hInstance);
  UnregisterClass("Steem Text Display",hInstance);
}
...
LRESULT CALLBACK PicButton_WndProc(HWND Win,UINT Mess,
  ...
  bool DoPaint=false,CheckState=false;
  TPicButtonInfo *Inf;
  switch(Mess) {
  case WM_SETTEXT:
  ...
  case WM_CREATE:
  {
    ...
    int Texti=atoi(Text);
    Ico=hGUIIcon[MIN(Texti,RC_NUM_ICONS-1)];
    Inf=NULL;
    if(Ico) 
    {
      ICONINFO ii;
  ...
  case WM_LBUTTONUP:
    if(GetPropI(Win,"ClickedIn")) 
    {
      ...
      int Id=GetDlgCtrlID(Win);
      HWND hPrt=GetParent(Win);
      PostMessage(hPrt,WM_COMMAND,MAKEWPARAM(Id,BN_UNPUSHED),(LPARAM)Win);
      if(GET_X_LPARAM(lPar)<rc.right && GET_Y_LPARAM(lPar)<rc.bottom||Mess==WM_KEYUP)
        SendMessage(hPrt,WM_COMMAND,MAKEWPARAM(Id,BN_CLICKED),(LPARAM)Win);
    }
    CheckState=true;
    break;
  case WM_ENABLE:
  ...
  if(CheckState) 
  {
    UINT_PTR OldState=GetPropI(Win,"State");
    UINT_PTR NewState=UINT_PTR((GetFocus()==Win && IsWindowEnabled(Win)) ? 2 : 0);
    bool Over=false;
  ...
  if(DoPaint) 
  {
    ...
    UINT_PTR State=GetPropI(Win,"State");
    GET_THIS;
    DC=GetDC(Win);
    if(State==1) 
    { // Down
...
HBITMAP STCharChoose_CreateSTCharBitmap(BYTE STAscii,HBRUSH FgBr,HBRUSH BkBr) {
  ...
  HBITMAP STCharsBmp=LoadBitmap(hInstance,"ST_CHARS");
...
LRESULT CALLBACK STCharChoose_WndProc(HWND Win,UINT Mess,
  ...
  if(GetWindowLong(Win,GWL_STYLE)&1) 
  {
    ...
    case WM_CREATE:
    {
      ...
      SetWindowPos(Win,HWND_TOPMOST,rc.left,MIN(rc.top,(LONG)GetScreenHeight()
        -(y+22)),WinWid+2,y+22,0);
      return 0;
    }
    case WM_PAINT:
    {
      ...
      if(OverCharBmp) 
      {
        ...
        BitBlt(DC,(int)GetPropI(Win,"OverX"),(int)GetPropI(Win,"OverY"),
          18,18,TempDC,0,0,SRCCOPY);
      }
      DeleteDC(TempDC);
      ReleaseDC(Win,DC);
    ...
    case WM_LBUTTONUP:case WM_MBUTTONUP:case WM_RBUTTONUP:
    {
      ...
      if(Mess==WM_MOUSEMOVE) 
      {
        if((BYTE)HIWORD(Val)!=(BYTE)GetPropI(Win,"OverSTAscii")) 
        {
          ...
          if(GetPropI(Win,"OverSTAscii")&&wPar) 
            SetCapture(Win);
          SetPropI(Win,"OverSTAscii",(INT_PTR)LOBYTE(HIWORD(Val)));
          SetPropI(Win,"OverX",(INT_PTR)(pt.x/20)*20+1);
          SetPropI(Win,"OverY",(INT_PTR)(pt.y/20)*20+1);
  ...
  else 
  {
    ...
    case WM_PAINT:
    {
      ...
      BYTE STAscii=(BYTE)HIWORD(GetProp(Win,"Selection"));
    ...
    case CCPOP_CHOSEN:
      ...
      SendMessage(GetParent(Win),WM_COMMAND,MAKEWPARAM(GetDlgCtrlID(Win),
        (WORD)(LOWORD(lPar)?CBN_SELENDOK:CBN_SELENDCANCEL)),(LPARAM)Win);
    ...
    case WM_LBUTTONDOWN:case WM_MBUTTONDOWN:case WM_RBUTTONDOWN:
    {
      ...
      HWND Pop=CreateWindowEx(0,"Steem ST Character Chooser","",WS_POPUP
        |WS_VISIBLE|WS_BORDER|WS_CHILD|1,MIN(rc.left,(LONG)GetScreenWidth()-258),
        rc.bottom,258,100,Win,NULL,hInstance,NULL);
...
LRESULT CALLBACK HyperLinkWndProc(HWND Win,UINT Mess,WPARAM wPar,LPARAM lPar) {
  bool DoPaint=false,CheckState=false,GetSize=false;
  ...
  case WM_LBUTTONUP:
    if((GetWindowLong(Win,GWL_STYLE) & HL_STATIC)==0) 
    {
      if(GetPropI(Win,"ClickedIn")) 
      {
        ...
        RECT rc;
        GetClientRect(Win,&rc);
        if(GET_X_LPARAM(lPar)<rc.right && GET_Y_LPARAM(lPar)<rc.bottom) 
        {
          int Len=(int)DefWindowProc(Win,WM_GETTEXTLENGTH,0,0)+1;
          char *Text=new char[Len+1];
          DefWindowProc(Win,WM_GETTEXT,Len,(LPARAM)Text);
          ...
          if(strchr(Text,'|'))
            tp=strchr(Text,'|')+1;
          ShellExecute(0,0,"iexplore.exe",tp,0,SW_SHOWNORMAL); //TODO?
          delete[] Text;
        }
      }
      CheckState=true;
  ...
  if(CheckState) 
  {
    if((GetWindowLong(Win,GWL_STYLE) & HL_STATIC)==0) 
    {
      UINT_PTR OldState=GetPropI(Win,"State"),NewState=0;
      bool Over=false;
      if(IsWindowEnabled(Win)) 
      {
        POINT pt;
...
LRESULT CALLBACK TextDisplayGroupBox_WndProc(HWND Win,UINT Mess,
                                              WPARAM wPar,LPARAM lPar) {
  //HWND TextBox=GetDlgItem(Win,IDC_TEXTCONTROL);
  switch(Mess) {
  ...
  case WM_NOTIFY: // link in rtf file
  {
    ...
    if(wPar==TGeneralInfo::IDC_TEXTCONTROL && pnmh->code==EN_LINK)
    {
...
HWND CreateTextDisplay(HWND daddy,int x,int y,int w,int h,INT_PTR id) {
  ...
  HWND tb= 
    ...
    (id==TGeneralInfo::ID_RICHTEXT) ? CreateWindowEx(WS_EX_CLIENTEDGE,RICHEDIT_CLASS,0
    ,mask,0,0,w,h,gb,(HMENU)TGeneralInfo::IDC_TEXTCONTROL,hInstance,NULL) : 
#endif
    CreateWindowEx(WS_EX_CLIENTEDGE,"Edit","",mask,0,0,w,h,gb,
    (HMENU)TGeneralInfo::IDC_TEXTCONTROL,hInstance,NULL);
...
//---------------------------------------------------------------------------
int hxc_buttonpicker::timer_notify_proc(void *t,Window win,INT_PTR id)
{
