...
int iolist_length=0;
Tiolist_entry iolist[IOLIST_MAX_ENTRIES];


void iolist_add_entry(MEM_ADDRESS ad,char*name,int bytes,char*bitmask,BYTE*ptr) {
  ASSERT(iolist_length<IOLIST_MAX_ENTRIES);
  ...
  iolist_length++;
  //TRACE("add %X (%d bytes) as %s, iolist_length %d\n",ad,bytes,(name?name:""),iolist_length);
};
...
void iolist_init() {
...
#if defined(SSE_420R5) // A+4=E
  iolist_add_entry(0x4BE,"the_env",4);
#else
  ...
  iolist_add_entry(0xff8203,"VBASEMID",1);
  ...
  iolist_add_entry(0xff8261,"SHIFT (NO GLU)",1);
  iolist_add_entry(0xff8264,"HSCROLL (No GLU)",1);
  ...
  iolist_add_entry(0xff8901,"STE Sound Control",1,"Loop|Play");
  iolist_add_entry(0xff8903,"STE Sound Start High",1);
  iolist_add_entry(0xff8905,"STE Sound Start Mid",1);
  iolist_add_entry(0xff8907,"STE Sound Start Low",1);
  iolist_add_entry(0xff8909,"STE Sound Current High",1);
  iolist_add_entry(0xff890b,"STE Sound Current Mid",1);
  iolist_add_entry(0xff890d,"STE Sound Current Low",1);
  iolist_add_entry(0xff890f,"STE Sound End High",1);
  iolist_add_entry(0xff8911,"STE Sound End Mid",1);
  iolist_add_entry(0xff8913,"STE Sound End Low",1);
  iolist_add_entry(0xff8921,"STE Sound Mode",1,"Mono|.|.|.|.|.|Freq|Freq");
  ...
  iolist_add_entry(0xFF8A28,"ENDMASK 1",2);
  iolist_add_entry(0xFF8A2A,"ENDMASK 2",2);
  iolist_add_entry(0xFF8A2C,"ENDMASK 3",2);
  ...
  iolist_add_entry(0xFF8A3C,"Flags1",1,"Bsy|Hog|Smudge|.|Line3|Line2|Line1|Line0");
  iolist_add_entry(0xFF8A3D,"Flags2",1,"FXSR|NFSR|.|.|Skew3|Skew2|Skew1|Skew0");

  iolist_add_entry(0xFF8C81,"SCC",1); // MSTE addresses
  iolist_add_entry(0xFF8C83,"SCC",1);
  iolist_add_entry(0xFF8C85,"SCC",1);
  iolist_add_entry(0xFF8C87,"SCC",1);
  iolist_add_entry(0xFF8E00,"VmeSysMask",1);
  iolist_add_entry(0xFF8C02,"VmeSysStat",1);
  iolist_add_entry(0xFF8C04,"VmeSysInt",1);
  iolist_add_entry(0xFF8C0c,"VmeMask",1);
  iolist_add_entry(0xFF8C0e,"VmeStat",1);
  iolist_add_entry(0xFF8C20,"Scu",1,".|.|.|.|.|.|cache|16MHz");

  iolist_add_entry(0xfffa01,"MFP GPIP (1)",1,"mono|ring|disk|acia|Blt|CTS|DCD|LPT");
  iolist_add_entry(0xfffa03,"MFP AER (2)",1,"mono|ring|disk|acia|Blt|CTS|DCD|LPT");
  iolist_add_entry(0xfffa05,"MFP DDR (3)",1,"mono|ring|disk|acia|Blt|CTS|DCD|LPT");
  iolist_add_entry(0xfffa07,"MFP IERA (4)",1,"mono|ring|timerA|Rx|RxErr|Tx|TxErr|timerB");
  iolist_add_entry(0xfffa09,"MFP IERB (5)",1,"disk|acia|timerC|timerD|Blt|CTS|DCD|LPT");
  iolist_add_entry(0xfffa0b,"MFP IPRA (6)",1,"mono|ring|timerA|Rx|RxErr|Tx|TxErr|timerB");
  iolist_add_entry(0xfffa0d,"MFP IPRB (7)",1,"disk|acia|timerC|timerD|Blt|CTS|DCD|LPT");
  iolist_add_entry(0xfffa0f,"MFP ISRA (8)",1,"mono|ring|timerA|Rx|RxErr|Tx|TxErr|timerB");
  iolist_add_entry(0xfffa11,"MFP ISRB (9)",1,"disk|acia|timerC|timerD|Blt|CTS|DCD|LPT");
  iolist_add_entry(0xfffa13,"MFP IMRA (10)",1,"mono|ring|timerA|Rx|RxErr|Tx|TxErr|timerB");
  iolist_add_entry(0xfffa15,"MFP IMRB (11)",1,"disk|acia|timerC|timerD|Blt|CTS|DCD|LPT");
  ...
  iolist_add_entry(0xfffa19,"MFP TACR (13)",1,"low|event|div2|div1|div0");
  iolist_add_entry(0xfffa1b,"MFP TBCR (14)",1,"low|event|div2|div1|div0");
  iolist_add_entry(0xfffa1d,"MFP TCDCR (15)",1,".|Cdiv2|Cdiv1|Cdiv0|.|Ddiv2|Ddiv1|Ddiv0");
...
*/
  ...
  iolist_add_entry(FAKE_IO_START+6,"OSD2 CPU",2,
    "trc|exc|.|.|.|.|.|.|.|.|.|.|.|.|.|.");
  iolist_add_entry(FAKE_IO_START+8,"OSD3 Glue",2,
    "tricks|RS|.|.|.|.|.|.|.|.|.|.|.|.|.|.");

  iolist_add_entry(FAKE_IO_START+10,"TRACE Level",2,
    // more detailed
    "2|3|.|.|.|.|.|.|.|.|.|.|.|.|.|.");

  iolist_add_entry(FAKE_IO_START+12,"TRACE GLU/MMU",2,
    "reg|vert|.|.|.|.|.|.|.|.|.|.|.|.|.|.");
  iolist_add_entry(FAKE_IO_START+14,"TRACE irq/mfp",2,
    "a|b|c|d|e|syn|rte|evt|.|.|.|.|.|.|.|.");
  iolist_add_entry(FAKE_IO_START+16,"TRACE disk",2,
    "str|data|psg|reg|wd|mfm|.|.|.|.|.|.|.|.|.|.");
...
//   15   14   13   12    11   10  9
  ...
  iolist_add_entry(FAKE_IO_START+28,"Trap",2,
    // to deactivate unwanted logging
    "1|2|13|14|.|.|.|.|.|.|.|.|.|.|.|.");
  ...
  iolist_add_entry(0xfffc00,"Keyboard ACIA SR",1,"IRQ|PE|OVR|FE|CTS|DCD|TDRE|RDRF");
  iolist_add_entry(0xfffc02,"Keyboard ACIA DR",1);
  iolist_add_entry(0xfffc04,"MIDI ACIA SR",1,"IRQ|PE|OVR|FE|CTS|DCD|TDRE|RDRF");
  iolist_add_entry(0xfffc06,"MIDI ACIA DR",1);
...
//    if(!(((iolist[n].ad)^ad)&~1)){ //even parts agree
#if SSE_VERSION>=420 && !defined(SSE_420R2) // there was no proper switch
    ...
    else if((iolist[n].ad & -4)==(ad & -4))// it was a bad idea, what if the address starts on say $FF8A32?
    { //even parts agree
      if(iolist[n].bytes==4)
      {
        return iolist+n;
...
void iolist_box_click(int xc,Tiolist_entry*il,BYTE*ptr) { //toggle bit clicked
  ...
  //  long mask=1;
    ...
    if(crashy)
      MessageBox(0,"A bus error occurred while trying|to edit this memory.",(char*)rowbombs,0);
  }
}
...
int iolist_box_draw(HDC dc,int x1,int y1,int w,int h,Tiolist_entry *il,BYTE *ptr) {
  if(il->bitmask.NotEmpty())
  {
