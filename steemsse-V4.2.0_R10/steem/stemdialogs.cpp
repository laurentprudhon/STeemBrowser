...
TStemDialog *DialogList[MAX_DIALOGS]=
{NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,
 NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL};
int nStemDialogs=0;

#ifdef WIN32
bool StemDialog_RetDefVal;
...
const UINT BPPToILC=ILC_COLOR32;//ILC_COLOR16; // seems OK now? CHECK
#endif




TStemDialog::TStemDialog() { // and no destructor defined
  Handle=NULL;
  ...
  Left=Top=FSLeft=FSTop=CW_USEDEFAULT;
#else
  Left=100;Top=100;
  FSLeft=50;FSTop=50;
#endif
...
void TStemDialog::GetPageControlList(DynamicArray<HWND> &ChildList) {
  ...
  while(Child)
  {
    if(GetDlgCtrlID(Child)<60000) // hardcoded 60000!!!
      ChildList.Add(Child);
...
void TStemDialog::ShowPageControls() {
  ...
  GetPageControlList(ChildList);
  for(int n=0;n<ChildList.NumItems;n++) 
    ShowWindow(ChildList[n],SW_SHOW); // can be tricky
}
...
LRESULT TStemDialog::DefStemDialogProc(HWND Win,UINT Mess,WPARAM wPar,LPARAM lPar) {
  ...
  TStemDialog *This=(TStemDialog*)GetWindowLongPtr(Win,GWLP_USERDATA);
  switch(Mess) {
  case WM_SYSCOMMAND:
    switch(wPar) {
  ...
  case WM_MOVE:
  {
    ...
    if(FullScreen)
    {
      if(!IsIconic(StemWin) && !IsZoomed(StemWin))
      {
...
void TStemDialog::LoadPosition(TConfigStoreFile* /*pCSF*/) {
  ...
  Disp.CheckCurrentMonitorConfig(Handle); // Update monitor rectangle
  int W=Disp.rcMonitor.right-Disp.rcMonitor.left;
  int H=Disp.rcMonitor.bottom-Disp.rcMonitor.top;
  Left=MAX(MIN(pCSF->GetInt(Section,"Left",Left),W),(int)Disp.rcMonitor.left);
  Top=MAX(MIN(pCSF->GetInt(Section,"Top",Top),H),(int)Disp.rcMonitor.top);
  FSLeft=pCSF->GetInt(Section,"FSLeft",FSLeft);
  FSLeft=MAX(MIN(FSLeft,W),(int)Disp.rcMonitor.left);
  FSTop=pCSF->GetInt(Section,"FSTop",FSTop);
  FSTop=MAX(MIN(FSTop,H),(int)Disp.rcMonitor.top);
#elif 1
...
#if defined(SSE_420R4)
#ifdef WIN32
void TStemDialog::UpdateLeftTop() {
  RECT rc;
  GetWindowRect(Handle,&rc);
  FSLeft=Left=rc.left; // not sure for FS
  FSTop=Top=rc.top;
}
#endif
#endif
...
void TStemDialog::UpdateDirectoryTreeIcons(DirectoryTree *pTree) {
  ...
  {
    pTree->FileMasksESL[n].Data[0]=(LONG_PTR)(hGUIIcon[pTree->FileMasksESL[n].Data[1]]);
  }
  pTree->ReloadIcons(BPPToILC);
}
      ...
      int icon_index,long input_mask,LPWINDOWPROC WinProc,bool resizable)
{
  ...
  XImage *xi=XGetImage(XD,IconPixmap,0,0,16,16,AllPlanes,ZPixmap);
  //ASSERT(xi);
  if(xi)
  {
    ChangeIcon(XD,Handle,xi);
    XDestroyImage(xi);
  }
