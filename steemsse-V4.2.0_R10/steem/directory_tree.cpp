...
DirectoryTree::DirectoryTree() {
  ...
  DragItem=NULL,DragEntered=false,DragButton=DropPos=0,DropTarget=NULL;
  IDBase=64300;
  il=NULL;
  ChooseOnly=AllowTypeChange=false;
  DoNotifyOnChange=true;
  ExpandTimerActive=false;
  OldTVWndProc=NULL;
  NotifyProc=NULL;
  MenuItem=DragItem=DropTarget=CurrentItem=RootItem=0;
  DragIL=il=0;
  WndProcRet=0;
  TreeID=NULL;
  NotifyThis=NULL;
...
bool DirectoryTree::Create(HWND Par,int x,int y,int w,int h,HMENU ID,DWORD Flags,
  ...
  hTree=CreateWindowEx(WS_EX_CLIENTEDGE,WC_TREEVIEW,"",WS_CHILD|Flags
    |TVS_HASLINES|COFlags|TVS_SHOWSELALWAYS|TVS_HASBUTTONS,x,y,w,h,Par,
...
HTREEITEM DirectoryTree::SelectItemByPath(EasyStr Path) {
  ...
  if(strstr(Path,RootFol)==Path.Text)
  {
    ...
    EasyStr sPathBuf;
    sPathBuf.SetLength(SSE_MAX_PATH);
    char *PathBuf=sPathBuf.Text,ItemName[501];
#else
    char PathBuf[MAX_PATH],ItemName[501];
#endif
    strncpy(PathBuf,Path.Text+strlen(RootFol)+1,SSE_MAX_PATH);
...
HTREEITEM DirectoryTree::NewItem(EasyStr ItemName,HTREEITEM Parent,int Type,
                                 bool Edit) {
  if(hTree==NULL) 
    return NULL;
  ...
  {
    FILE *fp=fopen(ParentFol+SLASH+Name+Ext,"wb");
    if(fp==NULL)
      return NULL;
    fclose(fp);
  }
...
#if defined(SSE_LONG_PATH)
  EasyStr sPath;
  sPath.SetLength(SSE_MAX_PATH);
  char *Path=sPath.Text;
#else
  char Path[MAX_PATH];
#endif
  ZeroMemory(Path,SSE_MAX_PATH);
  ...
  case WM_CONTEXTMENU:
  {
    ...
    if(Target)
    {
      ...
      { // Folder
        ...
        AppendMenu(Menu,MF_BYPOSITION|MF_STRING,IDBase+3,
          T("&Open Folder In Explorer"));
        if(!Root)
          AppendMenu(Menu,MF_BYPOSITION|MF_SEPARATOR,0,NULL);
      }
      if(!Root)
  ...
  case WM_COMMAND:
    ...
    if(LOWORD(wPar)>=IDBase && LOWORD(wPar)<IDBase+100)
    {
      ...
      case 2:
      {
        ...
        NewItem(T("New Folder"),Parent,false);
        break;
      }
      case 3:
...
LRESULT CALLBACK DirectoryTree::TVWndProc(HWND Win,UINT Mess,WPARAM wPar,
...
#if defined(SSE_GUI_KBD)
  case WM_KEYUP:
    if(wPar!=VK_SPACE /*&& !(wPar==VK_RETURN&&OptionBox.Handle
      &&OptionBox.Page==PAGE_CONFIG)*/) // not for configs //bad idea, triggers on new name
      break;
#endif
  ...
  case WM_RBUTTONDOWN:case WM_RBUTTONDBLCLK:
  {
    ...
    HTREEITEM Item=NULL;
    DT_TEST_STAGE(6) {
      hti.pt.x=GET_X_LPARAM(lPar);
      hti.pt.y=GET_Y_LPARAM(lPar);
#if defined(SSE_GUI_KBD)
      if(Mess==WM_KEYUP)
      {
        switch(wPar) {
        ...
        case VK_RETURN:
          Mess=WM_LBUTTONDBLCLK; // works but there's a bell
          break;
        }
      }
      else
      ...
      if(Mess==WM_LBUTTONDBLCLK) // load config // steem dependency!
      {
        if(OptionBox.Page==PAGE_CONFIG)
        {
          SendMessage(OptionBox.Handle,WM_COMMAND,MAKELONG(IDC_LOADCONFIG,BN_CLICKED),0);
          return 0;//?
        }
      }
    ...
    DT_TEST_STAGE(7) {
        ...
        &&(hti.flags & TVHT_ONITEMICON))
      {
        ...
        if(Type>0)
        {
          ...
          for(;;)
          {
            if(t>=This->FileMasksESL.NumStrings)
              t=1;
            if(t==Type) 
              break;
...
void DirectoryTree::DragEnd(bool Cancel) {
  ...
  if(DropPos==DT_DROP_ASCHILD && !Cancel && Dest)
  {
    bool Copy=false,SameFol=false;
    if(DragButton==2)
    {
      MenuItem=NULL;
      HMENU Menu=CreatePopupMenu();
      ...
      DestroyMenu(Menu);
      MSG mess;
      while(PeekMessage(&mess,hParent,WM_COMMAND,WM_COMMAND,PM_REMOVE))
...
HTREEITEM DirectoryTree::MoveItems(HTREEITEM Item,HTREEITEM NewParent,
  ...
  else
  {
    ...
    SHFILEOPSTRUCT fos;
    fos.hwnd=HWND(PopupParent?PopupParent:hParent);
    fos.wFunc=(Copy) ? FO_COPY : FO_MOVE;
    fos.pFrom=From;
    fos.pTo=Dest;
  ...
  if(OldParent && OldParent!=RootItem && !Copy)
  {
