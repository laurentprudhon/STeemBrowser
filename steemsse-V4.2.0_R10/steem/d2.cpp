...
cpu.cpp used to be (not anymore since v4), but rather than performing the
operations it simply generates a disassembly.
The only functions of interest to the outside program are disa_d2 (see
bottom of file) to disassemble an instruction and d2_routines_init to
...
DWORD d2_peekinvalid;
EasyStr d2_src,d2_dest,d2_command,d2_pc_rel_ex;
WORD d2_ap;
int d2_n_movem_regs;

char str_d[8][3]={"d0","d1","d2","d3","d4","d5","d6","d7"};
char str_a[8][3]={"a0","a1","a2","a3","a4","a5","a6","a7"};
char str_a_brackets[8][5]={"(a0)","(a1)","(a2)","(a3)","(a4)","(a5)","(a6)","(a7)"};

char str_pc[3]="pc";
char str_pc_rel[5]="(pc)";
char str_sp[6]="sp=a7";
...
void debug_change_upper() {
  for(int i=0;i<8;i++)
  {
    ...
    else
    {
      strlwr(str_d[i]);
      strlwr(str_a[i]);
      strlwr(str_a_brackets[i]);
      strlwr(str_pc);
      strlwr(str_pc_rel);
      strlwr(str_sp);
    }
  }
  SetWindowText(GetDlgItem(trace_window_handle,2001),debug_uppercase_disa?
    "SR before":"sr before");
  SetWindowText(GetDlgItem(trace_window_handle,2002),debug_uppercase_disa?
    "SR after":"sr after");
  set_up_reg_browser();
}
...
void set_up_reg_browser() {
  char ttt[200];
  int n=1;
  strcpy(reg_browser_entry_name[0],str_pc);
  reg_browser_entry_pointer[0]=&pc;
  for(int m=0;m<16;m++)
  {
    ttt[0]=(debug_uppercase_disa)?"DA"[int((m&8)?1:0)]:"da"[int((m&8)?1:0)];
    ttt[1]=(char)('0'+(m&7));
    ttt[2]='\0';
    strcpy(reg_browser_entry_name[n],ttt);
    reg_browser_entry_pointer[n]=(DWORD*)&(Cpu.r[m]);
    n++;
  }
  strcpy(reg_browser_entry_name[n],str_sp);
  reg_browser_entry_pointer[n]=(DWORD*)&(Cpu.r[15]);
  n++;
...
#if 0
//  return EasyStr("$")+HEXSl(a,4);
  if(a>-100&&a<100)return EasyStr((short)a);
  else
#endif
  if(a>0)return EasyStr("+$")+HEXS(a);
...
EasyStr d2_iriwo() {
  EasyStr et;
  if(((signed char)LOBYTE(d2_ap))<0)
    et=EasyStr("-$")+itoa((signed char)-LOBYTE(d2_ap),d2_t_buf,16);
  else
    et=EasyStr("+$")+itoa((signed char)LOBYTE(d2_ap),d2_t_buf,16);
...
INT_PTR d2_calc_iriwo(MEM_ADDRESS aaa) {
//  EasyStr(itoa((signed char)LOBYTE(d2_ap),d2_t_buf,10))+
//        "("+D2_aM+",r"+itoa((d2_ap>>12),d2_t_buf,10)+"."+
//        (BTSTb(d2_ap)?"L":"W")+")"
  MEM_ADDRESS ad=aaa;
  if(BTSTb(d2_ap))ad+=(LONG)Cpu.r[d2_ap>>12];
  else ad+=(signed short)LOWORD(Cpu.r[d2_ap>>12]);
  ad+=(signed char)LOBYTE(d2_ap);

  trace_add_entry("source offset register: ",reg_name(d2_ap>>12),TDE_BEFORE,
    true,int(BTSTb(d2_ap)?4:2),(MEM_ADDRESS*)&Cpu.r[d2_ap>>12]);
  trace_add_entry("immediate offset: ","",TDE_BEFORE,false,1,(MEM_ADDRESS*)
    (dpc+1));
...
BYTE d2_fetchB() {
  ...
  BUS_MASK=BUS_MASK_FETCH;
  BYTE x=(BYTE)d2_peek((MEM_ADDRESS)dpc+1);
...
WORD d2_fetchW() {
  BYTE old_bus_mask=BUS_MASK;
  BUS_MASK=BUS_MASK_FETCH;
  WORD x=d2_dpeek((MEM_ADDRESS)dpc);
  BUS_MASK=old_bus_mask;
  return x;
}
...
DWORD d2_fetchL() {
  BYTE old_bus_mask=BUS_MASK;
  BUS_MASK=BUS_MASK_FETCH;
  DWORD x=d2_lpeek((MEM_ADDRESS)dpc);
  BUS_MASK=old_bus_mask;
  return x;
}
...
BYTE d2_peek(MEM_ADDRESS ad) {
  BYTE x=0; // default
  d2_peekinvalid=0; //all valid
  BYTE old_bus_mask=BUS_MASK;
  BUS_MASK=(ad&1) ? (BUS_MASK_ACCESS|BUS_MASK_LOBYTE)
    : (BUS_MASK_ACCESS|BUS_MASK_HIBYTE);
  if(ad>=himem||Mmu.Confused)
  {
    ad&=0xffffff;
    if(ad>=MEM_IO_BASE)
    {
      BYTE old_mode=stem_runmode;
      stem_runmode=STEM_MODE_INSPECT;
      TRY_M68K_EXCEPTION
        x=io_read_b(ad);
      CATCH_M68K_EXCEPTION
        d2_peekinvalid=0xff; //byte invalid
      END_M68K_EXCEPTION
      stem_runmode=old_mode;
    }
    else if(ad>=Glue.cartbase)
    {
      if(ad>=0xfc0000)
      {
        if(tos_high && ad<(0xfc0000+192*1024))
          x=ROM_PEEK(ad-rom_addr);
        else 
        { 
          d2_peekinvalid=0xff;
          return 0; 
      ...
      else if(cart && ad<Glue.cartend)
        x=CART_PEEK(ad-Glue.cartbase);
    }
    else if(ad>=rom_addr)
    {
    ...
    else if(Mmu.Confused)
      x=mmu_confused_peek(ad,false);
  }
  else
    x=PEEK(ad);
//  d2_peekinvalid=0xff; // could be?
  BUS_MASK=old_bus_mask;
  return x;
}
...
*/
  ...
  if(ad>=mem_len)
  {
    ...
    if(ad>=MEM_IO_BASE)
    {
      ...
      stem_runmode=STEM_MODE_INSPECT;
      TRY_M68K_EXCEPTION
        x=io_read(ad); // in one go
      CATCH_M68K_EXCEPTION
        d2_peekinvalid|=0xffff; //word invalid (seems correct, ST won't peek byte by byte)
      END_M68K_EXCEPTION
      stem_runmode=old_mode;
    }
    else if(ad>=Glue.cartbase)
    {
      ...
      {
        if(tos_high && ad<(0xfc0000+192*1024))
          x=ROM_DPEEK(ad-rom_addr);
        else
          d2_peekinvalid=0xffff;
      }
      else if(cart && ad<Glue.cartend)
        x=CART_DPEEK(ad-Glue.cartbase);
    }
    else if(ad>=rom_addr)
    {
      if(ad<=(0xe00000+256*1024))      
      ...
      x=mmu_confused_dpeek(ad,false);
  }
  else
    x=DPEEK(ad);
//  d2_peekinvalid=0xffff; // could be?
  BUS_MASK=old_bus_mask;
  return x;
}
...
DWORD d2_lpeek(MEM_ADDRESS ad) {
  ...
  if(ad>=mem_len)
  {
    ...
    if(ad>=MEM_IO_BASE)
    {
      ...
      for(int nn=0;nn<2;nn++)
      {
        ...
        TRY_M68K_EXCEPTION
          x|=io_read(ad+nn);
        CATCH_M68K_EXCEPTION
          d2_peekinvalid|=0xffff; //word invalid
        END_M68K_EXCEPTION
      }
      stem_runmode=old_mode;
    }
    else if(ad>=Glue.cartbase)
    {
      ...
      {
        if(tos_high && ad<(0xfc0000+192*1024))
          x=ROM_LPEEK(ad-rom_addr);
        else
          d2_peekinvalid=0xffffffff;
      }
      else if(cart && ad<Glue.cartend)
        x=CART_LPEEK(ad-Glue.cartbase);
    }
    else if(ad>=rom_addr)
    {
      if(ad<=(0xe00000+256*1024))      
      ...
      x=mmu_confused_lpeek(ad,false);
  }
  else
    x=LPEEK(ad);
//   d2_peekinvalid=0xffffffff; // could be?
  BUS_MASK=old_bus_mask;
  return x;
}


// return false if bus/addr error
...
*/
  ...
  if(ad>=mem_len)
  {
    if(ad>=MEM_IO_BASE)
    {
      bool Ret=true;
      BYTE old_bus_mask=BUS_MASK;
      BUS_MASK=BUS_MASK_ACCESS|BUS_MASK_WORD|BUS_MASK_WRITE;
      TRY_M68K_EXCEPTION
        io_write_w(ad,val);
      CATCH_M68K_EXCEPTION
        Ret=0;
...
bool d2_lpoke(MEM_ADDRESS ad,LONG val) {
  ...
  if(ad>=mem_len)
  {
    if(ad>=MEM_IO_BASE)
    {
      ...
      BUS_MASK|=BUS_MASK_ACCESS|BUS_MASK_WORD|BUS_MASK_WRITE;
      TRY_M68K_EXCEPTION
        io_write_w(ad,val>>16);
        io_write_w(ad+2,val&0xffff);
      CATCH_M68K_EXCEPTION
        Ret=0;
      END_M68K_EXCEPTION
      BUS_MASK=old_bus_mask;
...
void d2_unrecognised() {
  ...
  d2_src_l="";
  d2_dest="";
  d2_command=EasyStr("dc.w $")+itoa(IRD,d2_t_buf,16);
...
EasyStr d2_get_movem_regs(bool backwards) {
  EasyStr dt="";
  int regs=d2_dpeek((MEM_ADDRESS)dpc);dpc+=2;
  bool had_one=false;
  d2_n_movem_regs=0;
  int msk=1,old_msk=0;
  if(backwards)
    msk=0x8000;
  for(int da=0;da<=1;da++) // 0 data register 1 address register
  {
    int mn=999;
...
//        dt+=EasyStr("[")+reg_name(da*8+reg)+"]";
      ...
      else
      {
        if(mn<reg-1)
        {
          ...
          //dt+=itoa((reg-1),d2_t_buf,10);
          dt+=reg_name(da*8+(reg-1));
          mn=999;had_one=true;
        }
        else if(mn==reg-1)
        {
    ...
    if(regs&old_msk)
    {
      if(mn<7)
      {
        ...
        dt+=EasyStr(reg_name(da*8+mn))+"-"+reg_name(da*8+7);
        had_one=true;
      }
      else if(mn==7)
      {
  ...
  if(d2_trace)
  {
    ...
    for(int rn=0;rn<16;rn++)
    {
      ...
      {
        d2_n_movem_regs++;
        trace_add_entry("register in movem: ",reg_name(rn),TDE_BEFORE|TDE_AFTER,true,4,(MEM_ADDRESS*)&Cpu.r[rn]);
      }
      if(backwards)
        msk>>=1;
      else
...
EasyStr d2_effective_address() {
  ...
  switch(IRD&BITS_543) {
  case BITS_543_010:
    trace_add_entry("source register: ",reg_name(PARAM_M+8),TDE_BEFORE,true,
      4,(MEM_ADDRESS*)&areg[PARAM_M]);
    trace_add_entry("predecrement stack pointer: ",str_sp,TDE_BEFORE|TDE_AFTER,
      true,4,(MEM_ADDRESS*)&Cpu.r[15]);
    return EasyStr("(")+D2_aM+")";
  case BITS_543_101:
    trace_add_entry("immediate offset: ","",TDE_BEFORE,false,2,
      (MEM_ADDRESS*)dpc);
    trace_add_entry("source register: ",reg_name(PARAM_M+8),TDE_BEFORE,true,4,
      (MEM_ADDRESS*)&areg[PARAM_M]);
    trace_add_entry("predecrement stack pointer: ",str_sp,TDE_BEFORE|TDE_AFTER,
      true,4,(MEM_ADDRESS*)&Cpu.r[15]);
  ...
  case BITS_543_110:
    d2_ap=d2_fetchW();
    ret=D2_IRIWO;
    trace_add_entry("source address register: ",reg_name(PARAM_M+8),TDE_BEFORE,
      true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
    trace_add_entry("source offset register: ",reg_name(d2_ap>>12),TDE_BEFORE,
      true,
      int(BTSTb(d2_ap)?4:2),(MEM_ADDRESS*)&Cpu.r[d2_ap>>12]);
    trace_add_entry("immediate offset: ","",TDE_BEFORE,false,1,
      (MEM_ADDRESS*)(dpc+1));
    trace_add_entry("predecrement stack pointer: ",str_sp,TDE_BEFORE|TDE_AFTER,
      true,4,
      (MEM_ADDRESS*)&Cpu.r[15]);
    dpc+=2;
    return ret;
  case BITS_543_111:
    switch(IRD&0x7) {
    case 0:
      ...
      trace_add_entry("effective absolute address: ",d2_src_w.c_str(),
        TDE_BEFORE,false,2,(MEM_ADDRESS*)dpc);
      trace_add_entry("predecrement stack pointer: ",str_sp,TDE_BEFORE
        |TDE_AFTER,true,4,(MEM_ADDRESS*)&Cpu.r[15]);
      dpc+=2;
      return ret;
    case 1:
      ret=EasyStr("$")+itoa(d2_fetchL(),d2_t_buf,16);
      trace_add_entry("effective absolute address: ",d2_src_w.c_str(),
        TDE_BEFORE,false,4,(MEM_ADDRESS*)dpc);
      trace_add_entry("predecrement stack pointer: ",str_sp,TDE_BEFORE
        |TDE_AFTER,true,4,(MEM_ADDRESS*)&Cpu.r[15]);
      dpc+=4;
      return ret;
    case 2:
      ...
      d2_pc_rel_ex+=EasyStr(" {$")+HEXSl((MEM_ADDRESS)dpc+(signed short)d2_fetchW(),6)+"}";
      trace_add_entry("immediate offset: ","",TDE_BEFORE,false,2,
        (MEM_ADDRESS*)dpc);
      trace_add_entry("predecrement stack pointer: ",str_sp,TDE_BEFORE
        |TDE_AFTER,true,4,(MEM_ADDRESS*)&Cpu.r[15]);
...
void d2_get_source_000_b() {
  d2_src_b=D2_dM;
  trace_add_entry("source register: ",reg_name(PARAM_M),TDE_BEFORE,true,1,(MEM_ADDRESS*)&Cpu.r[PARAM_M]);
}


void d2_get_source_000_w() {
  d2_src_w=D2_dM;
  trace_add_entry("source register: ",reg_name(PARAM_M),TDE_BEFORE,true,2,(MEM_ADDRESS*)&Cpu.r[PARAM_M]);
}


void d2_get_source_000_l() {
  d2_src_l=D2_dM;
  trace_add_entry("source register: ",reg_name(PARAM_M),TDE_BEFORE,true,4,(MEM_ADDRESS*)&Cpu.r[PARAM_M]);
}


void d2_get_source_001_b() {
  d2_src_b=D2_aM;
  trace_add_entry("source register: ",reg_name(8+PARAM_M),TDE_BEFORE,true,1,(MEM_ADDRESS*)&areg[PARAM_M]);
}


void d2_get_source_001_w() {
  d2_src_w=D2_aM;
  trace_add_entry("source register: ",reg_name(8+PARAM_M),TDE_BEFORE,true,2,(MEM_ADDRESS*)&areg[PARAM_M]);
}


void d2_get_source_001_l() {
  d2_src_l=D2_aM;
  trace_add_entry("source register: ",reg_name(8+PARAM_M),TDE_BEFORE,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
}


void d2_get_source_010_b() {
  d2_src_b=D2_BRACKETS_aM;
  trace_add_entry("source memory: ",d2_src_b.c_str(),TDE_BEFORE,false,1,
    (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]);
}


void d2_get_source_010_w() {
  d2_src_w=D2_BRACKETS_aM;
  trace_add_entry("source memory: ",d2_src_w.c_str(),TDE_BEFORE,false,2,
    (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]);
}


void d2_get_source_010_l() {
  d2_src_l=D2_BRACKETS_aM;
  trace_add_entry("source memory: ",d2_src_l.c_str(),TDE_BEFORE,false,4,
    (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]);
}


void d2_get_source_011_b() {
  d2_src_b=D2_BRACKETS_aM+"+";
  trace_add_entry("source memory: ",d2_src_b.c_str(),TDE_BEFORE,false,1,
    (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]);
  trace_add_entry("postincrement register: ",reg_name(8+PARAM_M),TDE_BEFORE
    |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
}


void d2_get_source_011_w() {
  d2_src_w=D2_BRACKETS_aM+"+";
  trace_add_entry("source memory: ",d2_src_w.c_str(),TDE_BEFORE,false,2,
    (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]);
  trace_add_entry("postincrement register: ",reg_name(8+PARAM_M),TDE_BEFORE
...
void d2_get_source_100_b() {
  d2_src_b=EasyStr("-")+D2_BRACKETS_aM;
  trace_add_entry("source memory: ",d2_src_b.c_str(),TDE_BEFORE,false,1,
    (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]-1);
  trace_add_entry("predecrement register: ",reg_name(8+PARAM_M),TDE_BEFORE
    |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
}


void d2_get_source_100_w() {
  d2_src_w=EasyStr("-")+D2_BRACKETS_aM;
  trace_add_entry("source memory: ",d2_src_w.c_str(),TDE_BEFORE,false,2,
    (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]-2);
  trace_add_entry("predecrement register: ",reg_name(8+PARAM_M),TDE_BEFORE
    |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
}


void d2_get_source_100_l() {
  d2_src_l=EasyStr("-")+D2_BRACKETS_aM;
  trace_add_entry("source memory: ",d2_src_l.c_str(),TDE_BEFORE,false,2,
    (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]-4);
  trace_add_entry("predecrement register: ",reg_name(8+PARAM_M),TDE_BEFORE
    |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
}
...
void d2_get_source_101_b() {
  ...
  trace_add_entry("immediate offset: ","",TDE_BEFORE,false,2,
    (MEM_ADDRESS*)dpc);
  trace_add_entry("source memory: ",d2_src_b.c_str(),TDE_BEFORE,false,1,
    (MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]+(signed short)d2_fetchW()));
  dpc+=2;
}
...
void d2_get_source_110_b() {
  ...
  trace_add_entry("source address register: ",reg_name(PARAM_M+8),TDE_BEFORE,
    true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
  trace_add_entry("source memory: ",d2_src_b.c_str(),TDE_BEFORE,false,1,
    (MEM_ADDRESS*)d2_calc_iriwo(areg[PARAM_M]));
  dpc+=2;
}
...
void d2_get_source_111_b() {
  ...
  case 0:
    ...
    trace_add_entry("source memory: ",d2_src_b.c_str(),TDE_BEFORE,false,1,
      (MEM_ADDRESS*)(INT_PTR)(0xffffff&(
      (LONG)((signed short)d2_fetchW()))));
    dpc+=2;
    break;
  case 1:
    d2_src_b=EasyStr("$")+itoa(d2_fetchL(),d2_t_buf,16);
    trace_add_entry("source memory: ",d2_src_b.c_str(),TDE_BEFORE,false,1,
      (MEM_ADDRESS*)(INT_PTR)(0xffffff&(d2_fetchL())));
    dpc+=4;
    break;
  case 2:
    ...
    d2_pc_rel_ex+=EasyStr(" {$")+HEXSl((MEM_ADDRESS)dpc+(signed short)d2_fetchW(),6)+"}";
    trace_add_entry("source memory: ",d2_src_b.c_str(),TDE_BEFORE,false,1,
      (MEM_ADDRESS*)(INT_PTR)(D2_PC_RELATIVE_PC+(signed short)d2_fetchW()));
    dpc+=2;
    break;
  case 3:
    d2_ap=d2_fetchW();
    ...
    trace_add_entry("source memory: ",d2_src_b.c_str(),TDE_BEFORE,false,1,
      (MEM_ADDRESS*)d2_calc_iriwo(D2_PC_RELATIVE_PC));
    dpc+=2;
    break;       //what do bits 8,9,a  of extra word do?  (not always 0)
  case 4:
//      ap=d2_fetchL();dpc+=4;
...
void d2_get_source_111_l() {
  ...
  case 0:
    ...
    {
      trace_add_entry("source memory: ",d2_src_l.c_str(),TDE_BEFORE,false,4,
        (MEM_ADDRESS*)(INT_PTR)(0xffffff&((LONG)((signed short)d2_fetchW()))));
    }
    dpc+=2;
    break;
  case 1:
  ...
  case 2:
    ...
    d2_pc_rel_ex+=EasyStr(" {$")+HEXSl((MEM_ADDRESS)dpc+(signed short)d2_fetchW(),6)+"}";
    trace_add_entry("source memory: ",d2_src_w.c_str(),TDE_BEFORE,false,4,
      (MEM_ADDRESS*)(INT_PTR)(D2_PC_RELATIVE_PC+(signed short)d2_fetchW()));
    dpc+=2;
    break;
  case 3:
...
//      ap=d2_fetchL();dpc+=4;
    d2_src_l=EasyStr("#$")+itoa(d2_fetchL(),d2_t_buf,16);
    trace_add_entry("source immediate: ",d2_src_l.c_str(),TDE_BEFORE,false,
      4,(MEM_ADDRESS*)dpc);
    dpc+=4;
    break;
  default:
...
void d2_get_dest_000_b() {
  d2_dest=D2_dM;
  trace_add_entry("dest register: ",reg_name(PARAM_M),TDE_BEFORE|TDE_AFTER,
    true,1,(MEM_ADDRESS*)&Cpu.r[PARAM_M]);
}


void d2_get_dest_000_w() {
  d2_dest=D2_dM;
  trace_add_entry("dest register: ",reg_name(PARAM_M),TDE_BEFORE|TDE_AFTER,
    true,2,(MEM_ADDRESS*)&Cpu.r[PARAM_M]);
}


void d2_get_dest_000_l() {
  d2_dest=D2_dM;
  trace_add_entry("dest register: ",reg_name(PARAM_M),TDE_BEFORE|TDE_AFTER,
    true,4,(MEM_ADDRESS*)&Cpu.r[PARAM_M]);
}


void d2_get_dest_001_b() {
  d2_dest=D2_aM;
  trace_add_entry("dest register: ",reg_name(8+PARAM_M),TDE_BEFORE|TDE_AFTER,
    true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
}


void d2_get_dest_001_w() {
  d2_dest=D2_aM;
  trace_add_entry("dest register: ",reg_name(8+PARAM_M),TDE_BEFORE|TDE_AFTER,
    true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
}
...
void d2_get_dest_010_b() {
  d2_dest=D2_BRACKETS_aM;
  trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
    1,(MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]);
}


void d2_get_dest_010_w() {
  d2_dest=D2_BRACKETS_aM;
  trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,2,
    (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]);
}


void d2_get_dest_010_l() {
  d2_dest=D2_BRACKETS_aM;
  trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,4,
    (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]);
}


void d2_get_dest_011_b() {
  d2_dest=D2_BRACKETS_aM+"+";
  trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,1,
    (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]);
  trace_add_entry("postincrement register: ",reg_name(8+PARAM_M),TDE_BEFORE
    |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
}
...
void d2_get_dest_011_w() {
  d2_dest=D2_BRACKETS_aM+"+";
  trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
    2,(MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]);
  trace_add_entry("postincrement register: ",reg_name(8+PARAM_M),TDE_BEFORE
    |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
}


void d2_get_dest_011_l() {
  d2_dest=D2_BRACKETS_aM+"+";
  trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,4,
    (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]);
  trace_add_entry("postincrement register: ",reg_name(8+PARAM_M),TDE_BEFORE
...
void d2_get_dest_101_b() {
  ...
  trace_add_entry("immediate offset: ","",TDE_BEFORE,false,2,(MEM_ADDRESS*)dpc);
  trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,1,
    (MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]+(signed short)d2_fetchW()));
  dpc+=2;
}
...
void d2_get_dest_110_b() {
  d2_ap=d2_fetchW();
  d2_dest=D2_IRIWO;
  trace_add_entry("dest base address register: ",reg_name(PARAM_M+8),TDE_BEFORE,
    true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
  trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,1,
    (MEM_ADDRESS*)d2_calc_iriwo(areg[PARAM_M]));
  dpc+=2;
...
void d2_get_dest_111_b() {
  ...
  case 0:
    ...
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      1,(MEM_ADDRESS*)(INT_PTR)(0xffffff&
      ((LONG)((signed short)d2_fetchW()))));
    dpc+=2;
    break;
  case 1:
    ...
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      1,(MEM_ADDRESS*)(INT_PTR)(0xffffff&(d2_fetchL())));
    dpc+=4;
    break;
  default:
    d2_unrecognised();
...
void d2_get_dest_111_w() {
  ...
  case 0:
    d2_dest=EasyStr("$")+itoa(d2_fetchW(),d2_t_buf,16)+".W";
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      2,(MEM_ADDRESS*)(INT_PTR)(0xffffff&
      ((LONG)((signed short)d2_fetchW()))));
    dpc+=2;
    break;
  case 1:
    d2_dest=EasyStr("$")+itoa(d2_fetchL(),d2_t_buf,16);
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      2,(MEM_ADDRESS*)(INT_PTR)(0xffffff&(d2_fetchL())));
    dpc+=4;
    break;
  default:
    d2_unrecognised();
...
void d2_get_dest_111_l() {
  ...
  case 0:
    d2_dest=EasyStr("$")+itoa(d2_fetchW(),d2_t_buf,16)+".W";
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      4,(MEM_ADDRESS*)(INT_PTR)(0xffffff&
      ((LONG)((signed short)d2_fetchW()))));
    dpc+=2;
    break;
  case 1:
    d2_dest=EasyStr("$")+itoa(d2_fetchL(),d2_t_buf,16);
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      4,(MEM_ADDRESS*)(INT_PTR)(0xffffff&(d2_fetchL())));
    dpc+=4;
    break;
  default:
    d2_unrecognised();
...
void d2_get_dest_111_b_with_sr() {
  ...
  case 0:
    d2_dest=EasyStr("$")+itoa(d2_fetchW(),d2_t_buf,16)+".W";
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      1,(MEM_ADDRESS*)(INT_PTR)(0xffffff&((LONG)((signed short)d2_fetchW()))));
    dpc+=2;
    break;
  case 1:
    d2_dest=EasyStr("$")+itoa(d2_fetchL(),d2_t_buf,16);
...
void d2_get_dest_111_w_with_sr() {
  ...
  case 0:
    d2_dest=EasyStr("$")+itoa(d2_fetchW(),d2_t_buf,16)+".W";
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      2,(MEM_ADDRESS*)(INT_PTR)(0xffffff&((LONG)((signed short)d2_fetchW()))));
    dpc+=2;
    break;
  case 1:
    d2_dest=EasyStr("$")+itoa(d2_fetchL(),d2_t_buf,16);
...
void                              d2_btst() {
  ...
  {
    d2_dest=D2_dM;
    trace_add_entry("btst register: ",reg_name(PARAM_M),TDE_BEFORE,true,4,(MEM_ADDRESS*)&Cpu.r[PARAM_M]);
    //length = .l
  }
  else if(DEST_IS_ADDRESS_REGISTER)
  {
    d2_unrecognised();
  }
  else
...
void                              d2_movem_w_from_regs_or_ext_w() {
  ...
  case BITS_543_010:
    ...
    d2_dest=D2_BRACKETS_aM;
    trace_add_movem_block("dest memory: ",PARAM_M,TDE_BEFORE|TDE_AFTER,2,
      areg[PARAM_M],d2_n_movem_regs);
    break;
  case BITS_543_100:
    d2_command="movem.w";
    ...
    d2_dest=EasyStr("-")+D2_BRACKETS_aM;
    trace_add_movem_block("dest memory: ",PARAM_M,TDE_BEFORE|TDE_AFTER,2,
      areg[PARAM_M]-2*d2_n_movem_regs,d2_n_movem_regs);
    trace_add_entry("predecrement address register: ",reg_name(8+PARAM_M),
      TDE_BEFORE|TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
    break;
  case BITS_543_101:
    d2_command="movem.w";
    d2_src_w=d2_get_movem_regs(false);
    ...
    trace_add_entry("immediate offset: ","",TDE_BEFORE,false,2,(MEM_ADDRESS*)dpc);
    trace_add_movem_block("dest memory: ",PARAM_M,TDE_BEFORE|TDE_AFTER,2,
      areg[PARAM_M]+(signed short)d2_fetchW(),d2_n_movem_regs);
    dpc+=2;
    break;
  case BITS_543_110:
    d2_command="movem.w";
    ...
    d2_ap=d2_fetchW();
    d2_dest=D2_IRIWO;
    trace_add_entry("dest address register: ",reg_name(PARAM_M+8),TDE_BEFORE,
      true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
    trace_add_movem_block("dest memory: ",PARAM_M,TDE_BEFORE|TDE_AFTER,2,
      (MEM_ADDRESS)d2_calc_iriwo(areg[PARAM_M]),d2_n_movem_regs);
    dpc+=2;
    break;
  case BITS_543_111:
    d2_command="movem.w";
    ...
    case 0:
      d2_dest=EasyStr("$")+itoa(d2_fetchW(),d2_t_buf,16)+".W";
      trace_add_movem_block("dest memory: ",-1,TDE_BEFORE|TDE_AFTER,2,
        0xffffff&(DWORD)((LONG)((signed short)d2_fetchW())),
        d2_n_movem_regs);
      dpc+=2;
      break;
    case 1:
      d2_dest=EasyStr("$")+itoa(d2_fetchL(),d2_t_buf,16);
      trace_add_movem_block("dest memory: ",-1,TDE_BEFORE|TDE_AFTER,2,
        0xffffff&d2_fetchL(),d2_n_movem_regs);
      dpc+=4;
      break;
    default:
      d2_unrecognised();
...
void                              d2_movem_l_from_regs_or_ext_l() {
  ...
  case BITS_543_010:
    ...
    d2_dest=D2_BRACKETS_aM;
    trace_add_movem_block("dest memory: ",PARAM_M,TDE_BEFORE|TDE_AFTER,4,
      areg[PARAM_M],d2_n_movem_regs);
    break;
  case BITS_543_100:
    d2_command="movem.l";
    ...
    d2_dest=EasyStr("-")+D2_BRACKETS_aM;
    trace_add_movem_block("dest memory: ",PARAM_M,TDE_BEFORE|TDE_AFTER,4,
      areg[PARAM_M]-4*d2_n_movem_regs,d2_n_movem_regs);
    trace_add_entry("predecrement address register: ",reg_name(8+PARAM_M),
      TDE_BEFORE|TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
    break;
  case BITS_543_101:
  ...
  case BITS_543_111:
    ...
    case 0:
      d2_dest=EasyStr("$")+itoa(d2_fetchW(),d2_t_buf,16)+".W";
      trace_add_movem_block("dest memory: ",-1,TDE_BEFORE|TDE_AFTER,4,
        0xffffff&(DWORD)((LONG)((signed short)d2_fetchW())),
        d2_n_movem_regs);
      dpc+=2;
      break;
...
void                              d2_movem_l_to_regs() {
  ...
  case BITS_543_010:
    ...
    trace_add_movem_block("source memory: ",PARAM_M,TDE_BEFORE,4,areg[PARAM_M],
      d2_n_movem_regs);
    break;
  case BITS_543_011:
    d2_command="movem.l";
    d2_src_l=D2_BRACKETS_aM+"+";
    d2_dest=d2_get_movem_regs(false);
    trace_add_movem_block("source memory: ",PARAM_M,TDE_BEFORE|TDE_AFTER,4,
      areg[PARAM_M],d2_n_movem_regs);
    trace_add_entry("postincrement address register: ",reg_name(8+PARAM_M),
      TDE_BEFORE|TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
    break;
  case BITS_543_101:
    d2_command="movem.l";
    ...
    trace_add_entry("immediate offset: ","",TDE_BEFORE,false,2,(MEM_ADDRESS*)dpc);
    trace_add_movem_block("source memory: ",PARAM_M,TDE_BEFORE,4,areg[PARAM_M]
      +(signed short)d2_fetchW(),d2_n_movem_regs);
    dpc+=2;
    break;
  case BITS_543_110:
    d2_command="movem.l";
    ...
    trace_add_entry("source address register: ",reg_name(PARAM_M+8),TDE_BEFORE,
      true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
    trace_add_movem_block("source memory: ",PARAM_M,TDE_BEFORE,4,
      (MEM_ADDRESS)d2_calc_iriwo(areg[PARAM_M]),d2_n_movem_regs);
    dpc+=2;
    break;
  case BITS_543_111:
    ...
    case 0:
      d2_src_l=EasyStr("$")+itoa(d2_fetchW(),d2_t_buf,16)+".W";
      trace_add_movem_block("source memory: ",-1,TDE_BEFORE,4,
        0xffffff&(DWORD)((LONG)((signed short)d2_fetchW())),
        d2_n_movem_regs);
      dpc+=2;
      break;
    ...
    case 2:
      d2_src_l=EasyStr("+$")+itoa(d2_fetchW(),d2_t_buf,16)+"(pc)";
      d2_pc_rel_ex+=EasyStr(" {$")+HEXSl((MEM_ADDRESS)dpc
        +(signed short)d2_fetchW(),6)+"}";
//////////********************************** where is the pc-relative from????? ***************
      trace_add_movem_block("source memory : ",-1,TDE_BEFORE,4,
        D2_PC_RELATIVE_PC+(signed short)d2_fetchW(),d2_n_movem_regs);
      dpc+=2;
      break;
    case 3:
      d2_ap=d2_fetchW();
      d2_src_l=D2_IRIWO_PC;
      trace_add_movem_block("source memory : ",-1,TDE_BEFORE,4,
        (MEM_ADDRESS)d2_calc_iriwo(D2_PC_RELATIVE_PC),d2_n_movem_regs);
      dpc+=2;
      break;
    default:
      d2_unrecognised();
...
void                              d2_movem_w_to_regs() {
  ...
  case BITS_543_010:
    ...
    d2_src_w=D2_BRACKETS_aM;
    d2_dest=d2_get_movem_regs(false);
    trace_add_movem_block("source memory: ",PARAM_M,TDE_BEFORE,2,
      areg[PARAM_M],d2_n_movem_regs);
    break;
  case BITS_543_011:
    d2_command="movem.w";
    d2_src_w=D2_BRACKETS_aM+"+";
    d2_dest=d2_get_movem_regs(false);
    trace_add_movem_block("source memory: ",PARAM_M,TDE_BEFORE|TDE_AFTER,2,
      areg[PARAM_M],d2_n_movem_regs);
    trace_add_entry("postincrement address register: ",reg_name(8+PARAM_M),
      TDE_BEFORE|TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
    break;
  ...
  case BITS_543_111:
    ...
    case 0:
      d2_src_w=EasyStr("$")+itoa(d2_fetchW(),d2_t_buf,16)+".W";
      trace_add_movem_block("source memory: ",-1,TDE_BEFORE,2,
        0xffffff&(DWORD)((LONG)((signed short)d2_fetchW())),
        d2_n_movem_regs);
      dpc+=2;
      break;
...
void                              d2_jsr() {
  ...
  default:
    d2_command="jsr";
    trace_add_entry("program counter: ",str_pc,TDE_BEFORE|TDE_AFTER,true,4,
      (MEM_ADDRESS*)&pc);
    d2_src_w=d2_effective_address();
  }
}
...
void                              d2_chk() {
  ...
  else
  {
    D2_GET_SOURCE_W;
    d2_dest=D2_dN;
    trace_add_entry("dest data register: ",d2_dest.c_str(),TDE_BEFORE|
      TDE_AFTER,true,2,(MEM_ADDRESS*)&(Cpu.r[PARAM_M]));
  }
}
...
void                              d2_lea() {
  ...
  default:
    d2_command="lea";
    d2_dest=D2_aN;
    trace_add_entry("dest address register: ",reg_name(PARAM_N+8),TDE_BEFORE
      |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
    d2_src_w=d2_effective_address();
  }
}
...
void                              d2_line_4_stuff() {
  d2_jump_line_4_stuff[IRD&(BITS_543|0x7)]();
}


void                              d2_trap() {
  d2_command="trap";
  d2_src=EasyStr("#")+itoa((IRD&0xf),d2_t_buf,10);
}
...
void                              d2_link() {
  ...
  trace_add_entry("predecrement stack pointer: ",str_sp,TDE_BEFORE|TDE_AFTER,
    true,4,(MEM_ADDRESS*)&Cpu.r[15]);
  trace_add_entry("link address register: ",reg_name(PARAM_M+8),TDE_BEFORE|
    TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
...
void                              d2_unlk() {
  d2_command="unlk";
  d2_dest=D2_aM;
  trace_add_entry("stack pointer: ",str_sp,TDE_BEFORE|TDE_AFTER,true,4,
    (MEM_ADDRESS*)&Cpu.r[15]);
  trace_add_entry("link address register: ",reg_name(PARAM_M+8),TDE_BEFORE
    |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
...
void                              d2_move_to_usp() {
  d2_command="move";
  d2_src_l=D2_aM;
  trace_add_entry("source register: ",reg_name(8+PARAM_M),TDE_BEFORE,true,4,
    (MEM_ADDRESS*)&areg[PARAM_M]);
  d2_dest="usp";
  trace_add_entry("dest: ","usp",TDE_BEFORE|TDE_AFTER,true,4,
    (MEM_ADDRESS*)lpUSP);
}
...
void                              d2_move_from_usp() {
  ...
  d2_dest=D2_aM;
  trace_add_entry("dest register: ",reg_name(8+PARAM_M),TDE_BEFORE,true,4,
    (MEM_ADDRESS*)&areg[PARAM_M]);
}
...
void                              d2_stop() {
  d2_command="stop";
  d2_src_w=EasyStr("#$")+itoa(d2_fetchW(),d2_t_buf,16);
  trace_add_entry("immediate source: ","",TDE_BEFORE,false,2,(MEM_ADDRESS*)dpc);
  dpc+=2;
}
...
void                              d2_movec() {
  ...
  {
    d2_src_w=dtt;
    d2_dest=EasyStr("r")+itoa(((d2_ap&0xf000)>>12),d2_t_buf,10);
  }
  else
  {
    d2_dest=dtt;
    d2_src_w=EasyStr("r")+itoa(((d2_ap&0xf000)>>12),d2_t_buf,10);
  }
}
...
void                              d2_dbCC_or_sCC() {
  if(DEST_IS_ADDRESS_REGISTER)
  {
    ...
    d2_command+=("t\0\0ra\0hi\0ls\0cc\0cs\0ne\0eq\0vc\0vs\0pl\0mi\0ge\0lt\0gt\0le\0"
      +3*((IRD&0xf00)>>8));
    d2_src_w=D2_dM;
    trace_add_entry("counter register: ",reg_name(PARAM_M),TDE_BEFORE|TDE_AFTER,
      true,2,(MEM_ADDRESS*)&Cpu.r[PARAM_M]);
    d2_ap=d2_fetchW();
    if(d2_ap&0x8000)
    {
    ...
    trace_add_entry("immediate offset: ",d2_dest.c_str(),TDE_BEFORE,false,2,(MEM_ADDRESS*)dpc);
    d2_dest+=Str(" {$")+HEXSl((MEM_ADDRESS)dpc+(signed short)d2_ap,6)+"}";
    dpc+=2;
  }
  else
  {
...
void                              d2_or_b_to_dN() {
  d2_command="or.b";
  d2_dest=D2_dN;
  trace_add_entry("dest register: ",reg_name(PARAM_N),TDE_BEFORE|TDE_AFTER,
    true,1,(MEM_ADDRESS*)&Cpu.r[PARAM_N]);
  D2_GET_SOURCE_B;
}
...
void                             d2_or_w_to_dN() {
  ...
  d2_dest=D2_dN;
  trace_add_entry("dest register: ",reg_name(PARAM_N),TDE_BEFORE|TDE_AFTER,
    true,2,(MEM_ADDRESS*)&Cpu.r[PARAM_N]);
  D2_GET_SOURCE_W;
}
...
void                             d2_or_l_to_dN() {
  ...
  d2_dest=D2_dN;
  trace_add_entry("dest register: ",reg_name(PARAM_N),TDE_BEFORE|TDE_AFTER,
    true,4,(MEM_ADDRESS*)&Cpu.r[PARAM_N]);
  D2_GET_SOURCE_L;
}
...
void                              d2_or_b_from_dN_or_sbcd() {
  ...
  case BITS_543_000:
    d2_command="sbcd";
    d2_src_b=D2_dM;
    trace_add_entry("source register: ",reg_name(PARAM_M),TDE_BEFORE,true,1,
      (MEM_ADDRESS*)&Cpu.r[PARAM_M]);
    d2_dest=D2_dN;
    trace_add_entry("dest register: ",reg_name(PARAM_N),TDE_BEFORE|TDE_AFTER,
      true,1,(MEM_ADDRESS*)&Cpu.r[PARAM_N]);
    break;
  case BITS_543_001:
    ...
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
      false,1,(MEM_ADDRESS*)(INT_PTR)(areg[PARAM_N]-1));
    trace_add_entry("predecrement register: ",reg_name(8+PARAM_N),TDE_BEFORE
      |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
    break;
  default:
    d2_command="or.b";
    d2_src_b=D2_dN;
    trace_add_entry("source register: ",reg_name(PARAM_N),TDE_BEFORE,true,1,
      (MEM_ADDRESS*)&Cpu.r[PARAM_N]);
    D2_GET_DEST_B;
  }
}
...
void                             d2_or_w_from_dN() {
  ...
  default:
    d2_command="or.w";
    d2_src_w=D2_dN;
    trace_add_entry("source register: ",reg_name(PARAM_N),TDE_BEFORE,true,2,
      (MEM_ADDRESS*)&Cpu.r[PARAM_N]);
    D2_GET_DEST_W;
  }
}
...
void                             d2_or_l_from_dN() {
  ...
  default:
    d2_command="or.l";
    d2_src_l=D2_dN;
    trace_add_entry("source register: ",reg_name(PARAM_N),TDE_BEFORE,true,4,
      (MEM_ADDRESS*)&Cpu.r[PARAM_N]);
    D2_GET_DEST_L;
  }
}
...
void                             d2_suba_w() {
  d2_command="suba.w";
  d2_dest=D2_aN;
  trace_add_entry("dest register: ",reg_name(8+PARAM_N),TDE_BEFORE|TDE_AFTER,
    true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
  D2_GET_SOURCE_W;
}
...
void                              d2_sub_b_from_dN() {
  ...
  case BITS_543_001:
    ...
    d2_dest=EasyStr("-")+D2_BRACKETS_aN;
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      1,(MEM_ADDRESS*)(INT_PTR)(areg[PARAM_N]-1));
    trace_add_entry("predecrement register: ",reg_name(8+PARAM_N),TDE_BEFORE
      |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
    break;
  default:
...
void                              d2_sub_w_from_dN() {
  ...
  case BITS_543_000:
    d2_command="subx.w";
    d2_src_w=D2_dM;
    trace_add_entry("source register: ",reg_name(PARAM_M),TDE_BEFORE,true,2,
      (MEM_ADDRESS*)&Cpu.r[PARAM_M]);
    d2_dest=D2_dN;
    trace_add_entry("dest register: ",reg_name(PARAM_N),TDE_BEFORE|TDE_AFTER,
      true,2,(MEM_ADDRESS*)&Cpu.r[PARAM_N]);
  ...
  case BITS_543_001:
    d2_command="subx.w";
    d2_src_w=EasyStr("-")+D2_BRACKETS_aM;
    trace_add_entry("source memory: ",d2_src_b.c_str(),TDE_BEFORE,false,2,
      (MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]-2));
    trace_add_entry("predecrement register: ",reg_name(8+PARAM_M),TDE_BEFORE
      |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
    d2_dest=EasyStr("-")+D2_BRACKETS_aN;
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      2,(MEM_ADDRESS*)(INT_PTR)(areg[PARAM_N]-2));
    trace_add_entry("predecrement register: ",reg_name(8+PARAM_N),TDE_BEFORE
      |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
    break;
  default:
...
void                              d2_sub_l_from_dN() {
  ...
  case BITS_543_000:
    d2_command="subx.l";
    d2_src_l=D2_dM;
    trace_add_entry("source register: ",reg_name(PARAM_M),TDE_BEFORE,true,4,
      (MEM_ADDRESS*)&Cpu.r[PARAM_M]);
    d2_dest=D2_dN;
    trace_add_entry("dest register: ",reg_name(PARAM_N),TDE_BEFORE|TDE_AFTER,
      true,4,(MEM_ADDRESS*)&Cpu.r[PARAM_N]);
  ...
  case BITS_543_001:
    d2_command="subx.l";
    d2_src_l=EasyStr("-")+D2_BRACKETS_aM;
    trace_add_entry("source memory: ",d2_src_b.c_str(),TDE_BEFORE,false,4,
      (MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]-4));
    trace_add_entry("predecrement register: ",reg_name(8+PARAM_M),TDE_BEFORE|
      TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
    d2_dest=EasyStr("-")+D2_BRACKETS_aN;
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      4,(MEM_ADDRESS*)(INT_PTR)(areg[PARAM_N]-4));
    trace_add_entry("predecrement register: ",reg_name(8+PARAM_N),TDE_BEFORE
      |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
    break;
  default:
...
void                              d2_cmp_b() {
  d2_command="cmp.b";
  d2_dest=D2_dN;
  trace_add_entry("dest register: ",reg_name(PARAM_N),TDE_BEFORE,true,1,
    (MEM_ADDRESS*)&Cpu.r[PARAM_N]);
  D2_GET_SOURCE_B;
}
...
void                             d2_cmp_w() {
  d2_command="cmp.w";
  d2_dest=D2_dN;
  trace_add_entry("dest register: ",reg_name(PARAM_N),TDE_BEFORE,true,2,
    (MEM_ADDRESS*)&Cpu.r[PARAM_N]);
  D2_GET_SOURCE_W;
}
...
void                             d2_cmp_l() {
  d2_command="cmp.l";
  d2_dest=D2_dN;
  trace_add_entry("dest register: ",reg_name(PARAM_N),TDE_BEFORE,true,4,
    (MEM_ADDRESS*)&Cpu.r[PARAM_N]);
  D2_GET_SOURCE_L;
}
...
void                             d2_cmpa_w() {
  d2_command="cmpa.w";
  d2_dest=D2_aN;
  trace_add_entry("dest register: ",reg_name(8+PARAM_N),TDE_BEFORE,true,4,
    (MEM_ADDRESS*)&areg[PARAM_N]);
  D2_GET_SOURCE_W;
}
...
void                              d2_eor_b() {
  if(DEST_IS_ADDRESS_REGISTER)
  {
    ...
    trace_add_entry("source memory: ",d2_src_b.c_str(),TDE_BEFORE,false,1,
      (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]);
    trace_add_entry("postdecrement register: ",reg_name(8+PARAM_M),TDE_BEFORE
      |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
    d2_dest=D2_BRACKETS_aN+"+";
    trace_add_entry("dest memory: ",d2_src_b.c_str(),TDE_BEFORE,false,1,
      (MEM_ADDRESS*)(INT_PTR)areg[PARAM_N]);
    trace_add_entry("postdecrement register: ",reg_name(8+PARAM_N),TDE_BEFORE
      |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
  }
  else
  {
...
void                             d2_eor_w() {
  if(DEST_IS_ADDRESS_REGISTER)
  {
    ...
    trace_add_entry("postdecrement register: ",reg_name(8+PARAM_M),TDE_BEFORE
      |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
    d2_dest=D2_BRACKETS_aN+"+";
    trace_add_entry("dest memory: ",d2_src_w.c_str(),TDE_BEFORE,false,2,
      (MEM_ADDRESS*)(INT_PTR)areg[PARAM_M]);
    trace_add_entry("postdecrement register: ",reg_name(8+PARAM_M),TDE_BEFORE
      |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
  }
...
void                             d2_eor_l() {
  if(DEST_IS_ADDRESS_REGISTER)
  {
    ...
    trace_add_entry("postdecrement register: ",reg_name(8+PARAM_M),TDE_BEFORE
      |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
    d2_dest=D2_BRACKETS_aN+"+";
    trace_add_entry("dest memory: ",d2_src_l.c_str(),TDE_BEFORE,false,4,
      (MEM_ADDRESS*)(INT_PTR)areg[PARAM_N]);
    trace_add_entry("postdecrement register: ",reg_name(8+PARAM_N),TDE_BEFORE
      |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
  }
  else
  {
    d2_command="eor.l";
    d2_src_l=D2_dN;
    trace_add_entry("source register: ",reg_name(PARAM_N),TDE_BEFORE,true,
      4,(MEM_ADDRESS*)&Cpu.r[PARAM_N]);
    D2_GET_DEST_L;
  }
}
...
void                             d2_and_w_from_dN_or_exg_like() {
  ...
  case BITS_543_000:
    d2_command="exg";
    d2_src_l=D2_dN;
    trace_add_entry("exg register: ",reg_name(PARAM_N),TDE_BEFORE|TDE_AFTER,
      true,4,(MEM_ADDRESS*)&Cpu.r[PARAM_N]);
    d2_dest=D2_dM;
    trace_add_entry("exg register: ",reg_name(PARAM_M),TDE_BEFORE|TDE_AFTER,
      true,4,(MEM_ADDRESS*)&Cpu.r[PARAM_M]);
    break;
  case BITS_543_001:
    d2_command="exg";
    d2_src_l=D2_aN;
    trace_add_entry("exg register: ",reg_name(8+PARAM_N),TDE_BEFORE|TDE_AFTER,
      true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
    d2_dest=D2_aM;
    trace_add_entry("exg register: ",reg_name(8+PARAM_M),TDE_BEFORE|TDE_AFTER,
      true,4,(MEM_ADDRESS*)&areg[PARAM_M]);
    break;
  default:
    d2_command="and.w";
...
/*
    ...
    d2_dest=D2_dM;
    trace_add_entry("exg register: ",reg_name(PARAM_M),TDE_BEFORE|TDE_AFTER,true,4,(MEM_ADDRESS*)&Cpu.r[PARAM_M]);
*/
    d2_unrecognised();
    break;
  case BITS_543_001:
...
void                              d2_add_w_from_dN() {
  ...
  case BITS_543_000:
    d2_command="addx.w";
    d2_src_w=D2_dM;
    trace_add_entry("source register: ",reg_name(PARAM_M),TDE_BEFORE,true,
      2,(MEM_ADDRESS*)&Cpu.r[PARAM_M]);
    d2_dest=D2_dN;
    trace_add_entry("dest register: ",reg_name(PARAM_N),TDE_BEFORE|TDE_AFTER,
      true,2,(MEM_ADDRESS*)&Cpu.r[PARAM_N]);
    break;
...
void d2_0000() { //immediate stuff
  ...
  case BITS_876_100:
    if(DEST_IS_ADDRESS_REGISTER)
    {
      d2_command="movep.w";
      d2_src_w=EasyStr("$")+itoa(d2_fetchW(),d2_t_buf,16)+D2_BRACKETS_aM;
      trace_add_entry("source memory: ",d2_src_w.c_str(),TDE_BEFORE,false,1,
        (MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]+(signed short)d2_fetchW()));
      trace_add_entry("source memory: 2+",d2_src_w.c_str(),TDE_BEFORE,false,1,
        (MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]+(signed short)d2_fetchW()+2));
      dpc+=2;
      d2_dest=D2_dN;
      trace_add_entry("dest register: ",reg_name(PARAM_N),TDE_BEFORE|TDE_AFTER,
        true,2,(MEM_ADDRESS*)&Cpu.r[PARAM_N]);
  ...
  case BITS_876_101:
    if(DEST_IS_ADDRESS_REGISTER)
    {
      ...
      trace_add_entry("source memory: 2+",d2_src_w.c_str(),TDE_BEFORE,false,1,
        (MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]+(signed short)d2_fetchW()+2));
      trace_add_entry("source memory: 4+",d2_src_w.c_str(),TDE_BEFORE,false,1,
        (MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]+(signed short)d2_fetchW()+4));
      trace_add_entry("source memory: 6+",d2_src_w.c_str(),TDE_BEFORE,false,1,
        (MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]+(signed short)d2_fetchW()+6));
      dpc+=2;
      d2_dest=D2_dN;
      trace_add_entry("dest register: ",reg_name(PARAM_N),TDE_BEFORE|TDE_AFTER,
        true,4,(MEM_ADDRESS*)&Cpu.r[PARAM_N]);
  ...
  case BITS_876_110:
    if(DEST_IS_ADDRESS_REGISTER)
    {
      ...
      d2_dest=EasyStr("$")+itoa(d2_fetchW(),d2_t_buf,16)+D2_BRACKETS_aM;
      trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
        false,1,(MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]+(signed short)d2_fetchW()));
      trace_add_entry("dest memory: 2+",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
        false,1,(MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]+(signed short)d2_fetchW()+2));
      dpc+=2;
    }
    else
    {
  ...
  case BITS_876_111:
    if(DEST_IS_ADDRESS_REGISTER)
    {
      ...
      trace_add_entry("dest memory: 2+",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
        false,1,(MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]+(signed short)d2_fetchW()+2));
      trace_add_entry("dest memory: 4+",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
        false,1,(MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]+(signed short)d2_fetchW()+4));
      trace_add_entry("dest memory: 6+",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
        false,1,(MEM_ADDRESS*)(INT_PTR)(areg[PARAM_M]+(signed short)d2_fetchW()+6));
      dpc+=2;
    }
    else
    {
    ...
    break;
  default:
    d2_jump_line_0[(IRD&(BITS_876|BITS_ba9))>>6]();
  }
...
void d2_0001() {  //move.b
  ...
  case BITS_876_010:
    d2_dest=D2_BRACKETS_aN;
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
      false,1,(MEM_ADDRESS*)(INT_PTR)areg[PARAM_N]);
    break;
  case BITS_876_011:
    d2_dest=D2_BRACKETS_aN+"+";
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
      false,1,(MEM_ADDRESS*)(INT_PTR)areg[PARAM_N]);
    trace_add_entry("postincrement register: ",reg_name(8+PARAM_N),TDE_BEFORE
      |TDE_AFTER,true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
    break;
  case BITS_876_100:
    d2_dest=EasyStr("-")+D2_BRACKETS_aN;
  ...
  case BITS_876_101:
    ...
    trace_add_entry("immediate offset: ","",TDE_BEFORE,false,2,(MEM_ADDRESS*)dpc);
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
      false,1,(MEM_ADDRESS*)(INT_PTR)(areg[PARAM_N]+(signed short)d2_fetchW()));
    dpc+=2;
    break;
  case BITS_876_110:
    d2_ap=d2_fetchW();
    d2_dest=D2_IRIWO_N;
    trace_add_entry("dest base address register: ",reg_name(PARAM_N+8),
      TDE_BEFORE,true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
      false,1,(MEM_ADDRESS*)d2_calc_iriwo(areg[PARAM_N]));
    dpc+=2;
    break;
  case BITS_876_111:
    switch(IRD&BITS_ba9) {
    case BITS_ba9_000:
      d2_dest=EasyStr("$")+itoa(d2_fetchW(),d2_t_buf,16)+".w";
      trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
        false,1,(MEM_ADDRESS*)(INT_PTR)(0xffffff&((LONG)((signed short)d2_fetchW()))));
      dpc+=2;
      break;
    case BITS_ba9_001:
      d2_dest=EasyStr("$")+itoa(d2_fetchL(),d2_t_buf,16);
...
void d2_0010() {
  ...
  case BITS_876_001:
    d2_command="movea.l";
    d2_dest=D2_aN;
    trace_add_entry("dest register: ",reg_name(PARAM_N+8),TDE_BEFORE|TDE_AFTER,
      true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
    break;
  case BITS_876_010:
    d2_dest=D2_BRACKETS_aN;
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      4,(MEM_ADDRESS*)(INT_PTR)areg[PARAM_N]);
    break;
  case BITS_876_011:
    d2_dest=D2_BRACKETS_aN+"+";
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
  ...
  case BITS_876_101:
    ...
    trace_add_entry("immediate offset: ","",TDE_BEFORE,false,2,(MEM_ADDRESS*)dpc);
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      4,(MEM_ADDRESS*)(INT_PTR)(areg[PARAM_N]+(signed short)d2_fetchW()));
    dpc+=2;
    break;
  case BITS_876_110:
    d2_ap=d2_fetchW();
    ...
    trace_add_entry("dest base address register: ",reg_name(PARAM_N+8),
      TDE_BEFORE,true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
      false,4,(MEM_ADDRESS*)d2_calc_iriwo(areg[PARAM_N]));
    dpc+=2;
    break;
  case BITS_876_111:
    switch(IRD&BITS_ba9) {
    case BITS_ba9_000:
      d2_dest=EasyStr("$")+itoa(d2_fetchW(),d2_t_buf,16)+".w";
      trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
        false,4,(MEM_ADDRESS*)(INT_PTR)(0xffffff&((LONG)((signed short)d2_fetchW()))));
      dpc+=2;
      break;
    case BITS_ba9_001:
      d2_dest=EasyStr("$")+itoa(d2_fetchL(),d2_t_buf,16);
      trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
        false,4,(MEM_ADDRESS*)(INT_PTR)(0xffffff&d2_fetchL()));
      dpc+=4;
      break;
    default:
      d2_unrecognised();
...
void d2_0011() {
  ...
  case BITS_876_010:
    d2_dest=D2_BRACKETS_aN;
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      2,(MEM_ADDRESS*)(INT_PTR)areg[PARAM_N]);
    break;
  case BITS_876_011:
    d2_dest=D2_BRACKETS_aN+"+";
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
  ...
  case BITS_876_101:
    ...
    trace_add_entry("immediate offset: ","",TDE_BEFORE,false,2,(MEM_ADDRESS*)dpc);
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,false,
      2,(MEM_ADDRESS*)(INT_PTR)(areg[PARAM_N]+(signed short)d2_fetchW()));
    dpc+=2;
    break;
  case BITS_876_110:
    d2_ap=d2_fetchW();
    ...
    trace_add_entry("dest base address register: ",reg_name(PARAM_N+8),
      TDE_BEFORE,true,4,(MEM_ADDRESS*)&areg[PARAM_N]);
    trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
      false,2,(MEM_ADDRESS*)d2_calc_iriwo(areg[PARAM_N]));
    dpc+=2;
    break;
  case BITS_876_111:
    switch(IRD&BITS_ba9) {
    case BITS_ba9_000:
      d2_dest=EasyStr("$")+itoa(d2_fetchW(),d2_t_buf,16)+".w";
      trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
        false,2,(MEM_ADDRESS*)(INT_PTR)(0xffffff&((LONG)((signed short)d2_fetchW()))));
      dpc+=2;
      break;
    case BITS_ba9_001:
      d2_dest=EasyStr("$")+itoa(d2_fetchL(),d2_t_buf,16);
      trace_add_entry("dest memory: ",d2_dest.c_str(),TDE_BEFORE|TDE_AFTER,
        false,2,(MEM_ADDRESS*)(INT_PTR)(0xffffff&d2_fetchL()));
      dpc+=4;
      break;
    default:
      d2_unrecognised();
...
void d2_0100() {
  d2_jump_line_4[(IRD&(BITS_ba9|BITS_876))>>6]();
}

void d2_0101() {
  d2_jump_line_5[(IRD&BITS_876)>>6]();
}
...
void d2_0110() {  //bCC
  ...
  if(d2_ap)
  {
    ...
    if(d2_ap&0x80)
    {
      d2_dest=EasyStr("-$")+HEXS(0x100-d2_ap);
      d2_ap=(WORD)(-(0x100-d2_ap));
    }
    else
    {
      d2_dest=EasyStr("+$")+HEXS(d2_ap);
    }
    d2_dest+=Str(" {$")+HEXSl((MEM_ADDRESS)dpc+(signed short)d2_ap,6)+"}";
  }
  else
  {
    ...
    trace_add_entry("branch offset: ","",TDE_BEFORE,false,2,(MEM_ADDRESS*)dpc);
    d2_dest+=Str(" {$")+HEXSl((MEM_ADDRESS)dpc+(signed short)d2_ap,6)+"}";
    dpc+=2;
  }
}
...
void d2_0111() {  //moveq
  ...
  {
    d2_command="moveq";
    //d2_src_b=EasyStr("#")+STRS((signed char)LOBYTE(IRD));
    d2_src_b=EasyStr("#$")+HEXS(LOBYTE(IRD));
    d2_dest=D2_dN;
    trace_add_entry("dest register: ",reg_name(PARAM_N),TDE_BEFORE|TDE_AFTER,
      true,4,(MEM_ADDRESS*)&Cpu.r[PARAM_N]);
  }
...
void d2_1000() { //or, div, sbcd
  d2_jump_line_8[(IRD&BITS_876)>>6]();
}

void d2_1001() { //sub
  d2_jump_line_9[(IRD&BITS_876)>>6]();
}

void d2_1010() {  //line-a
  d2_command="line-A";
  d2_src_w=EasyStr("#$")+HEXS(IRD&0xfff);
}

void d2_1011() { //cmp, eor
  d2_jump_line_b[(IRD&BITS_876)>>6]();
}

void d2_1100() { // and, abcd, exg, mul
  d2_jump_line_c[(IRD&BITS_876)>>6]();
}

void d2_1101() {   //add
  d2_jump_line_d[(IRD&BITS_876)>>6]();
}

void d2_1110() {  //bit shift
  d2_jump_line_e[(IRD&(BITS_876|BITS_543))>>3]();
}
...
void d2_routines_init() {
  ...
  d2_jump_line_4[B6_001011]=d2_unrecognised;
  ...
  d2_jump_line_4_stuff[B6_110100]=d2_unrecognised;
  d2_jump_line_4_stuff[B6_110101]=d2_rts;
  d2_jump_line_4_stuff[B6_110110]=d2_trapv;
  d2_jump_line_4_stuff[B6_110111]=d2_rtr;
  d2_jump_line_4_stuff[B6_111010]=d2_unrecognised;
  d2_jump_line_4_stuff[B6_111011]=d2_unrecognised;
...
EasyStr disa_d2(MEM_ADDRESS new_dpc,WORD pir) {
  ...
  old_dpc=(MEM_ADDRESS)dpc;
  d2_src="";
  d2_dest="";
  d2_command="";
  d2_pc_rel_ex="";
  d2_high_nibble_jump_table[IRD>>12]();     //execute
  if(strstr(d2_command.c_str(),"dc.w")==NULL)
  {
    dt+=d2_command;
    bool src_is_empty=d2_src.IsEmpty();
    bool dest_is_empty=d2_dest.IsEmpty();
    // remove space before some .L
    if(!src_is_empty && d2_src.Lefts(2)==".l"
      || src_is_empty && ! dest_is_empty && d2_dest.Lefts(2)==".l")
      ;
    else
      dt+=" ";
    if(src_is_empty)
