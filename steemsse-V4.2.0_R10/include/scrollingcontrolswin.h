...
class ScrollControlWin {
  static LRESULT WINAPI ScrollWndProc(HWND Win,UINT Mess,WPARAM wPar,
                                      LPARAM lPar);
  static LRESULT WINAPI ControlPageWndProc(HWND Win,UINT Mess,WPARAM wPar,
                                           LPARAM lPar);
  HWND Scroller;
public:
  ScrollControlWin();
  ScrollControlWin(DWORD Style,int x,int y,int w,int h,
               HWND Parent,HMENU Id,HINSTANCE hInst,LPVOID CreateStuff=NULL);
  ...
  HWND Create(DWORD Style,int x,int y,int w,int h,HWND Parent,HMENU Id,
              HINSTANCE hInst,LPVOID CreateStuff=NULL);
  ...
  static bool ShouldPassOnMessage(UINT Mess);
};
