...
public:
  ...
  REFERENCE_TIME *Timing,*pRTiming,*pWTiming,LastTiming;
#endif
  CircularBuffer(DWORD Size=0);
  ~CircularBuffer();
  bool Create(DWORD Size);
#if defined(SSE_DIRECTMIDI)
  bool AddByte(BYTE Data,REFERENCE_TIME timing=0);
  bool AddBytes(BYTE *pData,DWORD DataLen,REFERENCE_TIME timing=0);
#else
  bool AddByte(BYTE Data),AddBytes(BYTE *pData,DWORD DataLen);
#endif
  void Reset();
  void Destroy();
