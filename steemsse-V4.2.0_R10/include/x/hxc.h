...
// apparently all this hxc stuff was made by Steem authors?
...
typedef int HXC_LISTVIEWNOTIFYPROC(hxc_listview*,int,INT_PTR);
typedef int HXC_SCROLLBARNOTIFYPROC(hxc_scrollbar*,int,INT_PTR);
typedef int HXC_BUTTONNOTIFYPROC(hxc_button*,int,int*);
typedef int HXC_DROPDOWNNOTIFYPROC(hxc_dropdown*,int,INT_PTR);
typedef int HXC_EDITNOTIFYPROC(hxc_edit*,int,INT_PTR);
typedef int HXC_SCROLLAREANOTIFYPROC(hxc_scrollarea*,int,INT_PTR);
...
typedef int HXC_TIMERPROC(void*,Window,INT_PTR);
...
private:
public:
  static bool SetProp(Display *XD,Window Win,XContext Prop,DWORD_PTR Val);
  static DWORD_PTR GetProp(Display *XD,Window Win,XContext Prop);
...
public:
  ...
  hxc_button(Display *_d,Window _p,int _x,int _y,int _w,int _h,
    LPHXC_BUTTONNOTIFYPROC np,void *o,int tp,char *txt,int i,
    DWORD bk,XFontStruct *fs=NULL)
  {
...
//  int get_required_height(int line_width); done by wordwrap!
  ...
  static int scrollbar_notify_proc(hxc_scrollbar* sb,int mess,INT_PTR i);
