...
hxc_listview::hxc_listview()
{
  ...
  clicked_in=0;
  in_combo=0;
  text_trunc_mode=LVTTM_ELLIPSIS;
                      ...
                      int w,int h,LPHXC_LISTVIEWNOTIFYPROC pass_notifyproc)
{
  ...
  if (in_combo){
    ...
    SetProp(XD,handle,cModal,-1); // Can't disable if no parent window!
                            ...
                            unsigned long bk,unsigned long fore)
{
  ...
  if (columns.NumItems){
    ...
    int col_gap=8;
    if (text_trunc_mode==LVTTM_CUT) col_gap=1;
  	char *t=coltext.Text;
	  for (int n=-1;n<columns.NumItems;n++){
      char *next_t=t+strlen(t)+1;

      bool draw_text=true;
      if (n>=0) if (columns[n]<=0) draw_text=0;
      if (t[0]==0) draw_text=0;
      if (draw_text){
        if (n>=0) x=columns[n];
        int col_wid=99999;
        // Find x of next column that is visible
        for (int c=n+1;c<columns.NumItems;c++){
          if (columns[c]>x){
            col_wid=MAX(columns[c]-x-col_gap,1);
            break;
          }
        }
        if (col_wid<99999){
          int len=(int)strlen(t);
          if (XTextWidth(font,t,len)>=col_wid){
            if (text_trunc_mode==LVTTM_ELLIPSIS){
              char *right=t+len-1;
              *(right--)='.';
              if (right>=t) *(right)='.';
            }
            while (XTextWidth(font,t,len)>=col_wid){
              char *right=t+len-1;
              *(right--)=0; len--;
              if (len<=0) break;
              if (text_trunc_mode==LVTTM_ELLIPSIS){
        ...
        if (t[0]) XDrawString(XD,text_pix,text_gc,x,ty,t,strlen(t));
      }
		  t=next_t;
		}
	}else{
    ...
    case ButtonPress:
    {
      ...
      if (This->in_combo){
        ...
        if (Press && over==real_over && (buttonmask & Button1Mask)){
          ...
          if (Press){
            if (over==real_over && This->clicked_in){
              ...
              if (mx>=0 && buttonmask) This->changesel(over);
            }
  				}else{
            if (This->dragging>-1){
        ...
        if (doubleclick){
          ...
          This->clicked_in=0; // Double click might bring up modal dialog, stopping the ButtonRelease message
