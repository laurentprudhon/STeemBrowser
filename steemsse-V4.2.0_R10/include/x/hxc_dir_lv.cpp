      ...
      if (is_link<2 || show_broken_links){
        ...
        if (S_ISDIR(s.st_mode)){
          ...
          for (int n=2;n<ext_sl.NumStrings;n++){
            ...
            if (match){
              if (notifyproc){
                int newn=notifyproc(this,DLVN_GETTYPE,(INT_PTR)notify_path.Text);
                if (newn) n=newn;
              }
...
EasyStr hxc_dir_lv::movecopylink_item(int action,EasyStr src,EasyStr dest_fol)
{
  ...
  if (action==MCL_MOVE){
    ...
    if (notifyproc){
      ...
      notifyproc(this,DLVN_CONTENTSCHANGE,(INT_PTR)&ccn);
    }
...
//---------------------------------------------------------------------------
int hxc_dir_lv::lv_notifyproc(hxc_listview *lv,int mess,INT_PTR i)
{
	...
	switch (mess){
        ...
        int ret=0;
        if (This->notifyproc) ret=This->notifyproc(This,DLVN_FOLDERCHANGE,(INT_PTR)new_fol.Text);
        if (ret==0){
          This->fol=new_fol;
    			This->refresh_fol();
          lv->changesel(0);
    ...
    case LVN_KEYPRESS:
      ...
      if (lv->sel>-1){
        if (lv->sl[lv->sel].Data[DLVD_TYPE]>0){
          ...
          if (i!=XK_Delete && i!=XK_F2 && i!=XK_Escape) //SSE
          {
            if(This->notifyproc)
              return This->notifyproc(This,DLVN_KEYPRESS,(INT_PTR)i);
          }
        }
      }
      return 0;
    ...
    case LVN_CANTDRAG:
		...
		case LVN_DROP:
		{
			...
			if (This->notifyproc && ds->on>=0) This->notifyproc(This,DLVN_SELCHANGE,ds->on);
 			...
 			if (ds->in_lv==0){
 				if (This->notifyproc) This->notifyproc(This,DLVN_DROP,(INT_PTR)ds);
 				break;
 			}
...
void hxc_dir_lv::delete_item()
{
  ...
  refresh_fol();
  if (notifyproc) notifyproc(this,DLVN_ITEMDELETED,(INT_PTR)file.Text);
}
//---------------------------------------------------------------------------
void hxc_dir_lv::rename_item()
{
  ...
  if (name.NotEmpty()){
    ...
    if (notifyproc){
      if (type==1){
        notifyproc(this,DLVN_FOLDERMOVED,(INT_PTR)file.Text);
      }
...
//---------------------------------------------------------------------------
int hxc_dir_lv::drag_popup_notifyproc(hxc_popup *pop,int mess,INT_PTR i)
{
