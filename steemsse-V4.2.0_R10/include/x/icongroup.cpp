...
//---------------------------------------------------------------------------
DWORD IconGroup::LoadIconsFromMemory(Display *pass_Disp,BYTE *Mem,int IcoWid,
...
DWORD IconGroup::LoadIcons(Display *pass_Disp,char *File,int IcoWid,DWORD RGBTransparent)
{
  ...
  if (fp){
    ...
    if (fread(&FileInfo,1,sizeof(FileInfo),fp)==sizeof(FileInfo)){
      if (fread(&loaded_bi,1,sizeof(loaded_bi),fp)==sizeof(loaded_bi)){
        if (loaded_bi.biCompression==0 /*BI_RGB*/){
	        if (HIWORD(loaded_bi.biPlanes_biBitCount)==24){
	        	BmpBitsPerPixel=24;
      	  }else if (HIWORD(loaded_bi.biPlanes_biBitCount)==8){
      		  BmpBitsPerPixel=8;
      		  if (fread(Pal,1,256*4,fp)!=256*4) Err=IGERR_NOTBMP;
          }else if (HIWORD(loaded_bi.biPlanes_biBitCount)==1){
      		  BmpBitsPerPixel=1;
          }else{
	          Err=IGERR_WRONGDEPTH;
    ...
    if (Err==0){
      ...
      Pixels=new BYTE[NumBytes];
      fseek(fp,*(LPDWORD)(FileInfo+10) /*bfOffBits*/,SEEK_SET);
      if (fread(Pixels,1,NumBytes,fp)==NumBytes){
        FreeIcons();
        bi=loaded_bi;
        XD=pass_Disp;
        IconWidth=IcoWid;
    ...
    if (MaskImg){
      ...
      if (Depth==8){
      	WhiteCol=255;
  			...
  			DWORD BGRMask[3]={(DWORD)Img->blue_mask,(DWORD)Img->green_mask,(DWORD)Img->red_mask};
