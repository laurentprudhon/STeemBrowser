...
typedef int HXC_DIRLVNOTIFYPROC(hxc_dir_lv*,int,INT_PTR);
typedef HXC_DIRLVNOTIFYPROC* LPHXC_DIRLVNOTIFYPROC;

class hxc_dir_lv
{
private:
	static int lv_notifyproc(hxc_listview*,int,INT_PTR);
  static int drag_popup_notifyproc(hxc_popup*,int,INT_PTR);
public:
	hxc_dir_lv();
	~hxc_dir_lv() {};
