...
bool hxc_scrollarea::create(Display*d,Window daddy,int x,int y,int w,int h,LPHXC_SCROLLAREANOTIFYPROC np)
{
  ...
  SetProp(XD,handle,cWinProc,(DWORD_PTR)surface_WinProc);
  SetProp(XD,handle,cWinThis,(DWORD_PTR)this);
  SetProp(XD,frame,cWinProc,(DWORD_PTR)frame_WinProc);
  SetProp(XD,frame,cWinThis,(DWORD_PTR)this);
...
int hxc_scrollarea::scrollbar_notify_proc(hxc_scrollbar *sb,int mess,INT_PTR i)
{
