...
*/
  ...
  if (res_users==0){
    ...
    font=NULL;
    for (int fp=0;fp<font_sl.NumStrings;fp++){
      font=XLoadQueryFont(XD,font_sl[fp].String);
      if (font) break;
    }
    if (font==NULL) {
#if defined(SSE_UNIX_STATIC_VAR_INIT)
...
void hxc::free_colours(Display*XD){
  unsigned long ColList[6]={    col_grey,    col_border_dark,    col_border_light,
...
bool hxc::SetProp(Display *XD,Window Win,XContext Prop,DWORD_PTR Val)
{
  if (XD==NULL) return 0;
...
DWORD_PTR hxc::GetProp(Display *XD,Window Win,XContext Prop)
{
  if (XD==NULL) return 0;
...
/*  Doing this in the constructor doesn't seem to work in later versions
    ...
    There's some C++ subtlety to understand here, when we have time...
*/
  if (font_sl.NumStrings==0){
    font_sl.Sort=eslNoSort;
    font_sl.Add("-adobe-helvetica-bold-r-normal-*-14-100-*-*-*-*-iso8859-1");
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
Window hxc::create_modal_dialog(Display *XD,int w,int h,char *title,
                                bool create_okcancel) {
  load_res(XD);
  ...
  SetProp(XD,handle,cWinProc,(DWORD_PTR)modal_dialog_winproc);
  SetProp(XD,handle,cWinThis,(DWORD_PTR)XD);
...
#endif//#ifndef _HXC_CONTROLS_CPP
