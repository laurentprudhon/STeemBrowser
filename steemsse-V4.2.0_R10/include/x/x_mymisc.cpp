...
//https://gist.github.com/jarcode-foss/c39bacf97cd88a02d7c6d6d70b482dd5
int ChangeIcon(Display *Disp,Window Win,XImage *xi) {
  int ec=0;
  size_t w=xi->width,h=xi->height;
  char *data=xi->data;
  size_t sz = w * h;
  size_t asz = sz + 2;
  ...
  for (size_t y = 0; y < h; ++y) {
    for (size_t x = 0; x < w; ++x) {
      DWORD pix=(DWORD) data[4*(x+(y*h)+0)];
      SWAP_BIG_ENDIAN_DWORD(pix);
      off[y*h+x+2]=pix;
    }
  }
  ...
  off[0] = w;
  off[1] = h;
  ec=XChangeProperty(Disp, Win, XInternAtom(Disp, "_NET_WM_ICON", true),
                  XA_CARDINAL, 32, PropModeReplace,
              (const unsigned char*) off, asz);
  //assert(ec==1); // poorly documented
  free(off);
  return ec;
}
...
EasyStr find_file_i(EasyStr path,EasyStr fp) //search for file f in path
{
  NO_SLASH(path);
  int sp;
  for(;;){
    for (sp=0;fp[sp];sp++) if (fp[sp]=='/')break;
    if (fp[sp]){  //found slash
      EasyStr f2=fp;
      ...
      if (dp){
        for(;;){
          ep=readdir(dp);
          if (ep==NULL) break;
          if (strcmpi(ep->d_name,fp)==0){ //filenames match apart for case
...
//---------------------------------------------------------------------------
bool SetProp(Display *XD,Window Win,XContext Prop,DWORD_PTR Val)
{
  return XSaveContext(XD,Win,Prop,(XPointer)Val)==0;
}
//---------------------------------------------------------------
DWORD_PTR GetProp(Display *XD,Window Win,XContext Prop)
{
  XPointer Dat=0;
  if(XFindContext(XD,Win,Prop,&Dat))Dat=0;
  return (DWORD_PTR)Dat;
}
//---------------------------------------------------------------------------
DWORD_PTR RemoveProp(Display *XD,Window Win,XContext Prop)
{
...
//---------------------------------------------------------------------------
void shell_execute(char *path,char *replacements,Display *XD)
{
  if (fork()==0){
    // You get here in the child process
    if (XD) close(ConnectionNumber(XD));

    EasyStr comline=path;
    if (replacements){
      EasyStr replace_list=replacements;
      int n_rep=0;
      char *p=replace_list;
      while (p[0]){
        if (p[0]=='\n') p[0]=0, n_rep++;
      ...
      p=replace_list;
      for (int n=0;n<n_rep;n+=2){
        if (p[0]){
          // p=what to replace, p+strlen(p)+1=replacement text
          while (comline.InStr(p)>=0){
            int i=comline.InStr(p);
            comline.Delete(i,strlen(p));
            comline.Insert(p+strlen(p)+1,i);
          }
        }
        if (n+2>=n_rep) break;
        p+=strlen(p)+1;
    ...
    char args[4096],*p=args,*arglist[11];
    memset(args,0,sizeof(args));
    memset(arglist,0,sizeof(arglist));
    bool in_quotes=0,in_arg=0;
    int n_arg=0;
    for (int i=0;i<comline.Length();i++){
      if (comline[i]!=' ' || in_quotes){
        if (comline[i]=='\"'){
          in_quotes=!in_quotes;
        }else{
          if (in_arg==0) arglist[n_arg]=p;
          in_arg=true;
          *(p++)=comline[i];
        }
      }else{
...
/*
    execlp("netscape", "netscape",Text.Text,NULL);
    execlp("konqueror", "konqueror",NULL);
*/
