...
NOTE:
When you use EasyStr variables in a class, the class initialiser can't be
...
private:
  ...
  void ResizeBuf(int size);
  int bufsize; // not INT_PTR
  static char numbuf[32];
  //static char Empty_Text[4];
  static char Empty_Text[sizeof(INT_PTR)];
public:
  EasyStr();
  EasyStr(char);
  EasyStr(char *);
  ...
  EasyStr(bool);
#ifdef _WIN64
  // Windows-only, with SSE_X64 those types are long in GCC - TDDO?
  EasyStr(INT_PTR);
  EasyStr(DWORD_PTR);
#endif
  ~EasyStr();
  ...
  //int GetBufSize();
  EasyStr& Insert(EasyStr ToAdd,int Pos); //What to add, where to add it
  EasyStr& Delete(int start,int count);   //First char to delete, number of chars to delete
  char *Right();
  char *Rights(int numchars);
  char RightChar();
  EasyStr Lefts(int numchars);
  EasyStr Mids(int firstchar,int numchars);
  EasyStr UpperCase();
  EasyStr LowerCase();
  EasyStr& LPad(int NewLen,char c);      //New length of string, char to pad with
  EasyStr& RPad(int NewLen,char c);      //Ditto
  int InStr(char *);
