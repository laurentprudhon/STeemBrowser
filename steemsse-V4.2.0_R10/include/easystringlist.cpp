...
#endif
void EasyStringList::ResizeBuf(int const NewSize) {
  Size=MAX(NewSize,(int)ESL_MINSIZE); // NewSize<=Size
  if(NumStrings)
  {
    ...
    NumStrings=MIN(NumStrings,NewSize); // NumStrings<=NewSize
    TESL_Data *NewInfo=new TESL_Data[Size];
    if(NumStrings/*&NumStrings<=Size*/) // NumStrings<=Size
...
int EasyStringList::InsertAt(int n,long nDat,char *AddStr,...) {
  //  used in Unix build
  // fix context menu in disk manager xsteem
  va_list vl;
  va_start(vl,AddStr);
